 ...
 * CHANGES
        ...
        27.06.2003 nataly   - для Уральска слово "район" заменено на "область"
        ...
        15.12.2004 dpuchkov - добавил shared пеерменные ch_date и ch_KS.
        07.06.2006 dpuchkov - добавил тег для корректного отображения (у филиалов перенес Наименование города на строку выше).
        08.09.2006 u00777 - добавлен БИК.
        04/05/2012 evseev - изменил путь к логотипу
        28.08.2012 evseev - иин/бин
*/

def shared var ch_date as date .
def shared var ch_KS as char .
...
def var v-bik as char format "x(9)".
...
put stream m-out unformatted
   "<table align=""center"" border=""0"" cellpadding=""0"" cellspacing=""3"" width=""95%"">" skip
    "<tr><td align=""right""><img src=""c://tmp/top_logo_bw.jpg""></td></tr><br><br>" skip
    "<tr><td align=""right""><h3>Налоговый комитет</h3></td></tr>" skip
    "<tr><td align=""right""><h3>" v-rnn "</h3></td></tr>"   skip
    "<tr><td align=""right""><h3>" v-city "</h3></td></tr>" skip
    "<tr><td align=""right""><h3> КС-" ch_KS " от " ch_date "</h3></td></tr></table>" skip

    "<P align=""justify"" style=""font-size:14.0pt"">" cmp.name " " v-city ", БИК " v-bik " ставит Вас в известность о том, что " skip
    trim(trim(cif.prefix) + " " + trim(cif.name)) ", ИИН/БИН&nbsp;" cif.bin ", " skip.
...
case fl :
  when 1 then
      put stream m-out unformatted
        "открыт счет N&nbsp;" s-lon " в&nbsp;" crc.code " с&nbsp;" string(g-today, "99.99.9999").
  when 2 then do:
      find first sub-cod where sub-cod.sub = "cif" and sub-cod.acc = s-lon and sub-cod.d-cod = "clsa" no-lock no-error.
      find first codfr where codfr.codfr = "clsa" and codfr.code = sub-cod.ccode no-lock no-error.

      put stream m-out unformatted
        "закрыт счет N&nbsp;" s-lon " с&nbsp;" string(g-today, "99.99.9999") " по причине&nbsp;: " lc(codfr.name[1]).
  end.
  when 3 then
      put stream m-out unformatted
        "открыт счет N&nbsp;" s-lon " в&nbsp;" crc.code " с&nbsp;" string(g-today, "99.99.9999")
