 ...
 * MODULE
        Депозиты
 * DESCRIPTION
        Закрытие счетов по депозитам "метролюкс"
 * RUN
        вызов из процедуры
 * CALLER
        tdacls.p
...
End Function.
   ...
   if not avail aaa then do:
      run savelog( "tdaclose8", aaa.aaa + " Не найден счет").
...
/*   if aaa.opnamt = 0 then do:
   ...
   if not avail acvolt then do:
      run savelog( "tdaclose8", aaa.aaa + " счет открыт НЕКОРРЕКТНО").
   ...
   if (aaa.cr[1] - aaa.dr[1]) - aaa.hbal <> 0 then do:
      run savelog( "tdaclose8", aaa.aaa + " Одним из пользователей было сделано частичное изъятие но выплата клиенту не произведена").
   ...
   for each aad where aad.aaa = aaa.aaa and aad.who = "bankadm" no-lock:
       v-sumchkamt1 = v-sumchkamt1 + /*aad.sumg*/ aad.cam - aad.dam .
   end.

   run savelog( "tdaclose8", " 125. " + aaa.aaa + "  " + string(v-sumchkamt1)).

   if (aaa.cr[1] - aaa.dr[1]) < d_1% then do:
      message "Не достаточно средст на 1м уровне для удержания %%. Досрочное закрытие невозможно!" skip "" view-as alert-box question buttons ok title "".
      return.
   end.
   ...
   /*if i-mon < 1 then do:
      ...
      run savelog( "tdaclose8", aaa.aaa + " Прошло меньше месяца").
   ...
   do:
      run savelog( "tdaclose8", aaa.aaa + " Прошло больше месяца").*/
      v-sumchkamt = 0.
      ...
      d_1% = v-sumchkamt1.
      d_3% = 0. /*(aaa.cr[2] - aaa.dr[2]).*/
   /*end.*/
   if date(acvolt.x3) <= g-today then d_1% = 0.

   run savelog( "tdaclose8", aaa.aaa + "Сумма в размере " + trim(string((aaa.cr[1] - aaa.dr[1]) - d_1% + d_3%  ,'z,zzz,zzz,zz9.99-')) + crc.code + " будет перечислена на счет!").
   ...
   if v-ans then run savelog( "tdaclose8", aaa.aaa + " v-ans = true").
   else run savelog( "tdaclose8", aaa.aaa + " v-ans = false").
   ...
   if g-today <> aaa.regdt then do:
         ...
         if d_3% > 0 and d_3% <= (aaa.cr[2] - aaa.dr[2]) then do:
            run savelog( "tdaclose8", aaa.aaa + " Проводка с 2 на 1 уровень").
         ...
         if d_1% > 0 and d_1% <= (aaa.cr[1] - aaa.dr[1]) then do:
            run savelog( "tdaclose8", aaa.aaa + " Проводка с 1 на 2 уровень").
            ...
            if rcode ne 0 then do:
               run savelog( "tdaclose8", aaa.aaa + " UNI0074 " + rdes).
         ...
         if s-amt11 > 0 then do:
            run savelog( "tdaclose8", aaa.aaa + " Проводка со 2 на 11 уровень").
            v-jh = 0.
            ...
            if rcode ne 0 then do:
               run savelog( "tdaclose8", aaa.aaa + " cda0003 " + rdes).
         ...
         if s-amt1 > 0 then do:
            run savelog( "tdaclose8", aaa.aaa + " Урегулируем разность если на 2 ур > чем на 11").
            v-jh = 0.
            ...
            if rcode ne 0 then do:
               run savelog( "tdaclose8", aaa.aaa + " uni0048 " + rdes).
               message "uni0048" ' ' rdes. pause. undo,retry.
            end.
         end.
         aaa.sta = "E".
         run savelog( "tdaclose8", aaa.aaa + " статус " + aaa.sta).

   end.
   else do:      /* счет открыли сегодня */
      aaa.sta = "E".
      message "Депозит успешно закрыт".
      run savelog( "tdaclose8", aaa.aaa + "Депозит успешно закрыт. статус " + aaa.sta).
      pause.
   end.
