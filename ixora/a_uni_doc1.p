 ...
 * BASES
        BANK COMM
 * CHANGES
                29.11.2011 Luiza - создан по подобию uni_doc.p для шаблонов uni00025, uni0068, uni0074,
                            uni0093, uni0052, uni0023.
                07/03/2012 Luiza - добавила шаблон uni0118
                07/03/2012 Luiza - отменила шаблон uni0023
                11.03.2012 damir - добавил печать оперционного ордера, printvouord.p.
                04.04.2012 damir - переход на новые форматы всех ордеров.
...
def var l as int.
def var jj as int.
def var v-cif as char format "x(50)".

define temp-table tempuni no-undo
    field trx as char format "x(7)"
    field des as char format "x(40)".
    create tempuni. tempuni.trx = "uni0025".
    create tempuni. tempuni.trx = "uni0074".
    create tempuni. tempuni.trx = "uni0093".
    create tempuni. tempuni.trx = "uni0052".
    /*create tempuni. tempuni.trx = "uni0023".*/
    create tempuni. tempuni.trx = "uni0068".
    create tempuni. tempuni.trx = "uni0118".
    create tempuni. tempuni.trx = "uni0163".
    create tempuni. tempuni.trx = "vnb0024".
for each tempuni.
    find first trxhead where trxhead.system = substring(tempuni.trx,1,3) and trxhead.code = int(substring(tempuni.trx,4,4)) no-lock no-error.
    if available trxhead then tempuni.des = trxhead.des.
end.

    DEFINE QUERY q-uni FOR tempuni.
    DEFINE BROWSE b-uni QUERY q-uni
           DISPLAY tempuni.trx no-label tempuni.des label "Выберите код шаблона " format "x(50)"
           WITH  7 DOWN.
    DEFINE FRAME f-uni b-uni  WITH overlay 1 COLUMN SIDE-LABELS row 5 COLUMN 25 width 80 NO-BOX.
...
__________________________________________________________________________________________" skip(1)
    ...
    ujo.whn label "Дата документа" skip
    ujo.info label "Клиент(наименование/ФИО)" format "x(50)" skip(1)
    b_link skip(1)
      ...
      if  ujolink.parnum  = 2 then do:
        ...
        if trim(v-cif) <> "" then do:
            ...
            if available aaa then do:
                ...
                ujolink.parval:screen-value in browse b_link = aaa.aaa.
                hide frame f-help.
                find first cif where cif.cif = v-cif no-lock no-error.
                ujo.info = cif.name.
                displ ujo.info with frame uni_main.
            end.
            else do:
                MESSAGE "СЧЕТ КЛИЕНТА НЕ НАЙДЕН.".
                ujolink.parval:screen-value in browse b_link = "".
                return.
            end.
        end.
...
on "u1" of this-procedure do:           /* Edit */
    ...
    enable b_link main_menu ujo.num ujo.chk ujo.info with frame uni_main.
...
on choose of main_menu in frame uni_main do:
    ...
    vparam = "".
    for each ujolink where ujolink.docnum eq seq_number /* and ujolink.parnum <= l */no-lock:
       find first w-cods where w-cods.number = ujolink.parnum no-error.
       if not available w-cods then vparam = vparam + ujolink.parval + vdel.
    end.
    templ = ujo.sys + ujo.code.
    run trxsim ("",templ, vdel, vparam, 1, output rcode, output rdes, output vparr).
    if rcode ne 0 then do:
       message rdes.
       pause.
       if rcode = 28 then run chgsts (m_sub, seq_number, "e28"). else run chgsts (m_sub, seq_number, "err").
    end.
   else do:
        run chgsts (m_sub, seq_number, "new").
        /*message "Необходим контроль в п.м. 2.4.1.12! ~n'Контроль универсальных операций'!" view-as alert-box.*/
    end.
    if_view = false.
    disable all with frame uni_main.
...
on row-leave of b_link in frame uni_main do:
    ...
    if new_document then do:
        ...
        if b_link:current-row-modified and  not (uni_template = "uni0023" and  ujolink.parnum = 3) then do:
    ...
    if ujolink.parnum = 2 then do:
        find first aaa where aaa.aaa = ujolink.parval no-lock no-error.
...
if new_document then do:
    ...
    /*run Get_template.*/
    OPEN QUERY  q-uni FOR EACH tempuni no-lock.
    ENABLE ALL WITH FRAME f-uni.
    wait-for return of frame f-uni
    FOCUS b-uni IN FRAME f-uni.
    uni_template = tempuni.trx.
    hide frame f-uni.
    displ uni_template with frame f_templ.
    ...
    display ujo.num ujo.chk ujo.jh ujo.whn ujo.info with frame uni_main.
...
Procedure Ujo_line.
    ...
    if new_document then do:
        ...
        if uni_template = "uni0023" and  param_number = 3 then ujolink.parval = "492120".
    end.
...
Procedure Create_Transaction.
    ...
    if avail dfb then do:
        if dfb.gl = 105210 or dfb.gl = 105220 then do:
    ...
    release cursts.
    find cursts where cursts.sub = "ujo" and  cursts.acc = seq_number no-lock no-error.
    if available cursts then do:
        if cursts.sts = "e28" then do:
            message "Нехватка средств для проведения операции!" view-as alert-box.
            undo, return.
        end.
        if cursts.sts = "err" then do:
            message "Документ сохранен с ошибками! ~n'Необходимо отредактировать'!" view-as alert-box.
            undo, return.
        end.
        if cursts.sts <> "con" then do:
            message "Документ должен пройти контроль в п.м. 2.4.1.12! ~n'Контроль универсальных операций'!" view-as alert-box.
            undo, return.
        end.
    end.
    else do:
...
Procedure Edit_document.
    ...
    enable b_link b_apply b_cancel ujo.num ujo.chk ujo.info with frame uni_main.
...
Procedure Print_vnebal_prih.
    ...
    put stream m-out "Trx.Nr." + string(s-jh) + "      " + ofc.name + "/" +
                    ofc.ofc + "     " + string(ujo.whn) + ujo.info format "x(78)" skip.
