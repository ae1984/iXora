...
End Function.
   ...
   if not avail aaa then do:
      run savelog( "tdaclose5", aaa.aaa + " Не найден счет").
      message "Не найден счет" aaa.aaa. pause. return.
   end.

   if aaa.opnamt = 0 then do:
      run savelog( "tdaclose5", aaa.aaa + " счет открыт НЕКОРРЕКТНО дата открытия не совпадает с взносом основной суммы").
   ...
   if not avail acvolt then do:
      run savelog( "tdaclose5", aaa.aaa + " счет открыт НЕКОРРЕКТНО").
   ...
   if (aaa.cr[1] - aaa.dr[1]) - aaa.hbal <> 0 then do:
      run savelog( "tdaclose5", aaa.aaa + " Одним из пользователей было сделано частичное изъятие но выплата клиенту не произведена").
   ...
   if i-mon < 1 then do:
      run savelog( "tdaclose5", aaa.aaa + " Прошло меньше месяца").
...
/*      d_1% = (aaa.cr[1] - aaa.dr[1]) - v-sumchkamt - aaa.stmgbal. */
   ...
   do:
      run savelog( "tdaclose5", aaa.aaa + " Прошло больше месяца").
      v-sumchkamt = 0.
...
pause 111. */
       ...
       if (aaa.cr[1] - aaa.dr[1]) >= (acvolt.bonusopnamt + v-sumchkamt + d_sumrt - (aaa.accrued - (aaa.cr[2] - aaa.dr[2]) - aaa.stmgbal)) then do:
          ...
          if i-mon >= 18 then do:
             run savelog( "tdaclose5", aaa.aaa + " i-mon >= 18").
             d_1% = 0.
        ...
        vendsum = 0.
               for each jl where jl.acc = aaa.aaa and jl.lev = 1 and jl.jdt >= aaa.expdt and jl.rem[1] begins "Выплата процентов" no-lock use-index acc :
                   if jl.dc <> "D" then do:
                      vendsum = vendsum + abs(jl.cam - jl.dam).
                   end.
               end.

              if g-today >= aaa.expdt then do:
                 d_1% = vendsum.
                 d_3% = 0.
              end.
   ...
   run savelog( "tdaclose5", aaa.aaa +  "Сумма в размере " + trim(string((aaa.cr[1] - aaa.dr[1]) - d_1% + d_3%  ,'z,zzz,zzz,zz9.99-')) + crc.code + " будет перечислена на счет!").
   ...
   if v-ans then run savelog( "tdaclose5", aaa.aaa + " v-ans = true").
   else run savelog( "tdaclose5", aaa.aaa + " v-ans = false").
   ...
   if g-today <> aaa.regdt then do:
         ...
         if d_3% > 0 and d_3% <= (aaa.cr[2] - aaa.dr[2]) then do:
            run savelog( "tdaclose5", aaa.aaa + " Проводка с 2 на 1 уровень ").
         ...
         if d_1% > 0 and d_1% <= (aaa.cr[1] - aaa.dr[1]) then do:
            run savelog( "tdaclose5", aaa.aaa + " Проводка с 1 на 2 уровень").
            ...
            if rcode ne 0 then do:
               run savelog( "tdaclose5", aaa.aaa + " UNI0074 " + rdes).
         ...
         if s-amt11 > 0 then do:
            run savelog( "tdaclose5", aaa.aaa + " Проводка со 2 на 11 уровень").
            v-jh = 0.
            ...
            if rcode ne 0 then do:
               run savelog( "tdaclose5", aaa.aaa + " cda0003 " + rdes).
         ...
         if s-amt1 > 0 then do:
            run savelog( "tdaclose5", aaa.aaa + " Урегулируем разность если на 2 ур > чем на 11").
            v-jh = 0.
            ...
            if rcode ne 0 then do:
               run savelog( "tdaclose5", aaa.aaa + " uni0048 " + rdes).
               message "uni0048" ' ' rdes. pause. undo,retry.
            end.
         end.
         aaa.sta = "E".
         run savelog( "tdaclose5", aaa.aaa + " статус " + aaa.sta).

   end.
   else do:      /* счет открыли сегодня */
      aaa.sta = "E".
      message "Депозит успешно закрыт".
      run savelog( "tdaclose5", aaa.aaa + "Депозит успешно закрыт. статус " + aaa.sta).
      pause.
   end.
