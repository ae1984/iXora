 ...
 * MODULE
        Финансовые отчеты
 * DESCRIPTION
        Отчет 1-СБ - подборка данных по одному филиалу
 * RUN
 ...
 * AUTHOR
        08.01.2004 nadejda
	16.02.2004 valery изменен принцип отбора резидентов
 * CHANGES
        09.10.2006 u00600 - алиас "ast" заменен на "txb" для совместимости c r-brfilial.i
        16/06/08  marinav - нерезидетны тоже учитываются, дата закрытия счета в sub-code
        26/09/08  marinav - нерезиденты не попадают
        07.07.10 marinav - исключила 4 июня
        02/08/211 evseev - из-за високосного года краткосросные депозиты попадали в долгосрочные. Решение if v-days = 366 then v-days = 365.
        20/01/2012 lyubov - включила суммы по вкладам, которые были отозваны (закрыты) в течении отчетного месяца
        23.02.2012 Lyubov - исключила валюту рубль
        01.03.2012 Lyubov - отменила последние изменения
 */
...
for each t-gl:
  for each txb.aaa where txb.aaa.gl = t-gl.gl no-lock:
    ...
    if txb.aaa.sta = "c" and txb.aaa.cltdt < v-dtb then do:
       find first txb.sub-cod where txb.sub-cod.sub = 'cif' and  txb.sub-cod.acc = txb.aaa.aaa and txb.sub-cod.d-cod = 'clsa' no-lock no-error.
               if avail txb.sub-cod and txb.sub-cod.rdt < v-dtb then next.
    end.
    ...
    if v-stroka = 4 then do:
      if i = 1 and t-gl.glstr = "2223" and (txb.aaa.expdt - txb.aaa.regdt < 3 or txb.aaa.expdt = ? ) then v-stroka = 2.
      else do:
        /* для срочных депозитов - деление на строки по срокам вклада */
        if txb.aaa.expdt = ? then v-stroka = 9.
        else do:

          v-days = txb.aaa.expdt - txb.aaa.regdt.
          if v-days = 366 then v-days = 365.
          v-mc = v-days / 30.
          v-god = v-days / 365.
          ...
          if v-god <= 1 then do:
            ...
            if v-mc > 3                then v-stroka = v-stroka + 3.
          end.
          else do:
            if v-god <= 5 then v-stroka = v-stroka + 4.
                          else v-stroka = v-stroka + 5.
          end.
        end.
      end.
    end.
