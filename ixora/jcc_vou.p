...
/** jcc_vou.p
    KONTS -- KASE (klienta voucher ) **/
/* 01.10.02 nadejda - наименование клиента заменено на форма собств + наименование */
/* 05.07.04 dpuchkov - добавлено поле Доверенное лицо перекомпиляция  */

{chbin.i}
define shared variable v_doc   like joudoc.docnum.
/* define shared variable l-dovperson as logical. */
...
put space(25) "КАССОВЫЙ ОРДЕР (ВЫПЛ.)" skip.
put
"============================================================================="
    skip
    cmp.name space(23)
...
if l-dovperson then do:
  put  "Доверенное лицо: " + cif.badd[1] skip.
end.
*/
...
if substring (siknauda, r-index (siknauda, ".") + 1, 2) eq "00" then do:
    if length (vardiem) lt 63 then put "Сумма прописью:" skip(2) vardiem + " " +
