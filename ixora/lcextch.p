 ...
 * MODULE
        Trade Finance
 * DESCRIPTION
        доп.расходы внешние External Charges
 * RUN
...
def new shared var s-event    like lcevent.event init 'extch'.
def new shared var s-number   like lcevent.number.
def new shared var s-sts      like lcevent.sts.
def new shared var s-ftitle   as char init ' EXTERNAL CHARGES '.
...
if s-ourbank <> 'TXB00' then do:
    message 'External Charges can be created only in Central Office!' view-as alert-box error.
    return.
end.
...
{mainlc.i
 ...
 &prefind    = " assign v-find    = yes
              ...
              repeat on endkey undo, return:
                  ...
                  s-lc = caps(s-lc).
                  find first LC where LC.LC = s-lc and lc.lc begins s-lcprod and lookup(lc.lcsts,'FIN,CLS,CNL') > 0  no-lock no-error.
                  if not avail LC then run lchelp3('FIN,CLS,CNL').
 ...
 &postfind = " if avail LC then v-lcsts = LC.LCsts.
               ...
               do on error undo,return:
                    v-yes = no.
                    if v-lcsts = 'FIN' then
                    message 'Do you want to create a new Event (External Charges)?' VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO TITLE ' QUESTION !' update v-yes.
                    display v-cif v-cifname s-lc v-lcsts v-lcerrdes s-number with frame frevent.
                    ...
                    if v-yes then do transaction:
                        ...
                        if avail lcevent then do:
                            if lcevent.sts <> 'FIN' then do:
                                ...
                                update s-number with frame frevent.
                                find last lcevent where lcevent.lc = s-lc and lcevent.event = s-event and lcevent.number = s-number no-lock no-error.
                                if avail lcevent then assign s-sts = lcevent.sts v-chose = yes.
                            end.
