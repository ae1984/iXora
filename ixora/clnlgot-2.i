 ...
 * MODULE
        Клиентская база
 * DESCRIPTION
        Установка льготных тарифов по EUROFOOD Co LTD (льготная группа 2)
 ...
 * AUTHOR
        29.12.2003 nadejda
 * CHANGES
        29.01.2004 nadejda - добавлены обрабатываемые тарифы по ТЗ 714
...
if p-cif = "" then do:
  for each {1}cif where {1}cif.pres = "2" no-lock:
    create t-cif.
    t-cif.cif = {1}cif.cif.
  end.
end.
...
if not avail t-cif then do:
  message skip " Клиенты с льготным обслуживанием вида 2 не найдены !" 
          skip(1) view-as alert-box button ok title " ОШИБКА ! ".
  return.
end.


def var v-tarif1 as char init "101,102,104,109,111,112,121,126,137,141,142,147,151,156,157,163,164,165,166,167,168,177,192,201,214,216,401,403,409,801,807". /* тарифы бесплатные */
def var v-tarif2 as char init "123,130,131,132,133,134,135,136,148,153,158,211,950,951,970,971,972". /* тарифы 5% */
def var v-tarif3 as char init "204,205,218,802,804". /* тарифы особые */
