...
run savelog('dcls_ForteUniversal','74').
find first aaa where lookup(aaa.lgr,"B09,B10,B11") > 0 no-lock no-error.
if not avail aaa then return.
fname1 = 'ForteUniversal_' + substring(string(g-today),1,2) + substring(string(g-today),4,2) + '.txt'.
output stream m-out to ForteUniversal.txt.
...
run savelog('dcls_ForteUniversal','89').
for each lgr where lgr.intcal = "D" use-index intcal:
    if lookup(lgr.lgr,"B09,B10,B11") = 0 then next.
    vcnt = 0. vtotaip = 0. vtotaipc = 0.
    run savelog('dcls_ForteUniversal','93. ' + lgr.lgr).
    c-aaa:
    for each aaa where aaa.lgr = lgr.lgr  break by aaa.crc:
        run savelog('dcls_ForteUniversal','96. ' + aaa.aaa).
        ...
        run savelog('dcls_ForteUniversal','109. ' + aaa.aaa).
        ...
        if lgr.intcal = "D" then do: /*ежедневное начисление*/
           ...
           DoUpr = false.
           uv-date = ?.
           uv-date = EventInRange(lgr.intcal, g-today, s-target - 1). /*Вычисляем количество начисленных процентов  */
           if uv-date <> ? then do:
        ...
        if not avail crc then do:
           run mail("id00787@metrocombank.kz", "METROCOMBANK <abpk@metrocombank.kz>", "ОШИБКА: в dcls_ForteUniversal",
        ...
        if IntBase() - jl.cam = 0  then vbal = IntBase().
        v-diffsum = IntBase() - jl.cam.
        if jl.cam * 3 >= v-diffsum then vbal = IntBase().
        if jl.cam * 3 < v-diffsum then do:
           vbal = jl.cam * 3 + jl.cam.
           run savelog('dcls_ForteUniversal','155. ' + aaa.aaa + ' ' + string(IntBase()) = ' -> ' + string(vbal)).
           run tdaremholda(aaa.aaa).
           run tdasethold(aaa.aaa, vbal).
        end.

        run savelog('dcls_ForteUniversal','161. ' + aaa.aaa).
        if DoPay then do:
           if ev-date <> ? then do:
              ...
              run savelog('dcls_ForteUniversal','168. ' + aaa.aaa).
              create accr-m10.
              assign
              ...
              run PayInterests.
              run savelog('dcls_ForteUniversal','191. ' + aaa.aaa + ' ' + string(ev-date)).
              run savelog('dcls_ForteUniversal','192. ' + aaa.aaa + ' ' + string(aaa.expdt)).
              /*if v-intday2 > 0 and ev-date < aaa.expdt - 1 then do:
                  v-m10 =  v-intday2 * vbal * aaa.rate / (aaa.base * 100).
                  run savelog('dcls_ForteUniversal','177. ' + aaa.aaa).
                  create accr-m10.
                  assign
                      accr-m10.aaa     = aaa.aaa
                      accr-m10.rate    = aaa.rate
             ...
             if DoUpr then do:
                if uv-date <> ? then do:
                   ...
                   v-intday2 = s-intday - v-intday1.
                   v-m10 =  vbal * aaa.rate * v-intday1 / (aaa.base * 100).
                   run savelog('dcls_ForteUniversal','206. ' + aaa.aaa).
                   create accr-m10.
                   assign
                       accr-m10.aaa     = aaa.aaa
                       accr-m10.rate    = aaa.rate
                   ...
                   if v-intday2 > 0 then do:
                      ...
                      v-m10 =  v-intday2 * vbal * aaa.rate / (aaa.base * 100).
                      run savelog('dcls_ForteUniversal','235. ' + aaa.aaa).
                      create accr-m10.
                      assign
                          accr-m10.aaa     = aaa.aaa
                          accr-m10.rate    = aaa.rate
                ...
                run savelog('dcls_ForteUniversal','261. ' + aaa.aaa).
                create accr-m10.
                assign
                    accr-m10.aaa     = aaa.aaa
                    accr-m10.rate    = aaa.rate
        ...
        if v-accrued > 0  then do:
           run savelog('dcls_ForteUniversal','288. ' + aaa.aaa).
           ...
           run savelog('dcls_ForteUniversal','316. ' + aaa.aaa).
        end.
  ...
  if lgr.crc ne 1 then do:
    ...
    run savelog('dcls_ForteUniversal','394. ' + aaa.aaa).
  end. /* for each lgr */
...
output stream m-out close.
unix silent mv ForteUniversal.txt value(fname1).
                       ...
                       if cif.geo = '022' and cif.type = 'B' then do:
                          ...
                          if aaa.crc <> 1 then do:
                             vparam = string(d_tax)
                                  ...
                                  + vdel + string("15% подоходный налог, " + cif.name + " " + cif.jss)
                                  + vdel + v-nlg
                                  + vdel + string("15% подоходный налог, " + cif.name + " " + cif.jss).
                             ...
                             vparam = string(d_tax) + vdel + string(aaa.crc) + vdel +  aaa.aaa + vdel + string(v-nlg) + vdel +
                                      string("15% подоходный налог, " + cif.name + " " + cif.jss) + vdel + "390".
