...
def shared var s-optproc like optitsec.proc .
...
form with title s row 7 column 35 scroll 1 h down overlay frame frm.
form v-ofc label "Исполнитель" 
  with row 9 column 55 overlay frame frm1 .
form "                       INS- Добавить  F10- Удалить"
  with column 15 row 22 no-box overlay frame frm2 .

hide frame frm. hide frame frm1. hide frame frm2.

do transaction :
  find first optitsec where optitsec.proc = s-optproc 
       exclusive-lock no-error.
  if not avail optitsec or optitsec.ofcs = "" then do:
    ...
    if choise then do:      
      if not avail optitsec then do:
        create optitsec.
        assign optitsec.proc = s-optproc.
      end.
      do on endkey undo , leave :
        ...
        if not avail(ofc) then do :
          message color messages "Некорректный исполнитель !" . pause .
          leave .
        end .
        optitsec.ofcs = v-ofc + ",".
      end .
  ...
  do while n <> n1 :
    n1 = n .
    n = index(substring(optitsec.ofcs, n + 1), ",") + n .
    do while substring(optitsec.ofcs, n + 1, 1) = "," :
      optitsec.ofcs = substring(optitsec.ofcs, 1, n) + substring(optitsec.ofcs, n + 2) .
    end .
  end .
  optitsec.ofcs = optitsec.ofcs + "," .

  n = 1 .
  do while entry(n, optitsec.ofcs) <> "" :
    te = entry(n, optitsec.ofcs) .
    find first ofc where ofc.ofc = te no-lock no-error .
    if avail(ofc) then
      err = false .
    else do :
      err = false .
      message "Неизвестный исполнитель " + te + ". Удалить ? " update err .
    end .
    if not err then do :
      create ofct .
      ofct.ofc = te .
