/* kdrisob.p   Электронное кредитное досье
 * MODULE
        Кредитное досье
 * DESCRIPTION
        Источники погашения
...
define frame fr skip(1) " РИСКИ, СВЯЗАННЫЕ С ЗАЛОГОВЫМ ОБЕСПЕЧЕНИЕМ "  skip(1)
      ...
      if not avail kdaffil then do:
            create kdaffil. 
            kdaffil.bank = s-ourbank. kdaffil.code = '29'. kdaffil.kdlon = s-kdlon. 
