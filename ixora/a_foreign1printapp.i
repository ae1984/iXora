 ...
 * DESCRIPTION
        Описание - 2. ПЕРЕВОДЫ В ИН ВАЛЮТЕ БЕЗ ОТКРЫТИЯ СЧЕТА | 1. ОТПРАВЛЕНИЕ ПЕРЕВОДА
 * RUN
...
*/
v-inputfile = "/data/export/statement4.htm".
...
repeat:
    ...
    repeat:
        ...
        if v-str matches "*kemvydan*" then do:
            if whopassp <> "" then v-str = replace (v-str,"kemvydan",substr(trim(whopassp),1,21)).
        ...
        if v-str matches "*accpolcseht*" then do:
            if v_chpol <> "" then v-str = replace (v-str,"accpolcseht",trim(v_chpol)).
        ...
        if v-str matches "*iiknizhekomis*" then do:
            v-str = replace (v-str,"iiknizhekomis","-").
            next.
        end.
        ...
        if v-str matches "*celperev1*" then do:
            if substr(v-naznplat,1,38) <> "" then v-str = replace (v-str,"celperev1",trim(substr(v-naznplat,1,38))).
            else v-str = replace (v-str,"celperev1","").
            next.
        end.
        if v-str matches "*celperev2*" then do:
            if substr(v-naznplat,39,53) <> "" then v-str = replace (v-str,"celperev2",trim(substr(v-naznplat,39,53))).
            else v-str = replace (v-str,"celperev2","").
            next.
        end.
        if v-str matches "*celperev3*" then do:
            if substr(v-naznplat,92,53) <> "" then v-str = replace (v-str,"celperev3",trim(substr(v-naznplat,92,53))).
            else v-str = replace (v-str,"celperev3","").
            next.
        end.
        if v-str matches "*celperev4*" then do:
            if substr(v-naznplat,145,28) <> "" then v-str = replace (v-str,"celperev4",trim(substr(v-naznplat,145,28))).
        ...
        if v-str matches "*sdgsdfkgjkjbsdkjbhs*" then do:
            v-str = replace (v-str,"sdgsdfkgjkjbsdkjbhs","[&nbsp;&nbsp;] &nbsp; получателя &nbsp;[&nbsp;&nbsp;] &nbsp; отправителя &nbsp; [&nbsp;&nbsp;] &nbsp; получателя/отправителя").
            next.
        end.
