 ...
 * DESCRIPTION
        Процедура принятия платежей Астана Энерго в г. Астана - список
 * RUN
 ...
 * MENU
        База в г. Алматы, 3.2.10.6  (СУ - 4)
 * AUTHOR
        31/12/99 pragma
 * CHANGES
        20.06.2003 kanat Добавил печать чека при создании платежа
        21/04/04 sasco Раширил номер документа
        09/06/04 kanat добавил при удалении причину удаления - другая причина
*/
...
def browse b1
    ...
    display 
        ...
        commonpl.accnt                 label "Счет" format ">>>>>>9"
        commonpl.npl                   label "Назначение платежа" format "x(26)"
        commonpl.Sum                   label "~ Сумма"   format ">>,>>>,>>9.99"
        with 14 down title "АстанаЭнергоСервис" no-labels. 
...
ON CHOOSE OF bedt IN FRAME f1
    do:
        run as-es-in (dat, false, rowid(commonpl)).              
        b1:refresh().
    end.
...
ON CHOOSE OF bnew IN FRAME f1
    do:
    if true then do:
        run as-es-in (dat, true, rowid(commonpl)).
  ...
  run bks(s_payment,"NO").
        ...
        if return-value <> "" then do:
            open query q1 for each commonpl where date = dat and commonpl.txb = seltxb and
...
ON CHOOSE OF bdel IN FRAME f1 do:
    ...
    MESSAGE "Удалить?"
       ...
       TITLE "АстанаЭнергоСервис" UPDATE choice as logical.
        case choice:
          when true then do transaction:
            ...
            {comdelpay.i
             ...
             &where = " commonpl.grp = selgrp and commonpl.type = tcommonpl.type and commonpl.service = tcommonpl.service and commonpl.sum = tcommonpl.sum and commonpl.accnt = tcommonpl.accnt and commonpl.counter = tcommonpl.counter "
            }
            ...
            open query q1 for each commonpl where commonpl.date = dat and commonpl.txb = seltxb and 
