 ...
 * MODULE
        Платежная система
 * DESCRIPTION
        сверка Гросса по финальной выписке. 
 * RUN
 ...
 * AUTHOR
        31.01.08 marinav
 * CHANGES
        12.09.2008 galina - загрузка подтверждений по открытию/закрытию счетам юр.лиц
        14.05.2009 galina - загрузка подтверждений по изменению номера счетам юр.лиц
        29.12.10   marinav - сначала удаляются файлы с L:
*/
...
v-cls = g-today . 
 ...
 u_pid = "lbin" .        
       ...
       repeat : 
          ...
          if search(num[9]) eq num[9]  then do: 
                 ...
                 repeat: 
                     ...
                     do: 
                       if substr(t-str,4,4) = "O950" then do:
                          create t-qin.
                          t-qin.fname = "950 " + substr(num[9],length(v-lbin) + 1 ) + " " + num[6] + num[7] + " " + num[8] +  ",". 
   ...
   /*!!!*/do transaction:
        run lb-check ('gross').   /* сверка */

        MESSAGE "" . pause 0 . 
                                                                                                                                           
        v-text = " Сверка GROSS проведена : " + 
...
/*загрузка подтверждений по счетам юр.лиц*/
      run MT998_400_in.
/*загрузка подтверждений по изменению номера счета юр.лиц*/      
      run MT998aaa20_in.
/* Архивация */
    ...
    if yn then  do:
     ...
     /*unix value("ssh " + v-lbhst + " erase /q " + v-lbeks + "out\\\\*.*"  ).*/
