 ...
 * AUTHOR
        31/12/99 pragma
 * CHANGES
            07/05/2012 Luiza отменила команду экран
...
repeat:
    ...
    v-select = 0.
    run sel2 (" УДЕРЖАНИЕ КОМИССИИ ", "1. УДЕРЖАНИЕ КОМИССИИ СO СЧЕТА КЛИЕНТА |2. УДЕРЖАНИЕ КОМИССИИ БЕЗ ОТКРЫТИЯ СЧЕТА  |3. ВЫХОД ", output v-select).
    ...
    case v-select:
        ...
        when 2 then do:
            ...
            if v-ek = 1 then run auto_menu("2").
            if v-ek = 2 then do:
...
define sub-menu sub_ord
    menu-item o_print label "Печать".

define  menu u_menu menubar
    sub-menu sub_doc label "Документ"
    sub-menu sub_trx label "Транзакция"
    sub-menu sub_stm label "Штамп"
    sub-menu sub_ord label "Платежный ордер".

if v-select = 2  and v-ek = 2 then MENU-ITEM t_nal:SENSITIVE = true.
...
on choose of menu-item d_new do:
    ...
    new_document = true.
    if v_prg = "1" then run a_com1 persistent set var_handle (input new_document).
    if v_prg = "2" then run a_com2 persistent set var_handle (input new_document).
end.
...
on choose of menu-item o_print do:
    if valid-handle(var_handle) then do:
        run PrintPayDoc in var_handle.
    end.
