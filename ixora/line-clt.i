...
repeat:
  find first {&line}
       where {&line}.{&head} eq {&head}.{&head} use-index {&index} no-error.
  ...
  update clt.grp with frame xf.
  if clt.grp = "R" then
  update {&fldupdt} with frame clt.
  else if clt.grp = "M" then
  update {&fldupdt1} with frame clts.
  else
  update {&fldupdt2} with frame cltk.
