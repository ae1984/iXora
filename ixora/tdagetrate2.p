...
def output parameter vrate2 like aaa.rate.
...
if lowterm  = lowlowvalue and highterm = highhighvalue then do:
   find last prih where prih.pri = pri.pri and prih.until = vuntil no-lock no-error.
   if available prih then do: vrate = prih.rat. vrate2 = prih.rat. end.
   else do: vrate = pri.rate. vrate2 = pri.rate. end.
   return.
end.
