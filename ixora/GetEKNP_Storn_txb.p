 ...
 * BASES
        BANK COMM TXB
 * CHANGES
        21.12.2012 damir - changing copy GetEKNP_Storn.p, uses TXB base. Внедрено Т.З. № 1620.
*/
...
find txb.trxcods where txb.trxcods.trxh = vjh and txb.trxcods.trxln = vln and txb.trxcods.codfr = "locat" no-lock no-error.
if available txb.trxcods then do:
    if vdc = "C" then substring(KOd,1,1) = substring(txb.trxcods.code,1,1).
    else substring(KBe,1,1) = substring(txb.trxcods.code,1,1).
end.

find txb.trxcods where txb.trxcods.trxh = vjh and txb.trxcods.trxln = vln and txb.trxcods.codfr = "secek" no-lock no-error.
if available txb.trxcods then do:
    if vdc = "C" then substring(KOd,2,1) = substring(txb.trxcods.code,1,1).
    else substring(KBe,2,1) = substring(txb.trxcods.code,1,1).
end.

find txb.trxcods where txb.trxcods.trxh = vjh and txb.trxcods.trxln = vln and txb.trxcods.codfr = "spnpl" no-lock no-error.
