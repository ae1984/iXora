 ...
 * BASES
        BANK
 * AUTHOR
        18.09.2006 u00600 - автоматическое проставление реквизитов для п.8.3.3 - дебиторы с s-rotrz
 * CHANGES
        27/09/2006 u00600 - run psroupp-deb
                            наименование формы c psror-deb.f на psror.f (возвращение к старой форме)
...
{chbin.i}
 ...
 find first remdeb where remdeb.remtrz = s-remtrz no-lock no-error.   /*31.08.06 u00600*/
 if avail remdeb then v-grp = remdeb.grp.
 else v-grp = 0.
...
&predelete = "
               ...
               exclusive-lock no-error. /*закрыла до подписания акта u00600 26/07/06 удаление списания дебиторов*/
               find first remdeb where remdeb.remtrz = s-remtrz
               exclusive-lock no-error. /*u00600 31/08/06 удаление дебиторов*/
               ...
               if remtrz.jh1 ne ? or remtrz.jh2 ne ? or m_pid ne que.pid
                ...
                if avail debujo then delete debujo .
                if avail remdeb then delete remdeb . end . "
