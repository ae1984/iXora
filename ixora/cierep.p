...
{image2.i}
{report1.i 180}
vtitle = "LORO RЁґINU IZEJO№IE ATLIKUMI - CIET…S VAL®TAS".
{report2.i 180}
put skip.
for each cif where cif.cgr eq 410 no-lock:
    
    /* один филиал */
    for each aaa where aaa.cif eq cif.cif /*use-index cif*/
        and substring(aaa.aaa,1,3) < "200" no-lock:
        /*{report2.i 132}*/
        /*for each crc where crc.crc eq aaa.crc and crc.crc ne 4 and
            crc.crc > 1 and crc.crc <= 20 no-lock:*/
        for each crchs where crchs.crc eq aaa.crc and crchs.hs eq "H" no-lock:
            ...
            DO i = 1 to lastday:
                ndate[i] = adate.
                    ...
                    find last crchis where crchis.crc eq aaa.crc
                        and crchis.sts ne 9 and crchis.rdt le ndate[i]
                        no-lock no-error.

                    find first wfil where wfil.wcif eq aaa.cif
                        and wfil.wcrc eq aaa.crc no-error.
                    ...
                    wfil.wloro[i] =
                        aab.bal * crchis.rate[1] / crchis.rate[9].
...
ndate[i] = adate.
    ...
    find crchs where crchs.crc eq crc.crc and crchs.hs eq "H" no-lock no-error.
    if not available crchs then next.
    find last  glday where glday.gdt le ndate[i]
        and glday.gl eq ppa and glday.crc = crc.crc /*132100*/ no-lock no-error.
        find last crchis where crchis.crc eq glday.crc
            and crchis.sts ne 9 and crchis.rdt le ndate[i] no-lock no-error.
                if available glday then do:
                        wbal[i] = wbal[i] +
                            glday.bal * crchis.rate[1] / crchis.rate[9].
                        ost = wbal[i].
                        kdate = ndate[i].
                end.
    end.
...
DO i = 1 to lastday:
ndate[i] = adate.
    for each crc where crc.sts <> 9 and crc.crc <> 1:
    find crchs where crchs.crc eq crc.crc and crchs.hs eq "H" no-lock no-error.
    if not available crchs then next.
...
ndate[i] = adate.
    ...
    find last glday where glday.gdt le ndate[i]
        ...
        find last crchis where crchis.crc eq glday.crc and
            crchis.sts ne 9 and crchis.rdt le ndate[i] no-lock no-error.
                if available glday then do:
                    nbal[i] = nbal[i] +
                        glday.bal * crchis.rate[1] / crchis.rate[9].
                    sto = nbal[i].
...
for each cif where cif.cgr eq 407 no-lock:
    /*viena banka*/
    for each aaa where aaa.cif eq cif.cif and aaa.crc ne 1
        and substring(aaa.aaa,1,3) < "200" no-lock:
        /*{report2.i 132}*/
        /*
        for each crc where crc.crc eq aaa.crc and
            crc.crc > 1 and crc.crc <= 20 no-lock:
            */
        for each crchs where crchs.crc eq aaa.crc and crchs.hs eq "H" no-lock:
            adate = ddate.
            DO i = 1 to lastday:
                ndate[i] = adate.
                find last crchis where crchis.crc eq aaa.crc
                    and crchis.sts ne 9 and crchis.rdt le ndate[i] no-lock
                    no-error.
