 ...
 * BASES
        BANK COMM
 * CHANGES
        10/12/2009 galina - добавила приложение 4
...
{r-brfilial.i &proc = "pkpogash(v-dt1,v-dt2)"}

if v-select <> 1 then find first txb where txb.consolid and txb.txb = v-select - 2 no-lock no-error.
for each pkanketa where pkanketa.docdt >= v-dt1 and pkanketa.docdt <= v-dt2 and ((pkanketa.bank = txb.bank and v-select <> 1 ) or v-select = 1)  and pkanketa.sts = "99" no-lock: /* break by pkanketa.bank by pkanketa.crc:*/
  ...
  if not avail pkanketh or pkanketh.rescha[1] = '' or pkanketh.resdec[1] <= 0 then next.    

       create pk-lnpogfil.
       assign pk-lnpogfil.refamt = pkanketa.summa
              pk-lnpogfil.bank = pkanketa.bank
              pk-lnpogfil.crc = pkanketa.crc. 
end.

for each pk-lnpogfil no-lock break by pk-lnpogfil.bank by pk-lnpogfil.crc:
  v-refcount = v-refcount + 1.
  v-refsum = v-refsum + pk-lnpogfil.refamt.
  if last-of(pk-lnpogfil.crc) then do:
       create pk-lnpogfiltot.
       assign pk-lnpogfiltot.refamt = v-refsum
              pk-lnpogfiltot.refcount = v-refcount
              pk-lnpogfiltot.bank = pk-lnpogfil.bank
              pk-lnpogfiltot.crc = pk-lnpogfil.crc. 
              v-refcount = 0.
              v-refsum = 0.    
    end.          
end.  
