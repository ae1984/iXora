 ...
 * MODULE
        Депозиты
 * DESCRIPTION
        Закрытие счетов по депозитам "метро-Classic"
 * RUN
        вызов из процедуры
 * CALLER
        tdacls.p
...
End Function.
   ...
   if not avail aaa then do:
      run savelog( "tdaclose2", aaa.aaa + " Не найден счет").
      message "Не найден счет" aaa.aaa. pause. return.
   end.

   if aaa.opnamt = 0 then do:
      run savelog( "tdaclose2", aaa.aaa + " счет открыт НЕКОРРЕКТНО дата открытия не совпадает с взносом основной суммы").
   ...
   if not avail acvolt then do:
      run savelog( "tdaclose2", aaa.aaa + " счет открыт НЕКОРРЕКТНО").
   ...
   /* Прошло меньше месяца */
      ...
      if avail bf-acvolt then do:
         run Get_Rate_Real(t-aaa, output v-rt1).
      end.
      else
      v-rt1 = 0.
      ...
      do t-ind = t_date to t_date2:
         ...
         if avail tmp-conv then do:
            t-aaa = tmp-conv.aaa.
            run Get_Rate_Real(tmp-conv.aaa, output v-rt1).
...
if (aaa.cr[1] - aaa.dr[1]) >= ((acvolt.bonusopnamt + v-sumchkamt + d_sumrt) - ((v-sumchkamt + aaa.accrued) - ((aaa.cr[1] - aaa.dr[1]) + (aaa.cr[2] - aaa.dr[2])))) then do:

   d_1% = (aaa.cr[1] - aaa.dr[1]) - ((acvolt.bonusopnamt + v-sumchkamt + d_sumrt) - ((v-sumchkamt + aaa.accrued) - ((aaa.cr[1] - aaa.dr[1]) + (aaa.cr[2] - aaa.dr[2])))).
   d_3% = 0.
...
do:
   d_1% = 0.
   d_3% = ((acvolt.bonusopnamt + v-sumchkamt + d_sumrt) - ((v-sumchkamt + aaa.accrued) - ((aaa.cr[1] - aaa.dr[1]) + (aaa.cr[2] - aaa.dr[2])))) - (aaa.cr[1] - aaa.dr[1]).
end.
 ...
 */
   ...
   run savelog( "tdaclose2", aaa.aaa + "Сумма в размере" +
          trim(string((aaa.cr[1] - aaa.dr[1]) - d_1% + d_3%,'z,zzz,zzz,zz9.99-')) +
   ...
   if v-ans then run savelog( "tdaclose2", aaa.aaa + " v-ans = true").
   else run savelog( "tdaclose2", aaa.aaa + " v-ans = false").
   ...
   if g-today <> aaa.regdt then do:
         ...
         if d_3% > 0 and d_3% <= (aaa.cr[2] - aaa.dr[2]) then do:
            run savelog( "tdaclose2", aaa.aaa + " Проводка с 2 на 1 уровень").
         ...
         if d_1% > 0 and d_1% <= (aaa.cr[1] - aaa.dr[1]) then do:
            run savelog( "tdaclose2", aaa.aaa + " Проводка с 1 на 2 уровень ").
            ...
            if rcode ne 0 then do:
               run savelog( "tdaclose2", aaa.aaa + " UNI0074 " + rdes).
         ...
         if s-amt11 > 0 then do:
            run savelog( "tdaclose2", aaa.aaa + " Проводка со 2 на 11 уровень ").
            v-jh = 0.
            ...
            if rcode ne 0 then do:
               run savelog( "tdaclose2", aaa.aaa + " cda0003 " + rdes).
         ...
         if s-amt1 > 0 then do:
            run savelog( "tdaclose2", aaa.aaa + "  Урегулируем разность если на 2 ур > чем на 11 ").
            v-jh = 0.
            ...
            if rcode ne 0 then do:
               run savelog( "tdaclose2", aaa.aaa + " uni0048 " + rdes).
               message "uni0048" ' ' rdes. pause. undo,retry.
            end.
         end.
         aaa.sta = "E".
         run savelog( "tdaclose2", aaa.aaa + " статус " + aaa.sta).
   end.
   else do:      /* счет открыли сегодня */
      aaa.sta = "E".
      message "Депозит успешно закрыт".
      run savelog( "tdaclose2", aaa.aaa + "Депозит успешно закрыт. статус " + aaa.sta).
      pause.
   end.
...
Procedure Get_Rate_Real. /*Возврещает ставку счета*/
   ...
   if avail bf-acc then do:
     ...
     if bf-acc.crc = 3  then do: find sysc "rateeu" no-lock no-error.  if available sysc then r_rate = sysc.deval. end.
   end.
   else
   r_rate = 0.
end.
