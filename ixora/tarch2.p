...
define frame fr
   ...
   v-vubor3 no-label format 'x(70)' help 'Введиты коды через запятую. * - по всем клиентам.'
   with centered side-label row 5 title "УКАЖИТЕ ПЕРИОД ОТЧЕТА". 

dt1 = g-today. dt2 = g-today.

update dt1 dt2 v-vubor1 v-vubor2 v-vubor3 with frame fr.
hide frame fr.
...
do v-dt = dt1 to dt2:
   for each jl where jl.jdt = v-dt and jl.dc = 'c' no-lock:
       ...
       find first tarif2 where tarif2.kont = jl.gl and tarif2.stat = 'r' and tarif2.pakalp matches "*" + v-opis + "*" no-lock no-error.
       if not avail tarif2 then 
       find first tarif2 where tarif2.kont = jl.gl and tarif2.stat = 'r' and v-opis matches "*" + tarif2.pakalp + "*" no-lock no-error.
       if not avail tarif2 then next.
...
output to value("Cl_Turnover" + string(year(g-today), "9999") + string(month(g-today), "99") + string(day(g-today), "99") + ".xls").

put unformatted
    "ОТЧЕТ ПО ОБЩЕЙ СУММЕ ОБОРОТОВ ПО КЛИЕНТАМ, КОТОРЫМ УСТАНОВЛЕНЫ ИНДИВИДУАЛЬНЫЕ ЛЬГОТНЫЕ ТАРИФЫ." skip.
...
unix silent cptwin value("Cl_Turnover" + string(year(g-today), "9999") + string(month(g-today), "99") + string(day(g-today), "99") + ".xls") excel.
