...
/* run vcrepdpldat ("e", s-vcourbank, 0, v-dtb, v-dte, v-closed). */

run vcrepdexdat (s-vcourbank, 0, v-dtb, v-dte, v-closed, '1,5').
...
put stream vcrpt unformatted 
  "<TABLE width=""90%"" border=""0"" cellspacing=""0"" cellpadding=""3"" align=""center"" >" skip
    ...
    "&nbsp;&nbsp;&nbsp;Просим принять к сведению, что по Вашему экспортному контракту N " + trim(t-dolgs.ctnum) skip
    " от "  string(t-dolgs.ctdate, "99.99.9999") skip 
    " c "  v-partnername  " (паспорт сделки "  trim(v-psnum)  ")" skip
    " к настоящему времени не поступила экспортная выручка в сумме  "  skip
    sum2strd(t-dolgs.sumcon, 2)  " " v-ncrccod skip
    "&nbsp;&nbsp;&nbsp;Если имел место возврат инопартнером товара, то просим вас в течение " skip
    "5 дней предоставить копии таможенных деклараций в Департамент валютного контроля или в обслуживающий Вас СПФ.  <BR> " skip.

    if t-dolgs.sumusd > 10000 then do:
        put stream vcrpt unformatted 
        "&nbsp;&nbsp;&nbsp;   В противном случае, выше названная сумма," skip 
        " по действующему законодательству, подлежит лицензированию в Национальном Банке Республики Казахстан. <BR>"  skip
        "&nbsp;&nbsp;&nbsp;Убедительно просим Вас обратиться в Национальный Банк РК за получением лицензии.<BR>" skip.
        put stream vcrpt unformatted 
        "&nbsp;&nbsp;&nbsp;За дополнительными разъяснениями по вопросу лицензирования можете обращаться в" skip 
        "Департамент Валютного Контроля " CAPS(bank.cmp.name)  " или в обслуживающий Вас СПФ.<BR>" skip.  

    end. else do:
        put stream vcrpt unformatted 
        "&nbsp;&nbsp;&nbsp;В противном случае, предоставьте документы, подтверждающие принятие Вами " skip
        "мер по обеспечению зачисления в полном объеме экспортной выручки, а в случае невозможности получения " skip
        " экспортной выручки, возврата товара, ранее отправленного инопартнеру по данному контракту. <BR> " skip.  
        put stream vcrpt unformatted 
        "&nbsp;&nbsp;&nbsp;За дополнительными разъяснениями можете обращаться в" skip 
        "Департамент Валютного Контроля " CAPS(bank.cmp.name) "  или в обслуживающий Вас СПФ.<BR>" skip.  
