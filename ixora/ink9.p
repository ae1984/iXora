 ...
 * DESCRIPTION
        Полное приостановление операций за исключением пенсионных платежей и платежей в бюджет
 * RUN
 ...
 * CHANGES
        ...
        16/10/2009 galina - поправила сохранение комментария для отзыва в истории aas_hist.docprim1
        ...
        20.06.2012 evseev - отструктурировал код. вынес save_data в aas2his.i и переименовал в aas2his.  добавил mn
        06.09.2012 evseev - иин/бин
        15.07.2013 evseev - tz-1963
...
/* блок объявления переменных ->*/
    ...
    def browse b1 query q1 displ
        ...
        with 20 down title "Приостановление операций за искл. пенс. и плат в бюджет(действующие)" overlay.
    ...
    def frame getlistbin1
        aas.aaa format "x(20)" label     "Номер счета      " skip
        aas.regdt label                   "Дата регистрации " skip
        aas.docnum label                "Номер документа  " help "Номер документа на основании которого накладывается ограничение" skip
        aas.docdat label                "Дата документа   " help "Дата документа на основании которого накладывается ограничение" skip
        aas.bnf format "x(40)" label                   "Орган огранич(F2)" help "Наименование органа выставившего ограничение" skip
        aas.nkbin  format "x(16)"     label           "БИН              " validate((chk12_innbin(aas.nkbin)) ,'Неправильно введён БИН/ИИН') skip
        aas.payee label "Примечание       " skip
        with side-labels centered row 9.
    def frame getlist2
    ...
    def frame getlist3
        ...
        with side-labels centered row 8.
    def frame getlistbin3
        aas.who    label                "Логин менеджера блокирующего счет" skip
        aas.docnum label                "Номер документа блокирующего счет" skip
        aas.docdat label                "Дата документа блокирующего счет " skip
        aas.bnf format "x(35)" label "Организация, выставившая огранич." skip
        aas.nkbin format "x(35)" label "БИН                              " validate((chk12_innbin(aas.nkbin)) ,'Неправильно введён БИН/ИИН')
        with side-labels centered row 8.
    def frame getlock
    ...
    def frame getlist5
        ...
        with side-labels centered row 8.
    def frame getlistbin5
        aas.who  label "Установил     " skip
        aas.regdt  label "Дата создания." skip
        aas.nkbin format "x(25)" label "Орган огранич. " help "Наименование органа выставившего ограничение" skip
        aas.payee label "Примечание" help "Примечание"
        with side-labels centered row 8.
    def frame aacc
    ...
    def frame fr1 b1 skip bRedakt bprint bdetail brem bhistory bexit  with centered overlay row 3 width 90 top-only.
    def frame fr4 b4 skip bexit bdethis with centered overlay row 3 width 100 top-only.
    def frame fr5 b5 skip bdet bexit /*bdetail*/ with centered overlay row 3 top-only.
    def frame t_frame1
...
/* функции и процедуры ->*/
    PROCEDURE specindo.
        ...
        do:
            ...
            if avail pl-ofc then
            do:
                if pl-ofc.expr[1] <> '' then
                do:
                    do i = 1 to num-entries(pl-ofc.expr[1]):
                        cha = entry(i,pl-ofc.expr[1]).
                        if lookup(cha,sysc.chval) > 0 then
                        do:
                            boole = true.
                            leave.
                        end.
            ...
            if lookup(g-ofc,sysc.chval) > 0 or boole then
            do:
...
on help of aas.bnf in frame getlistbin1 do:
   empty temp-table t-lnbin.
   for each taxnk  where taxnk.bin <> "" no-lock use-index bin1:
       create t-lnbin.
       assign
           t-lnbin.bin  = taxnk.bin
           t-lnbin.name = taxnk.name.
   end.
   find first t-lnbin no-error.
   if not avail t-lnbin then do:
       message skip " Справочника нет !" skip(1) view-as alert-box button ok title "".
       return.
   end.
   {itemlist.i
      ...
      &frame = "row 6 centered scroll 1 20 down overlay title 'Выберите налоговый комитет' "
      &where = " true "
      &flddisp = " t-lnbin.bin label 'БИН' format 'x(12)'
                   t-lnbin.name label 'Наименование' format 'x(50)'"
      &chkey = "bin"
      &chtype = "string"
      &index  = "main"
   }
   v-cod = frame-value.
   find first taxnk where taxnk.bin = v-cod use-index bin1 no-lock no-error.
   if avail taxnk then
       assign
           aas.nkbin = taxnk.bin
           aas.bnf   = taxnk.name.
   display aas.nkbin aas.bnf with frame getlistbin1.
end.
...
on choose of bt_AddNew in frame a2 do:
   ...
   if avail aaa then do:
       ...
       aas.aaa = aaa.aaa.
       aas.payee = "За искл бюдж,пенсион,социальн.".
       if v-bin then displ aas.aaa with frame getlistbin1. else displ aas.aaa with frame getlist1.
       aas.whn = g-today.
       if v-bin then update aas.regdt aas.docnum aas.docdat aas.bnf /*aas.nkbin*/ aas.payee with frame getlistbin1.
       else update aas.regdt aas.docnum aas.docdat aas.bnf aas.dpname aas.payee with frame getlist1.
       if aas.nkbin = '' or length(aas.nkbin) <> 12 then do:
             message skip "БИН отсутствует!" view-as alert-box button Ok title "Ошибка!".
             undo,retry.
       end.
       aas.sta = 11.
       aas.mn = "50000".
       ...
       run savelog("aaahbal", "ink9 ; " + aaa.aaa + " ; " + string(aaa.hbal) + " ; " + string(aaa.hbal + aas.chkamt) + " ; " + string(aas.chkamt)).
       aaa.hbal = aaa.hbal + aas.chkamt.

       if v-bin then hide frame getlistbin1. else hide frame getlist1.
       ...
       if lookup (aaa.lgr , "138,139,140,143,144,145") > 0 then do:
           run mail("DPC@fortebank.com", "METROCOMBANK <abpk@metrocombank.kz>", "РПРО",
               "РПРО №" + aas.docnum + " от " + string(aas.docdat,"99/99/9999") + ". \n Зарегистрировал " + aas.who + " по счету " + aas.aaa + ", ИИН " + cif.bin + ", дата " + string(g-today,"99/99/9999")
               , "1", "", "").
...
on choose of bt_Find in frame a2 do:
    ...
    ON CHOOSE OF bhistory IN frame fr1 do:
       ...
       ON CHOOSE OF bdethis IN frame fr4 do: /*История операций*/
               ...
               ON CHOOSE OF bdet IN frame fr5 do: /*Детали*/
                  ...
                  if avail bufl then do:
                      if bufl.chgoper = "A" or  bufl.chgoper = "E" then do:
                          displ t-journal.docnum t-journal.docdat t-journal.bnf t-journal.payee /*t-journal.kbk*/  with frame getlock.
                          hide frame getlock.
                      end.
               ...
               for each aas_hist where aas_hist.aaa = t-aashist.aaa  and aas_hist.ln = t-aashist.ln exclusive-lock use-index aasprep:
                   ...
                   t-journal.nkbin    = aas_hist.nkbin  .
                   ...
                   if v-bin then find first taxnk where taxnk.bin = aas_hist.nkbin no-lock no-error.
                   else find first taxnk where taxnk.rnn = aas_hist.dpname no-lock no-error.
       ...
       p-aaa = ''.
       p-ln = 0.
       for each aas_hist where aas_hist.aaa = aas.aaa and aas_hist.sta = 11 NO-LOCK USE-INDEX aasprep:
    ...
    ON CHOOSE OF bRedakt IN frame fr1 do:
       ...
       if avail buf and avail aaa then do:
           if v-bin then do:
               display aas.who aas.regdt aas.nkbin aas.payee with frame getlistbin5.
               update aas.nkbin  aas.payee with frame getlistbin5.
           end. else do:
           ...
           s-aaa = aas.aaa.
           op_kod = 'E'.
           aas.mn = substr(aas.mn,1,3) + "58".
           RUN aas2his.
           if v-bin then do: if (aas.nkbin entered) or (aas.payee entered) then op_kod= 'E'. end.
           else do: if (aas.dpname entered) or (aas.payee entered) then op_kod= 'E'. end.
           if v-bin then hide frame getlistbin5.
           else hide frame getlist5.
       end.
    end.
    ON CHOOSE OF bprint IN frame fr1 do:
            ...
            put unformatted "--------------------------------------------------------------------------------------------------------" skip.
            put unformatted "          ПРИОСТАНОВЛЕНИЕ ОПЕРАЦИЙ ЗА ИСКЛ ПЛАТЕЖЕЙ В БЮДЖЕТ                                            " skip.
            put unformatted "--------------------------------------------------------------------------------------------------------" skip.
            put unformatted "  CIF    НАИМЕНОВАНИЕ        ДАТА РЕГ  СЧЕТ      N ДОК    ДАТА ДОК.  ОРГАН_ОГР         ПРИМЕЧАНИЕ       " skip.
            put unformatted "--------------------------------------------------------------------------------------------------------" skip.
            if v-dep = 1 then
                for each aas where aas.sta = 11 and aas.aaa = s_FindAcc and aas.ln <> 7777777 use-index aaaln:
                    find last b-cif where b-cif.cif = aas.cif no-lock no-error.
                    if v-bin then put unformatted "  " aas.cif " " b-cif.sname format 'x(20)' string(aas.regdt,"99/99/99") "  " aas.aaa format 'x(20)' " " aas.docnum format 'x(7)' "  " string(aas.docdat,"99/99/99") "   " aas.nkbin format 'x(15)'  "   " aas.payee  skip.
                    else put unformatted "  " aas.cif " " b-cif.sname format 'x(20)' string(aas.regdt,"99/99/99") "  " aas.aaa format 'x(20)' " " aas.docnum format 'x(7)' "  " string(aas.docdat,"99/99/99") "   " aas.dpname format 'x(15)'  "   " aas.payee  skip.
                end.
            if v-dep = 2 then
                for each aas where aas.sta = 11 and aas.cif = s_FindCIF and aas.ln <> 7777777 use-index aaaln:
                    find last b-cif where b-cif.cif = aas.cif no-lock no-error.
                    if v-bin then put unformatted "  " aas.cif " " b-cif.sname format 'x(20)' string(aas.regdt,"99/99/99") "  " aas.aaa format 'x(20)' " " aas.docnum format 'x(7)' "  " string(aas.docdat,"99/99/99") "   " aas.nkbin format 'x(15)'  "   " aas.payee  skip.
                    else put unformatted "  " aas.cif " " b-cif.sname format 'x(20)' string(aas.regdt,"99/99/99") "  " aas.aaa format 'x(20)' " " aas.docnum format 'x(7)' "  " string(aas.docdat,"99/99/99") "   " aas.dpname format 'x(15)'  "   " aas.payee  skip.
                end.
            if v-dep = 3 then
                for each aas where aas.sta = 11 and aas.who = s_FindLogin and aas.ln <> 7777777 use-index aaaln:
                    find last b-cif where b-cif.cif = aas.cif no-lock no-error.
                    if v-bin then put unformatted "  " aas.cif " " b-cif.sname format 'x(20)' string(aas.regdt,"99/99/99") "  " aas.aaa format 'x(20)' " " aas.docnum format 'x(7)' "  " string(aas.docdat,"99/99/99") "   " aas.nkbin format 'x(15)'  "   " aas.payee  skip.
                    else put unformatted "  " aas.cif " " b-cif.sname format 'x(20)' string(aas.regdt,"99/99/99") "  " aas.aaa format 'x(20)' " " aas.docnum format 'x(7)' "  " string(aas.docdat,"99/99/99") "   " aas.dpname format 'x(15)'  "   " aas.payee  skip.
                end.
            if v-dep = 4 then
                for each aas where aas.sta = 11 and aas.regdt >= dt_FindDateBegin and aas.regdt <= dt_FindDateEnd and aas.ln <> 7777777 use-index aaaln:
                    find last b-cif where b-cif.cif = aas.cif no-lock no-error.
                    if v-bin then put unformatted "  " aas.cif " " b-cif.sname format 'x(20)' string(aas.regdt,"99/99/99") "  " aas.aaa format 'x(20)' " " aas.docnum format 'x(7)' "  " string(aas.docdat,"99/99/99") "   " aas.nkbin format 'x(15)'  "   " aas.payee  skip.
                    else put unformatted "  " aas.cif " " b-cif.sname format 'x(20)' string(aas.regdt,"99/99/99") "  " aas.aaa format 'x(20)' " " aas.docnum format 'x(7)' "  " string(aas.docdat,"99/99/99") "   " aas.dpname format 'x(15)'  "   " aas.payee  skip.
                end.
    ...
    ON CHOOSE OF bdetail IN frame fr1 do:
       ...
       if avail buf then do:
           if v-bin then find first taxnk where taxnk.bin = aas.nkbin no-lock no-error.
           else find first taxnk where taxnk.rnn = aas.dpname no-lock no-error.
           if  avail taxnk then aas.bnf = taxnk.name.
           if v-bin then do:
               display aas.who aas.docnum aas.docdat aas.bnf  aas.nkbin /*aas.kbk*/ with frame getlistbin3.
               hide frame getlistbin3.
           end. else do:
               display aas.who aas.docnum aas.docdat aas.bnf  aas.dpname /*aas.kbk*/ with frame getlist3.
               hide frame getlist3.
           end.
       end.
    end.
       ...
       if avail buf then do:
           if yes-no ("Внимание!", "Вы действительно хотите снять ограничение?") then  do:
               ...
               if not aas.contr then do:
                   ...
                   aas.tim = time.
                   s-aaa = aaa.aaa.
                   aas.mn = substr(aas.mn,1,3) + "59".
                   RUN aas2his.
                   if lookup (aaa.lgr , "138,139,140,143,144,145") > 0 then do:
                       find first aaa where aaa.aaa = aas.aaa no-lock no-error.
                       find first cif where cif.cif = aaa.cif no-lock no-error.
                       run mail("DPC@fortebank.com", "METROCOMBANK <abpk@metrocombank.kz>", "Отзыв/удаление РПРО",
                           "Отзыв/удаление РПРО №" + aas.docnum + " от " + string(aas.docdat,"99/99/9999") + ". \n Зарегистрировал " + aas.who + " по счету " + aas.aaa + ", ИИН " + cif.bin + ", дата " + string(aas.whn,"99/99/9999")
                           , "1", "", "").
                       message "Вам необходимо выслать сканированную копию данного распоряжения на группу ДПК" view-as alert-box question buttons ok title "Сообщение".
                   end.
                   delete buf.
    ...
    repeat:
        ...
        case v-dep:
            when 1 then do: /*По номеру счета*/
                 ...
                 if i_indx <> 0 then do:
                     open query q1 for each aas where aas.sta = 11 and aas.aaa = s_FindAcc and aas.ln <> 7777777 use-index aaaln.
                     ...
                     if frame-index = 1 then do:
                         ...
                         p-aaa = ''.
                         p-ln = 0.
                         for each aas_hist where aas_hist.aaa = aaa.aaa and aas_hist.sta = 11 NO-LOCK USE-INDEX aasprep:
                             if aas_hist.ln <> p-ln then do transaction:
                                 p-aaa = aas_hist.aaa.
                                 p-ln = aas_hist.ln.
                                 create t-aashist.
            ...
            when 2 then do: /*По CIF коду    */
                 ...
                 open query q1 for each aas where aas.sta = 11 and  aas.cif = s_FindCIF and aas.ln <> 7777777 use-index aaaln.
                 b1:SET-REPOSITIONED-ROW (1, "CONDITIONAL").
                 ENABLE all with frame fr1 centered overlay top-only.
                 apply "value-changed" to b1 in frame fr1.
                 WAIT-FOR WINDOW-CLOSE of frame fr1.
            ...
            when 3 then do: /* По менеджеру */
                 ...
                 if v-dp3 = 1 then do:
                    open query q1 for each aas where aas.sta = 11 and aas.who = s_FindLogin and aas.ln <> 7777777 use-index aaaln.
                 end.
                 if v-dp3 = 2 then do:
                     update dt_1 label "Дата начала   "  validate(dt_1 <> ?, "Введите дату для поиска") skip
                         ...
                         open query q1 for each aas where aas.sta = 11 and aas.who = s_FindLogin and aas.regdt >= dt_1 and aas.regdt <= dt_2 and aas.ln <> 7777777 use-index aaaln.
                 end.
                 b1:SET-REPOSITIONED-ROW (1, "CONDITIONAL").
                 ENABLE all with frame fr1 centered overlay top-only.
                 apply "value-changed" to b1 in frame fr1.
            ...
            when 4 then do: /*За период      */
                  ...
                  open query q1 for each aas where aas.sta = 11 and aas.regdt >= dt_FindDateBegin and aas.regdt <= dt_FindDateEnd and aas.ln <> 7777777 use-index aaaln.
                  b1:SET-REPOSITIONED-ROW (1, "CONDITIONAL").
                  ENABLE all with frame fr1 centered overlay top-only.
                  apply "value-changed" to b1 in frame fr1.
                  WAIT-FOR WINDOW-CLOSE of frame fr1.
