...
/*  the name of the program is ANSWER.P
    this program need for form & send to SWIFT "Answers"
    messages
...
{global.i}
  ...
  def new shared var      itype    as char format "x(5)" initial ["MT196"].
  ...
  def new shared var xyz as int.  /* work variable for exit to the n96 scr*/
...
/*************************************************/
              ...
              if xyz = 3 then do:    /*  go to next field M:76  */ 
                 xyz = 0.
	       ...
	       if keyfunction(lastkey) = "GO" then do:
			...
			if a then do:
                           if (query_6[1]      eq "") then do:
                              message "Field M:76 must be fill out ".
                              bell.
                              undo,retry.
                            end.
			  leave queries00.
