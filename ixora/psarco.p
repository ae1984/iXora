 ...
 * MODULE
        Платежная система
 * DESCRIPTION
        Поиск исходящих платежей по различным параметрам
 * RUN
        главное меню
 * CALLER
 ...
 * CHANGES
       ...
       13.03.2006 u00600 добавила поиск по ФИО
*/            
...
def var vdtall as cha initial "ALL" label "2Дата" no-undo. 
def var v-source like remtrz.source label "Источник" no-undo.
def var v-pid like que.pid label "Код" no-undo.  
def var v-acc like remtrz.cracc label "Д.Сч." no-undo.
def var v-crc like remtrz.fcrc label "Валюта   " initial 1 no-undo. 
def var v-typeps as char format "x(3)" label "Тип" no-undo.
def var v-bankl like remtrz.rbank  no-undo.
def var v-sqn like remtrz.sqn label "Ссыл.N" no-undo.
def var v-amtf like remtrz.payment label "Сумма  С " no-undo.
def var v-amtt like remtrz.payment label "По " no-undo.
def var v-rdtf like remtrz.rdt label "Дата регистрации     С " no-undo.
def var v-rdtt like remtrz.rdt label " По " no-undo.
def var v-valdtf like remtrz.valdt1 label "     С " no-undo.
def var v-valdtt like remtrz.valdt1 label " По " no-undo.
def new shared var suma like remtrz.amt.
def new shared var sump like remtrz.payment.
def var v-npp as integer label "N платеж.поруч." format ">>>>>>>>9" no-undo.

/* u00600 13.03.2006*/
def var v-fio like remtrz.ord label "Ф.И.О." no-undo.
...
def var v-sname like cif.sname no-undo.
def var pr as log no-undo.
...
/*TITLE mmsg overlay no-labels row 5.  */
...
if s-remtrz = "" then do on error undo , retry :
 ...
 update v-source help 
     "O-исхюЦО I-вх.ручн SW-SWIFT Pxx-пункт H-homebank A-филиал LON-кредит"
   with frame arc.
 ...
 update v-sqn v-npp v-acc v-fio v-rdtf with frame arc.
  v-rdtt = v-rdtf.
  v-acc = trim(v-acc).
 ...
 for each remtrz where (remtrz.ptype = v-typeps or v-typeps = "ALL") and
      ...
      (remtrz.rbank = v-bankl or v-bankl = "ALL" ) and
      (remtrz.dracc = v-acc or v-acc = "ALL") and
      (index(remtrz.ord, v-fio) ne 0 or v-fio = "ALL") and         /* u00600 13.03.2006 */
      ...
      (vdtall = "ALL" or 
      (remtrz.valdt2 >= v-valdtf and remtrz.valdt2 <= v-valdtt )) and
      (remtrz.tcrc = v-crc or v-crc = 0) and remtrz.payment >= v-amtf and
       remtrz.payment <= v-amtt  use-index rdt no-lock :
