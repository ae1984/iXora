 ...
 * INHERIT
        Список вызываемых процедур - vccomexp.p
 * MENU
        Пункт меню
 * AUTHOR
        --/--/2012 damir
...
def var v-filename as char init "vccomexp.htm".
def var v-title as char.

v-title = "КОММЕРЧЕСКИМ КРЕДИТАМ<br>(ЭКСПОРТ)".

def stream vcrpt.
output stream vcrpt to value(v-filename).

{html-title.i &stream = "stream vcrpt" &title = "Задолжники по коммерческим кредитам"}
