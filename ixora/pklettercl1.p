...
if p-letternom = "" then do:
  run pknewletter (s-ourbank, s-paramnom, replace(s-codemask, "*", "cl1"), p_newved, s-lon, lon.rdt, output p-letternom).
...
if p-newfile then do:
  output stream rep to value(s-filename + "cl1.html").
  output stream rep close.
end.

output stream rep to value(s-filename + "cl1.html") append.
...
put stream rep unformatted
  "<P align=""justify"">"
       ...
       "а также сумма неустойки (штрафных санкций, пени) " v-dolgpena " (" v-dolgpenawrd ")&nbsp;тенге.<BR>" skip
