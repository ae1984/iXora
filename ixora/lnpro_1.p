 ...
 * DESCRIPTION
        Пролонгация экспресс-кредита (вступает в силу с 01.04.2010)
 * RUN
 ...
 * BASES
        BANK COMM
 * CHANGES
        05/04/2010 madiyar - не учтен случай предоставления отсрочки до следующего платежа по графику, исправил
        05/04/2010 madiyar - заполняется lnprohis.sts
        06/04/2010 madiyar - создаем запись в ln%his при увеличении срока кредита; новый lon.duedt пишем в lnprohis.dtend
        12/04/2010 madiyar - новый шаблон для переноса балансовой пени на отсрочку
        13/04/2010 madiyar - режим viewOnly для ДМиВК
        16/04/2010 madiyar - исправил некорректное изменение графика при выборе в качестве даты отсрочки последнего платежа по графику
        23/04/2010 madiyar - филиалы не имеют права реструктурировать кредиты с ОД более 500000 тенге
        13/05/2010 madiyar - добавил проверку на ненулевой остаток ОД по кредиту
        30/09/2010 madiyar - перенос пени на отсрочку возможен даже если срок кредита истек
        05/10/2010 madiyar - еще раз подправил по поводу окончания срока кредита
        06/10/2010 madiyar - добавил проверку на изменение срока кредита
        01/08/2011 dmitriy - перенес формирование доп соглашения в lnpro_2
                           - запретил обновлять график погашения (тоже перенес в lnpro_2)
        19/08/2011 dmitriy - переименовал в lnpro_1, чтобы изменения не касались остальных п.м. вызывающих lnpro
...
if lon.duedt <= g-today then do:
    ...
    /* ----- dmitriy -----*/
    if v-viewOnly = no then do:
    find first pktrans where pktrans.pkank = s-pkankln no-lock no-error.
        if avail pktrans then do:
            if pktrans.trans_dt <> today then do:
                 message "Дата изменения графика отличается от текущей!" view-as alert-box warning.
                 leave.
            end.
            v-dtend = pktrans.londt.
            v-dtpog = pktrans.oddt.
            v-dtpog2 = pktrans.percdt.
            v-perrate2 = pktrans.bonus.
            v-comrate2 = pktrans.commis.
        end.
        else message "График погашения не найден!" view-as alert-box warning.
     end.
    /* -------------------*/
    ...
    if v-dka then do:
        displ skip
              ...
              with row 9 centered side-labels overlay frame frdt_dka.
...
choice = no.
/*
message "Проверьте доп. соглашение.~nПровести реструктуризацию/пролонгацию/отсрочку пени?" view-as alert-box question buttons yes-no title "" update choice.
*/

message "Провести реструктуризацию/пролонгацию/отсрочку пени?" view-as alert-box question buttons yes-no title "" update choice.
...
if choice then do:
    if lon.prem > 0 then do:
        ...
        if v-perrate1 <> v-perrate2 then do transaction:
            ...
            assign ln%his.lon = lon.lon
                   ...
                   ln%his.rem = 'изменение % ставки - рестр/пролонг'
    ...
    do transaction:
        ...
        assign ln%his.lon = lon.lon
               ln%his.stdat = g-today
               ln%his.intrate = v-perrate2
               ln%his.rem = 'восстановление % ставки - рестр/пролонг'
               ln%his.opnamt = lon.opnamt
               ln%his.rdt = lon.rdt
               ln%his.cif = lon.cif
               ln%his.duedt = lon.duedt
    ...
    if loncon.sods1 <= 0 then do transaction:
        ...
        assign ln%his.lon = lon.lon
               ...
               ln%his.rem = 'восстановление ставки по штрафам - рестр/пролонг'
               ln%his.opnamt = lon.opnamt
               ln%his.rdt = lon.rdt
               ln%his.cif = lon.cif
               ln%his.duedt = lon.duedt
    ...
    if v-protype <> 4 then do:
        ...
        if v-bal7 > 0 then do:
            v-rem = "Рестр/пролонг Перенос ОД 7ур.->1ур., " + v-rnn + " " + v-name.
        ...
        if v-bal4 > 0 then do:
            v-rem = "Рестр/пролонг Перенос %% из начисленных вне баланса в начисленные, " + v-rnn + " " + v-name.
    ...
    /* переносим штрафы в отсроченные */
    if v-bal5 + v-bal16 > 0 then do:
        if v-bal5 > 0 then do:
            v-rem = "Рестр/пролонг Перенос внебалансовых штрафов на отсрочку, " + v-rnn + " " + v-name.
            v-param = string(v-bal5) + vdel + lon.lon + vdel + v-rem + vdel + vdel + vdel + vdel.
            s-jh = 0.
            run trxgen ("lon0130", vdel, v-param, "lon", lon.lon, output rcode, output rdes, input-output s-jh).
            if rcode ne 0 then do:
                message rdes.
                pause 1000.
                next.
        ...
        if v-bal16 > 0 then do:
            v-rem = "Рестр/пролонг Перенос штрафов (".
            if v-bal16_new > 0 then do:
                v-rem = v-rem + "KZT " + string(v-bal16_new) + " тек. года".
                if v-bal16_old > 0 then v-rem = v-rem + ", ".
            end.
            if v-bal16_old > 0 then v-rem = v-rem + "KZT " + string(v-bal16_old) + " пр.лет".
            v-rem = v-rem + ") на отсрочку, " + v-rnn + " " + v-name.
            v-param = string(v-bal16_new) + vdel + lon.lon + vdel + string(v-bal16_old) + vdel + string(v-bal16) + vdel + v-rem + vdel + vdel + vdel + vdel.
            s-jh = 0.
            run trxgen ("lon0129", vdel, v-param, "lon", lon.lon, output rcode, output rdes, input-output s-jh).
            if rcode ne 0 then do:
                message rdes.
                pause 1000.
                next.
    ...
    do transaction:
        create lnprohis.
        assign lnprohis.lon = lon.lon
               lnprohis.who = g-ofc
               lnprohis.whn = g-today
               lnprohis.type = "prolong"
               lnprohis.dth = v-dtpog
               lnprohis.dti = v-dtpog2
               lnprohis.dtend = v-dtend
               lnprohis.premNew = v-perrate2
               lnprohis.comNew = v-comrate2
               lnprohis.penAmt = v-bal5 + v-bal16
               lnprohis.sts = string(v-protype).
    end. /* transaction */

    message "Реструктуризация/пролонгация/отсрочка пени проведена успешно" view-as alert-box information.
