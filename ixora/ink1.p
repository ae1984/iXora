 ...
 * DESCRIPTION
        Полное приостановление операций по счету
 * RUN
 ...
 * CHANGES
        ...
        19.06.2012 evseev - добавил mn =  '10000'. вынес save_data в aas2his.i и переименовал в aas2his
...
/* блок объявления переменных ->*/
    ...
    def browse b1 query q1 displ
        ...
        with 12 down title "Полное приостановление операций по счету(действующие)" overlay.
...
on choose of bt_AddNew in frame a2 do:
    ...
    if avail aaa then do:
       ...
       aas.aaa = aaa.aaa.
       aas.payee = "Полное приостановл. операций по счету".
       displ aas.aaa with frame getlist1.
       aas.whn = g-today.
       update aas.regdt aas.docnum aas.docdat aas.dpname aas.payee with frame getlist1.
       aas.sta = 1.
       aas.mn = "10000".
       ...
       run savelog("aaahbal", "ink1 ; " + aaa.aaa + " ; " + string(aaa.hbal) + " ; " + string(aaa.hbal + aas.chkamt) + " ; " + string(aas.chkamt)).
...
on choose of bt_Find in frame a2 do:
   ...
   ON CHOOSE OF bhistory IN FRAME fr1 do:
      ...
      p-aaa = ''.
      p-ln = 0.
      for each aas_hist where aas_hist.aaa = aas.aaa and aas_hist.sta = 1 NO-LOCK USE-INDEX aasprep:
   ...
   ON CHOOSE OF bprint IN FRAME fr1 do:
      ...
      put unformatted "---------------------------------------------------------------------------------------------------------------------" skip.
      put unformatted "                    ПОЛНОЕ ПРИОСТАНОВЛЕНИЕ ОПЕРАЦИЙ ПО СЧЕТАМ                                           " skip.
      put unformatted "---------------------------------------------------------------------------------------------------------------------" skip.
      put unformatted "  CIF    НАИМЕНОВАНИЕ        ДАТА РЕГ         СЧЕТ          N ДОК    ДАТА ДОК.  ОРГАН_ОГР         ПРИМЕЧАНИЕ       " skip.
      put unformatted "---------------------------------------------------------------------------------------------------------------------" skip.
      if v-dep = 1 then
         for each aas where aas.sta = 1 and aas.aaa = s_FindAcc and aas.ln <> 7777777 use-index aaaln:
             find last b-cif where b-cif.cif = aas.cif no-lock no-error.
             put unformatted "  " aas.cif " " b-cif.sname format 'x(20)' string(aas.regdt,"99/99/99") "  " aas.aaa  format 'x(21)' " " aas.docnum format 'x(7)' "  " string(aas.docdat,"99/99/99") "   " aas.dpname format 'x(15)'  "   " aas.payee  skip.
         end.
      if v-dep = 2 then
         for each aas where aas.sta = 1 and aas.cif = s_FindCIF and aas.ln <> 7777777 use-index aaaln:
             find last b-cif where b-cif.cif = aas.cif no-lock no-error.
             put unformatted "  " aas.cif " " b-cif.sname format 'x(20)' string(aas.regdt,"99/99/99") "  " aas.aaa " " aas.docnum format 'x(7)' "  " string(aas.docdat,"99/99/99") "   " aas.dpname format 'x(15)'  "   " aas.payee  skip.
         end.
      if v-dep = 3 then
         for each aas where aas.sta = 1 and aas.who = s_FindLogin and aas.ln <> 7777777 use-index aaaln:
             find last b-cif where b-cif.cif = aas.cif no-lock no-error.
             put unformatted "  " aas.cif " " b-cif.sname format 'x(20)' string(aas.regdt,"99/99/99") "  " aas.aaa " " aas.docnum format 'x(7)' "  " string(aas.docdat,"99/99/99") "   " aas.dpname format 'x(15)'  "   " aas.payee  skip.
         end.
      if v-dep = 4 then
         for each aas where aas.sta = 1 and aas.regdt >= dt_FindDateBegin and aas.regdt <= dt_FindDateEnd and aas.ln <> 7777777 use-index aaaln:
             find last b-cif where b-cif.cif = aas.cif no-lock no-error.
             put unformatted "  " aas.cif " " b-cif.sname format 'x(20)' string(aas.regdt,"99/99/99") "  " aas.aaa " " aas.docnum format 'x(7)' "  " string(aas.docdat,"99/99/99") "   " aas.dpname format 'x(15)'  "   " aas.payee  skip.
         end.
      output close.
   ...
   ON CHOOSE OF bdetail IN FRAME fr1 do:
      ...
      if avail buf then do:
         display aas.who aas.docnum aas.docdat aas.dpname with frame getlist3.
         hide frame getlist3.
      end.
   end.
   /* Снять ограничение */
   ...
   repeat:
       ...
       case v-dep:
          when 1 then do: /*По номеру счета*/
              ...
              if i_indx <> 0 then do:
                 open query q1 for each aas where aas.sta = 1 and aas.aaa = s_FindAcc and aas.ln <> 7777777 use-index aaaln.
                 b1:SET-REPOSITIONED-ROW (1, "CONDITIONAL").
                 ENABLE all with frame fr1 centered overlay top-only.
                 apply "value-changed" to b1 in frame fr1.
                 WAIT-FOR WINDOW-CLOSE of frame fr1.
              end. else do:
                 display "По счету " + s_FindAcc + " нет инструкций по полной приостановке операций !" format "x(65)" with center row 7 frame bridin.
                 ...
                 if frame-index = 1 then do:
                    ...
                    ON CHOOSE OF bdethis IN FRAME fr4 /* История операций */ do:
                       message "". pause 0.
                       ...
                       for each aas_hist where aas_hist.aaa = t-aashist.aaa  and aas_hist.ln = t-aashist.ln no-lock use-index aasprep:
                           ...
                           create t-journal.
                               ...
                               if aas_hist.chgoper = 'A' then
                                  t-journal.name = "Введена  [" + ofc.name + "," + string(aas_hist.chgdat) + "," + STRING(aas_hist.chgtime, "hh:mm:ss") + "]" .
                               else
                                  if aas_hist.chgoper = 'E' then
                                     t-journal.name = "Изменена [" + ofc.name + "," + string(aas_hist.chgdat) + "," + STRING(aas_hist.chgtime, "hh:mm:ss") + "]" .
                                  else
                                     if aas_hist.chgoper = 'D' then
                                        t-journal.name = "Удалена  [" + ofc.name + "," + string(aas_hist.chgdat) + "," + STRING(aas_hist.chgtime, "hh:mm:ss") + "]" .
                                     else
                                        t-journal.name =  "                    ***" .
                                     t-journal.docnum   = aas_hist.docnum  .
                                     t-journal.docdat   = aas_hist.docdat  .
                    ...
                    p-aaa = ''.
                    p-ln = 0.
                    for each aas_hist where aas_hist.aaa = aaa.aaa and aas_hist.sta = 1 NO-LOCK USE-INDEX aasprep:
                        if aas_hist.ln <> p-ln then do transaction:
                           p-aaa = aas_hist.aaa.
                           p-ln = aas_hist.ln.
                           create t-aashist.
          ...
          when 2 then do: /* По CIF коду */
              ...
              open query q1 for each aas where aas.sta = 1 and  aas.cif = s_FindCIF and aas.ln <> 7777777 use-index aaaln.
              b1:SET-REPOSITIONED-ROW (1, "CONDITIONAL").
              ENABLE all with frame fr1 centered overlay top-only.
              apply "value-changed" to b1 in frame fr1.
              WAIT-FOR WINDOW-CLOSE of frame fr1.
          ...
          when 3 then do: /* По менеджеру */
              ...
              v-dp3 = 0.
              message s_FindLogin view-as alert-box.
              run sel2 (" Параметры поиска", " ПОКАЗАТЬ ВСЕ | ЗАДАТЬ ПЕРИОД", output v-dp3).
              if v-dp3 = 0  then return.
              if v-dp3 = 1 then do:
                 open query q1 for each aas where aas.sta = 1 and aas.who = s_FindLogin and aas.ln <> 7777777 use-index aaaln.
              end.
              if v-dp3 = 2 then do:
                 ...
                 open query q1 for each aas where aas.sta = 1 and aas.who = s_FindLogin and aas.regdt >= dt_1 and aas.regdt <= dt_2 and aas.ln <> 7777777 use-index aaaln.
              end.
          ...
          when 4 then do: /*За период      */
              ...
              open query q1 for each aas where aas.sta = 1 and aas.regdt >= dt_FindDateBegin and aas.regdt <= dt_FindDateEnd and aas.ln <> 7777777 use-index aaaln.
              b1:SET-REPOSITIONED-ROW (1, "CONDITIONAL").
              ENABLE all with frame fr1 centered overlay top-only.
              apply "value-changed" to b1 in frame fr1.
              WAIT-FOR WINDOW-CLOSE of frame fr1.
