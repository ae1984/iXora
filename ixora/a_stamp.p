 ...
 * MENU
     15
 * AUTHOR
        27/02/2013 Luiza создан по подобию x1-cash2
 * BASES
        BANK COMM
 * CHANGES
...
define input parameter s-jh as int.
...
if okey then do:
	v-amt = 0.
	ttt = vcha1 + vcha2 + trim(trim(cif.prefix) + " " + trim(cif.name)) .
	...
	IF v-yes eq false and (po-jh ne ? and po-jh ne 0) then do:
		hide frame mm.
			...
			do:
				...
				do:
					...
					do:
                        ...
                        if not avail cashpl then do: message " Не введен символ кас.плана !!!! ". pause. end.
					end.
				end.
			end.
			/* 10.05.94 */
...
/**********************************/
/* 10/08/2012 обработка транзакций по пополнению счетов по ПК*/
{pcpay.i}
