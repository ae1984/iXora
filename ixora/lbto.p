 ...
 * AUTHOR
        31/12/99 pragma
 * CHANGES
        06.04.2004 sasco переделал формат файла
        20.10.2004 tsoy    - список файлов теперь не в переменной а во временной таблице
        11.03.2005 sasco вызов tolbcop, tolbarc в один присест
        12.06.2007 id00004 - добавил формирование каталога с датой
        26/02/08   marinav - формировать каталог, если он не найден

*/

/*
 18.04.2003 nadejda   для всех команд "unix rm" добавлен параметр -f
*/ 
 def input parameter iddat as date. /* Дата для передачи в lb100.p */
 ...
 def var v-name as cha view-as selection-list INNER-CHARS 30 INNER-LINES 10 .
 def var v-narc as cha view-as selection-list INNER-CHARS 30 INNER-LINES 10 .

 /* define variable logpath as character initial "/data/log/alm/putplat" . */
 
 /*
 def frame finw v-name with title  "Created Outwards " no-label column 4 row 2.
 def frame farc v-narc    skip " STW - queue : " ainum
 with title  "Archived Outwards " column 40 no-label  row 2.
 */
...
find sysc where sysc.sysc = "lbtoa" no-lock no-error .
if not avail sysc or sysc.chval = "" then do:
 v-text = " ERROR !!! There isn't record LBTOA in sysc file !! ".
 message v-text .
 run lgps.
 return .
end.
...
if v-unidir = v-uniarh then do :
 v-text = " ERROR !!! Records LBTO and LBTOA are equal !! ".
 message v-text .
 run lgps.
 return .
end .
  ...
  do:
   ...
   if keylabel(lastkey) = "v" then  do:
    ...
    if substr(f-name,index(f-name,".") + 1) ne "tar.Z" then 
     ...
     num = "" .
     list-name = "" .
/*     input through value("gzip -cd " + v-uniarh + f-name ) .   */ 
...
/*   display N-BUF format "x(40)" entry(2,n-buf," ") . pause . */
   unix value("uttview " + v-dir + f-name + " " + entry(1,n-buf," ")). 
   v-tar:screen-value = n-buf .
   v-tar:help = " <ENTER> - view <F4> - leave " .
  end.
    
  on any-printable of bout in frame fout  do:
   ...
   if keylabel(lastkey) = "v" then  do:
    ...
    if substr(f-name,index(f-name,".") + 1) ne "gz" then 
     unix value("joe -rdonly  " +
       v-unidir + entry(1,t-qout.fname," ")) .
    else do:
     num = "" .
     list-name = "" .
     input through  
      value("gzip -cd " + v-uniarh + f-name + "| gtar tvf") .
     repeat :
      import num .
  ...
  do:
    ...
    if yn then do:
        ...
        n-buf = string(day(g-today),"99") + string(month(g-today),"99")
         ...
         for each t-qarc no-lock.
            if  index(t-qarc.fname,n-buf) gt 0 then do:
              yn = false .
              message " Clearing " + string(vnum) +
                  " archive has been already done. Continue  ?  "
                    update yn .
                    if yn then leave.
            end.
         end.
  ...
  on choose of uisend in frame fhelp 
      do:
       ...
       /* output stream l-out to value(logpath) append. 
       put stream l-out unformatted today " " string(time,"HH:MM:SS") 
                  "+++++ Пользователь нажал <SEND> ++++ Сформирован n-buf - " n-buf skip .
       ...
       for each t-qarc.
          if  index(t-qarc.fname,n-buf) gt 0 then do:
            yn = false .
            Message " Clearing " + string(vnum) + 
                    " archive has been already done. Overwrite it ?  " 
             update yn .  
             if yn then leave.
          end.
       end.
...
/* sasco добавил один ? в формат */
/*
          output stream l-out to value(logpath) append. 
          put stream l-out unformatted today " " string(time,"HH:MM:SS") "+++++ Запуск tolbcop +++++ " skip .
          output stream l-out close . 

          Message " C o p i n g  ..... " .
          input through  value('tolbcop "'     
            + daynum + '*" '  + n-buf  +  
            ' "' + v-unidir + '" ' + v-ekshst +
            " " + v-ekscop + ' ; echo $?') .
          
          Message " S e n d i n g ..... " .

          output stream l-out to value(logpath) append. 
          put stream l-out unformatted today " " string(time,"HH:MM:SS") "+++++ Запуск tolbarc +++++ " skip .
          output stream l-out close . 

          input through  value('tolbarc "' 
            + daynum + '*" '  + n-buf  +  
            ' "' + v-unidir + '" ' + v-ekshst + 
            " " + v-eksdir + ' ; echo $?') .  
          v-text = "" .
*/
          ...
          /* output stream l-out to value(logpath) append. 
          put stream l-out unformatted today " " string(time,"HH:MM:SS") "+++++ Запуск tolbcop & tolbarc +++++ " skip .
          output stream l-out close .  */
          ...
          /* output stream l-out to value(logpath) append. 
          put stream l-out unformatted today " " string(time,"HH:MM:SS") "+++++ Запуск цикла считывания ошибок +++++ " skip .
          output stream l-out close .  */
          ...
          if exitcod <> "0" then do :
           ...
           do:
            pause 0 . 
            v-text = "Electronic messages to NB was sended by " + 
             g-ofc . 
             run lgps . 
         ...
         /* output stream l-out to value(logpath) append. 
         put stream l-out unformatted today " " string(time,"HH:MM:SS") "+++++ Конец выгрузки +++++ " skip .
         output stream l-out close . */
...
repeat : 
 ...
 input through value("/bin/ls -lt " + v-unidir + "*.eks " +  
    v-unidir + "*.err " + " 2> /dev/null" ) .
 repeat : 
      import num . 
      create t-qout.
 ...
 pause 0 .
  ...
  wait-for close of this-procedure or 
    any-printable of bout in frame fout 
