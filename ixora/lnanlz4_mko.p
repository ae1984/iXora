 ...
 * MENU
        4-2-5-13 
 * AUTHOR
        01/04/2008 madiyar - скопировал из lnanlz4 с изменениями
 * BASES
        BANK COMM TXB
 * CHANGES
*/  
...
def var v-pkankln as integer no-undo.
 ...
 for each txb.lon where txb.lon.grp = 90 or txb.lon.grp = 92 no-lock:
    ...
    if avail txb.loncon then do:
        for each pkanketa where pkanketa.bank = s-ourbank and pkanketa.cif = txb.lon.cif no-lock:
            if entry(1,pkanketa.rescha[1]) = txb.loncon.lcnt then assign v-pkankln = pkanketa.ln v-credtype = pkanketa.credtype.
        end.
    end.
    if v-pkankln = 0 then do:
        /*
        message s-ourbank " - " txb.lon.cif " " txb.lon.lon " Анкета не найдена! " view-as alert-box error.
        */
        next.
    end.
    else find first pkanketa where pkanketa.bank = s-ourbank and pkanketa.credtype = v-credtype and pkanketa.ln = v-pkankln no-lock no-error.
