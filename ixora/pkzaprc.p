...
form
  ...
  v-summax4srok label "  МАКСИМАЛЬНАЯ СУММА " format "zzz,zzz,zzz,zz9.99" " " skip
  ...
  v-sumfin      label "       СУММА КРЕДИТА " format "zzz,zzz,zzz,zz9.99" " " skip
...
if pkanketa.rescha[3] <> '' then do:
  message " К анкете уже привязана карта " pkanketa.rescha[3] view-as alert-box buttons ok.
  return.
end.

v-sroku = pkanketa.srok.
...
do transaction on error undo, return:
   ...
   for each t-lnf no-lock:
     ...
     if t-lnf.nominal >= v-summax4srok then v-skip = yes.
   end.
