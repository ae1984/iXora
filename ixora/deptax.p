...
if s-kb <> 0 and month(g-today) = 12 and day(g-today) = 31 then
do:
    s_l_inkopl = false.
    return.
end.
...
do transaction:
    ...
    assign
        ...
        remtrz.sacc   = string(integer(s-aaa), "999999999")
    ...
    do:
        if available rnn then remtrz.ord = caps(trim( comm.rnn.lname ) + " " + trim( comm.rnn.fname ) + " " + trim( comm.rnn.mname )) + " /RNN/" + s-rnn.
        else
    ...
    if remtrz.ord = ? then
    do:
        run mail("IXqueuerr@fortebank.com", "bankadm@metrocombank.kz", "Поле ORD = ?", "deptax.p 160-170", "1", "", "").
    end.
