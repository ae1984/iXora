/* ibfl_Trx_AccToCard.p
 * MODULE
        ИБФЛ конвертация Счет -> Карт-Счет
 * DESCRIPTION
  ...
  do:
                     ...
                     "Конвертация собственных средств" + dlm + /**/
                               ...
                               Doc:ACC:valacc(Doc:crc) + dlm + /*транзитный валютный счет*/
                                  ...
                                  string(Doc:v_amount) + dlm + /*сумма в конечной валюте*/
                                       string(Doc:crc) + dlm + /*валюта конечная*/
                                                 "arp" + dlm +
                 /*Doc:vclientaccno*/ MsgCode:GetAcc() + dlm + /*счет клиента конечный*/
       ...
       /**********************************************************************************************************/
        ...
        do:
          ...
          run ow_send("DoTransaction","",p-acc_to,"","","PAYMTOIB",Doc:CRCC:get-code(p-acc_to),string(p-amount),vrem,"0",output v-des,output v-out).
          ...
          if Doc:com_conv > 0 then do:
            ...
            if Doc:com_accno = p-acc_from then
            do:
               ...
               /* снятие комиссии с карт-счета*/ 
                 ...
                 /* 
                  ...
                  run txb_trxgen ("dil0075", dlm, vparam, "arp", MsgCode:GetAcc(),0, output rcode, output rdes, input-output s-jh).
                  if rcode <> 0 then do:
                     /*Unfreeze*/
                      run ow_send("UnFreeze","",Doc:com_accno,"","","PAYMFIB","","","",IdFreeze,output v-des,output v-out).
                      if not v-out then do:
