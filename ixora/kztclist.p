 ...
 * CHANGES
     ...
     13.07.03 kanat - Вместо номера телефона и лицевого счета показывается только счет - извещение
...
ON CHOOSE OF bedt IN FRAME f1
    do:
        run kztcin (dat, false, rowid(commonpl)).              
        b1:refresh().
   end.
...
ON CHOOSE OF bnew IN FRAME f1
    do:
        run kztcin (dat, true, rowid(commonpl)).
