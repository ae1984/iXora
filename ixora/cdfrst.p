...
procedure cdfrrep:
  ...
  put skip(1).
  put g-today " at " string(time, "HH:MM") skip.
  put codfrname format nju skip(2). 
  ...
  for each codfr where codfr.codfr = ext_codfr and codfr.code <> ?  use-index main no-lock.
     put unformatted  codfr.code format 'x(15)'
                      codfr.name[1]  format 'x(33)' skip.
  end.
  

  put "--- End of report --------------------------------------------------" skip.
...
procedure info:
 ...
 display 
   ...
   "<p>       - Создать отчет"         at row 2 column 1
   "<up/down> - Выбор кодификатора"   at row 3 column 1
   "<F4>      - Выход"                         at row 4 column 1        
 with no-label title "Помощь" at row 30 column 2 frame jj.
