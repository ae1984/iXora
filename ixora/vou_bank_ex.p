 ...
 * MODULE
        ПРАГМА
 * DESCRIPTION
        Печать операционных и кассовых ордеров
        (Копия vou_bank с параметром для количества приходников/расходников)
 * RUN
 ...
 * CHANGES
          ...
          11.02.2004 kanat - добавил новый входной параметр для вывода (внес - подотчет, получил - погашение)
          07.03.2004 sasco поменял все WORKFILE и WORK-TABLE на TEMP-TABLE
          26.03.2004 sasco убрал формирование БКС
          ...
          30.07.2004 saltanat - добавила передаваемые значения(последние 3) для процедуры jl-prcdcs(v-paramins,KOd_, KBe_, KNP_).
...
define input parameter num_kas_orders as integer.
define input parameter v-paramins as char.
define input parameter p-prtorder as logical.
...
vcash = false.
xdam = 0. xcam = 0.
for each ljl of jh use-index jhln
no-lock
...
if vcash = true then do:
   s-jh = jh.jh.
   do inc = 1 to num_kas_orders:
      run jl-prcdcs(v-paramins,KOd_, KBe_, KNP_).
   end.
end.
