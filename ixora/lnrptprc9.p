 ...
 * MODULE
        Кредитный
 * DESCRIPTION
        Выписка по просроченным процентам
 * RUN
...
def var gl9 as integer no-undo.
find first trxlevgl where trxlevgl.gl = lon.gl and trxlevgl.subled = 'lon' and trxlevgl.level = 9 no-lock no-error.
if avail trxlevgl then gl9 = trxlevgl.glr.

for each histrxbal where histrxbal.sub = 'lon' and histrxbal.acc = lon.lon and histrxbal.lev = 9 no-lock:
    v-sum = histrxbal.dam - histrxbal.cam - v-sum0.
    for each jl where jl.sub = "lon" and jl.acc = lon.lon and jl.lev = 9 and jl.jdt > v-dt and jl.jdt <= histrxbal.dt no-lock:
        create wrk.
