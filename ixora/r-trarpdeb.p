...
update vledger validate(vledger = 0 or can-find(gl where gl.gl = vledger), "Не существует счет")
       vsubled validate(vsubled = "" or can-find(arp where arp.arp = vsubled), "Не существует ARP") with frame opt.
...
find first debgrp where debgrp.arp = vsubled no-lock no-error.
if avail debgrp then vsubled1 = debgrp.arp9.
find first debgrp where debgrp.arp9 = vsubled no-lock no-error.
if avail debgrp then vsubled1 = debgrp.arp.
