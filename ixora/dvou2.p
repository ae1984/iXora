...
/* uvou_bank.p */
/* last : настройка принтера из OFC */
...
/*find dealing_doc where dealing_doc.jh2 eq s-jh no-lock no-error.
v_doc = dealing_doc.docno.*/
dtreg   = jh.whn.
dttime  = jh.tim.
vv-cif  = jh.cif.
/*v-crcrate1 = dealing_doc.rate2.*/
        ...
        if vi = 1 then do:
            ...
            for each wf:
                if wf.wsub eq "cif" then do:
                    ...
                    sub-cod.acc eq wf.wacc and sub = 'arp' no-lock no-error.
