 ...
 * CHANGES
        ...
        11.01.2011 id00004 изменил метод отображения фреймов чтобы избавится от ошибки при компиляции
        19/01/2012 lyubov добавила период, ГК в заголовок, закоментировала вывод view frame detail.
        ...
        12.03.2012 damir - вывод формы в формате WORD (без возможности редактирования) Т.З. № 1256, добавил menu-prt....
...
for each crc no-lock where crc.crc <  5 break by crc.crc:
    ...
    find first t-jl where t-jl.crc = crc.crc no-lock no-error.
    if not avail t-jl then next gotom.
    ...
    put stream v-out unformatted
        ...
        "<TD colspan=8>" bank + "  " + string(vtdt,"99/99/9999") + vtm + "  " + ofi "</TD>" skip
        "</TR>" skip
        "<TR align=left style=""font-size:9.0pt"">" skip
    ...
    for each t-jl where t-jl.crc = crc.crc no-lock break by t-jl.crc:
        ...
        if last-of(t-jl.crc) then do:
            ...
            put stream v-out unformatted
                ...
                "<TD></TD>" skip
                "</TR>" skip
                "<TR align=center style=""font:bold;font-size:9.0pt"">" skip
                "<TD></TD>" skip
                "<TD></TD>" skip
