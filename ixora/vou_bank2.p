 ...
 * CHANGES
     ...
     01.07.2011 damir - добавил стр.256 - 280, добавил передаваемые значения в jl-prcd, если опер Дт - 1 Кт - 5, и Дт - 2 Кт - 5
     ...
     13.07.2011 damir - отражение кодов в обменных операциях.
     ...
     27/08/2011 evseev - входящее значение 3. Без вопросов печатать только операционный ордер
     09.09.2011 aigul - вывод КОД КБЕ КНП в ордерах
     12.09.2011 aigul - вывод КОД КБЕ КНП в ордерах только в 2-2-3,6-5-2,6-5-5, добавила BASES
...
/*ja-eknp*/
    if ljl.ln = 1 then do:
        ...
        if avail translat then do:
            ...
            if avail sub-cod then v-kod = "КОД: " + substr(sub-cod.rcode,1,2) + " КБе: " + substr(sub-cod.rcode,4,2) + " КНП: " + substr(sub-cod.rcode,7,3).
        end.
    ...
    if avail translat then do:
        find first sub-cod where sub-cod.acc = translat.nomer and sub-cod.ccod = "eknp" no-lock no-error.
        if avail sub-cod then if avail sub-cod then v-kod = "КОД: " + substr(sub-cod.rcode,1,2) + " КБе: " + substr(sub-cod.rcode,1,2) + " КНП: " + substr(sub-cod.rcode,7,3).
    end.
    if ljl.ln = 2 then do:
...
if oper_Ok = 3 then do: v-oper_Ok = oper_Ok. oper_Ok = 1. end.
...
if vcash = true then do:
   s-jh = jh.jh.
   if v-oper_Ok <> 3 then message "Печатать кассовый ордер?" update l-prn.
...
if vcash = true then do:
   s-jh = jh.jh.
   if l-prn then
   run jl-prcd3(KOd_, KBe_, KNP_, KOd_1, KBe_1, KNP_1, KOd_2, KBe_2, KNP_2, s-jh).  /* 30.07.04 saltanat - включила передаваемые значения *//*damir добавил новые переменные*/
end.
