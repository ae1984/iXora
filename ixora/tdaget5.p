 ...
 * MODULE
        Депозиты
 * DESCRIPTION
        Частичное изъятие сумм с депозитов "метро-супер-ЛЮКС"
 * RUN
        вызов из процедуры
 * CALLER
        tdacls2.p
 ...
 * CONNECT 
         bank
 * CHANGES 
        20.01.2009 id00004 добавил льготные условия для депозита "200759121" согласно СЗ от 19.01.2009
...
End Function.
   ...
   find last aaa where aaa.aaa = v-aaa exclusive-lock no-error.
     ...
     if aaa.crc = 3  then do: find sysc "rateeu" no-lock.   if available sysc then d-brate2 = sysc.deval. end.
   ...
   if i-mon < 1 then do:
      message "По депозиту нет начисленных процентов(изъятие сумм возможно через 18 месяцев) " skip "" view-as alert-box question buttons ok title "".
      return.
   end.
   ...
   if i-mon < 18 and v-sum > aaa.stmgbal then do: 
/*СЗ от 19.01.09 льготные условия " */
if  aaa.aaa <> "200759121" then do:
      message "Сумма изъятия превышает сумму выплаченных процентов " skip "" view-as alert-box question buttons ok title "".
      return.
end.
   end.
...
if aaa.aaa = "200759121" then do: /*СЗ от 19.01.09 льготные условия*/
    d_sumfreez = 5000000.
end.
