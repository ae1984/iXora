...
/* r-obrezf.p
   ...
   счета FUN 125110 125120 125200 
...
for each fun  where fun.gl = v-gl  no-lock.      /* 1 - по каждому счету */

    find bankl where bankl.bank = fun.bank no-lock no-error.   /* банк */

     if bankl.frbno <> 'KZ' then                 /* резидентство */
       v-rez = 2. 
      else v-rez = 1.
                                                 
   find crc of fun no-lock no-error.              /* валюта */
   
   for each jl where jl.acc eq fun.fun 
   ...
   w-rab.rez = v-rez.
   w-rab.pr = ' '.
   find last crchis where crchis.crc = fun.crc and crchis.rdt le jl.whn
           ...
           if v-acc = "" then 
              v-acc = string(jla.gl,"999999").
                ...
                if jla.subled = "ARP"
                   ...
                   then do:
                   find dfb where dfb.dfb = v-acc no-lock no-error.
                   v-acc = string(dfb.gl,"999999").  
                   end.
