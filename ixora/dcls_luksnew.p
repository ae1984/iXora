 ...
 * MODULE
        Закрытие опердня
 * DESCRIPTION
        Начисление процентов по депозитам "МЕТРОЛЮКС с 01/06/2011"
 * RUN
 ...
 * AUTHOR
        19.05.2011 evseev - взят за основу dcls_luks
 * BASES
        BANK COMM
 * CHANGES
...
fname1 = "acrluksnew" + substring(string(g-today),1,2) + substring(string(g-today),4,2) + ".txt".
...
for each lgr where lgr.led = "TDA" and lgr.feensf = 6 :
   ...
   for each aaa where aaa.lgr eq lgr.lgr break by aaa.crc:
        ...
        if lgr.intcal = "D" then do:
            ...
            if ev-date <> ? then do: DoPay = true. end.
            ...
            if DoPay then do:
                 ...
                 if aaa.regdt < 08/01/2011 then do:
   ...
   if dayacr.loval eq true then do:
      if vtotaip > 0 then do transaction:
         ...
         if not avail trxlevgl or not avail trxlevgl11 then do:
            ...
            Message v-text. put stream m-out unformatted v-text. output close. pause. return.
         end.
        ...
        if lgr.crc ne  1 then do:
           ...
           assign jl.jh =  jh.jh
                  ...
                  jl.dam = vtotaip
                  jl.dc =  "D"
                  jl.gl =  /*sysc.inval*/ /*1858*/ getConvGL(lgr.crc,"D")
                  jl.lev = 1
                  jl.crc = lgr.crc
                  jl.acc = ""
                  jl.rem[1] = "TOTAL " + string(vcnt) + " ACCOUNTS " + lgr.lgr.
           ...
           assign jl.jh =  jh.jh
                  ...
                  jl.cam = vtotaipc
                  jl.dc =  "C"
                  jl.gl =  /*sysc.inval*/ /*1858*/  getConvGL(1,"C")
                  jl.lev = 1
                  jl.crc = 1
                  jl.acc = ""
                  jl.rem[1] = "TOTAL " + string(vcnt) + " ACCOUNTS " + lgr.lgr.
...
Procedure EvaluateExpiryDate.
 ...
 if months = 2 and days = 29  and  (( (year(j_lstmdt)  + years) - 2000) modulo 4) <> 0 then do: months = 3.  days = 1.  end.

 j_expdt = date(months, days, year(j_lstmdt) + years).
 if month(j_lstmdt) <> month(j_lstmdt + 1) then do:
    if month(j_expdt) = 3 and day(j_expdt) = 1 then do: end.
    else j_expdt = j_expdt - 1.
 end.

End procedure.

run savelog('dcls_luksnew','644').