...
run savelog('dcls_ForteMaximum','74').
find first aaa where lookup(aaa.lgr,"B15,B16,B17,B18,B19,B20") > 0 no-lock no-error.
if not avail aaa then return.
fname1 = 'ForteMaximum_' + substring(string(g-today),1,2) + substring(string(g-today),4,2) + '.txt'.
output stream m-out to ForteProfitable.txt.
...
run savelog('dcls_ForteMaximum','89').
for each lgr where lgr.intcal = "D" use-index intcal:
    if lookup(lgr.lgr,"B15,B16,B17,B18,B19,B20") = 0 then next.
    vcnt = 0. vtotaip = 0. vtotaipc = 0.
    run savelog('dcls_ForteMaximum','93. ' + lgr.lgr).
    c-aaa:
    for each aaa where aaa.lgr = lgr.lgr  break by aaa.crc:
        run savelog('dcls_ForteMaximum','96. ' + aaa.aaa).
        ...
        run savelog('dcls_ForteMaximum','109. ' + aaa.aaa).
        ...
        if not avail crc then do:
           run mail("id00787@metrocombank.kz", "METROCOMBANK <abpk@metrocombank.kz>", "ОШИБКА: в dcls_ForteMaximum",
        ...
        run savelog('dcls_ForteMaximum','161. ' + aaa.aaa).
        if DoPay then do:
           if ev-date <> ? then do:
              ...
              run savelog('dcls_ForteMaximum','168. ' + aaa.aaa).
              create accr-m10.
              assign
              ...
              run PayInterests.
              run savelog('dcls_ForteMaximum','185. ' + aaa.aaa + ' ' + string(ev-date)).
              run savelog('dcls_ForteMaximum','186. ' + aaa.aaa + ' ' + string(aaa.expdt)).
              /*if v-intday2 > 0 and ev-date < aaa.expdt - 1 then do:
                  v-m10 =  v-intday2 * vbal * aaa.rate / (aaa.base * 100).
                  run savelog('dcls_ForteMaximum','177. ' + aaa.aaa).
                  create accr-m10.
                  assign
                      accr-m10.aaa     = aaa.aaa
                      accr-m10.rate    = aaa.rate
             ...
             if DoUpr then do:
                if uv-date <> ? then do:
                   ...
                   v-intday2 = s-intday - v-intday1.
                   v-m10 =  vbal * aaa.rate * v-intday1 / (aaa.base * 100).
                   run savelog('dcls_ForteMaximum','206. ' + aaa.aaa).
                   create accr-m10.
                   assign
                       accr-m10.aaa     = aaa.aaa
                       accr-m10.rate    = aaa.rate
                   ...
                   if v-intday2 > 0 then do:
                      ...
                      v-m10 =  v-intday2 * vbal * aaa.rate / (aaa.base * 100).
                      run savelog('dcls_ForteMaximum','235. ' + aaa.aaa).
                      create accr-m10.
                      assign
                          accr-m10.aaa     = aaa.aaa
                          accr-m10.rate    = aaa.rate
                ...
                run savelog('dcls_ForteMaximum','261. ' + aaa.aaa).
                create accr-m10.
                assign
                    accr-m10.aaa     = aaa.aaa
                    accr-m10.rate    = aaa.rate
        ...
        if v-accrued > 0  then do:
           run savelog('dcls_ForteMaximum','288. ' + aaa.aaa).
           ...
           run savelog('dcls_ForteMaximum','316. ' + aaa.aaa).
        end.
  ...
  if lgr.crc ne 1 then do:
    ...
    run savelog('dcls_ForteMaximum','394. ' + aaa.aaa).
  end. /* for each lgr */
...
output stream m-out close.
unix silent mv ForteProfitable.txt value(fname1).
                 ...
                 if avail urdpinfo and urdpinfo.rem1 = "1" then do:
                    if d_tax > 0 then do:
                       ...
                       if cif.geo = '022' and cif.type = 'B' then do:
                          ...
                          if avail bnlg-sysc then v-nlg = bnlg-sysc.chval.
                          if aaa.crc <> 1 then do:
                             /*vparam = string(d_tax) + vdel + aaa.aaa + vdel + string("15% подоходный налог, " + cif.name + " " + cif.bin) + vdel + "".*/
