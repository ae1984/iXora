...
form
    ...
    s-lccor    label 'Number of Corr.    ' validate(can-find(LCswt where LCswt.LC = s-LC and Lcswt.mt = 'O799' and LCswt.lccor = s-lccor no-lock),'Enter the Number of Correspondence!') format ">9" help "F2 - help" skip
    s-corsts   label 'Corr. status       ' format "x(5)" skip
    v-lcerrdes no-label format "x(60)" skip
    with width 70 side-label overlay centered row 3 frame frcor title ' INCOMING CORRESPONDENCE '.
...
on help of s-lccor in frame frcor do:
    {itemlist.i
     ...
     &where   = " LCswt.LC = s-lc and LCswt.mt = 'O799' and LCswt.lccor > 0"
     &chtype  = "integer"
