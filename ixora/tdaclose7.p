 ...
 * MODULE
        Депозиты
 * DESCRIPTION
        Закрытие счетов по депозитам "метро-супер-ЛЮКС"
 * RUN
        вызов из процедуры
 * CALLER
        tdacls.p
 ...
 * CONNECT
         bank
 * CHANGES
        25/08/2011 evseev - исправил ошибку bf-acc.cla на 18
        22/12/2011 evseev - добавил логирование
        28/10/2013Luiza  - ТЗ 1932 изменила параметры шаблона cda0003 и uni0048
*/
...
End Function.
   ...
   if not avail aaa then do:
      run savelog( "tdaclose7", aaa.aaa + " Не найден счет").
      message "Не найден счет" aaa.aaa. pause. return.
   end.

   if aaa.opnamt = 0 then do:
      run savelog( "tdaclose7", aaa.aaa + " счет открыт НЕКОРРЕКТНО дата открытия не совпадает с взносом основной суммы").
   ...
   if not avail acvolt then do:
      run savelog( "tdaclose7", aaa.aaa + " счет открыт НЕКОРРЕКТНО").
   ...
   if (aaa.cr[1] - aaa.dr[1]) - aaa.hbal <> 0 then do:
      run savelog( "tdaclose7", aaa.aaa + " Одним из пользователей было сделано частичное изъятие но выплата клиенту не произведена").
   ...
   if i-mon < 1 then do:
      run savelog( "tdaclose7", aaa.aaa + " Прошло меньше месяца").
   ...
   do:
      run savelog( "tdaclose7", aaa.aaa + " Прошло больше месяца").
      v-sumchkamt = 0.
...
/*               run Get_Rate_18(tmp-conv.aaa, t-ind, output v-rt1).  */
         ...
         d_sumrt = d_sumrt + ((v-opnamt * (v-rt1) ) / (aaa.base * 100)).
          ...
          if avail bf-aaa and avail bf-acvolt then do:
             ...
             if ev-date <> ? then do:
                run tdagetrate(bf-aaa.aaa, bf-aaa.pri, /*bf-aaa.cla*/ 18 , ev-date, bf-aaa.opnamt, output v-rt1).
                if i-mon < 18 then do:
   ...
   run savelog( "tdaclose7", aaa.aaa + "Сумма в размере " + trim(string((aaa.cr[1] - aaa.dr[1]) - d_1% + d_3%  ,'z,zzz,zzz,zz9.99-')) + crc.code + " будет перечислена на счет!").
   ...
   if v-ans then run savelog( "tdaclose7", aaa.aaa + " v-ans = true").
   else run savelog( "tdaclose7", aaa.aaa + " v-ans = false").
   ...
   if g-today <> aaa.regdt then do:
         ...
         if d_3% > 0 and d_3% <= (aaa.cr[2] - aaa.dr[2]) then do:
            run savelog( "tdaclose7", aaa.aaa + " Проводка с 2 на 1 уровень").
         ...
         if d_1% > 0 and d_1% <= (aaa.cr[1] - aaa.dr[1]) then do:
            run savelog( "tdaclose7", aaa.aaa + " Проводка с 1 на 2 уровень").
            ...
            if rcode ne 0 then do:
               run savelog( "tdaclose7", aaa.aaa + " UNI0074 " + rdes).
         ...
         if s-amt11 > 0 then do:
            run savelog( "tdaclose7", aaa.aaa + " Проводка со 2 на 11 уровень").
            v-jh = 0.
            ...
            if rcode ne 0 then do:
               run savelog( "tdaclose7", aaa.aaa + " cda0003 " + rdes).
         ...
         if s-amt1 > 0 then do:
            run savelog( "tdaclose7", aaa.aaa + " Урегулируем разность если на 2 ур > чем на 11").
            v-jh = 0.
            ...
            if rcode ne 0 then do:
               run savelog( "tdaclose7", aaa.aaa + " uni0048 " + rdes).
               message "uni0048" ' ' rdes. pause. undo,retry.
            end.
         end.
         aaa.sta = "E".
         run savelog( "tdaclose7", aaa.aaa + " статус " + aaa.sta).
   end.
   else do:      /* счет открыли сегодня */
      aaa.sta = "E".
      message "Депозит успешно закрыт".
      run savelog( "tdaclose7", aaa.aaa + "Депозит успешно закрыт. статус " + aaa.sta).
      pause.
   end.
...
Procedure Get_Rate_Real. /*Возврещает ставку счета*/
   ...
   if avail bf-acc then do:
      run tdagetrate(bf-acc.aaa, bf-acc.pri, /*bf-acc.cla*/ 18 , a_date, bf-acc.opnamt, output r_rate).
   end.
   else
   r_rate = 0.
end.
