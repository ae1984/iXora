 ...
 * MODULE
        Пенсионные платежи
 * DESCRIPTION
        Печать письма для клиента по возврату пенсионных взносов
 * RUN
 ...
 * AUTHOR
        10.12.2003 sasco
 * CHANGES
        12.12.2003 sasco добавил вывод факсимиле
        18.12.2003 sasco добавил и-шку для перекомпиляции
        22.12.2003 sasco добавил настройку вывода или нет факсимиле
*/


/*
    ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! 
    Необходимо настроить sysc.GCVPHD.chval = "должность|ФИО"
    - это директор подразделения, отвечающего за отправку 
    писем по пенсионным платежам

*/
{pnjcommon.i}

/* номер исходящего письма (привязка идет к sysc."GCVPLT".inval) */
define input parameter v-outnum as character.

/* заголовок - получатель, РНН, адрес, и т.д */
define input parameter v-header as character.

/* примечание - причина возврата */
define input parameter v-remark as character.

/* конец письма - таблица со списком людей (из swift.txt) */
...
put unformatted "<tr><td>" skip.
put unformatted "Вы отправляли через наш банк платеж в Накопительный Пенсионный Фонд. К сожалению, Вы неверно указали реквизиты.<br><br>" skip.
put unformatted "Обращаемся к Вам с просьбой уточнить, с каким Накопительным Пенсионным Фондом Вы заключили договор, уточнить СИК вкладчика (-ов)." skip.
put unformatted "Вы можете предоставить правильные реквизиты и всю необходимую информацию в банк в рабочее время с 9.00 до 16.00, и мы отправим деньги повторно.<br><br>" skip.
