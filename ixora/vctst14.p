 ...
 * MODULE
        Валютный контроль
        Тестирование новых форматов сообщений
 * DESCRIPTION
        Приложение 14 - сводная информация за месяц о результатах
 * RUN
...
for each txb where txb.consolid = true and (p-bank = "all" or (txb.bank = s-vcourbank)) no-lock:
  ...
  run vctst14dat (txb.bank, p-depart).
...
if p-option = "rep" then
  run vctst14out.p ("vcrep14.htm", (p-bank <> "all"), v-name, (p-depart <> 0), v-depname).
else*/
  run vctst14msg.
