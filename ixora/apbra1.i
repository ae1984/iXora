 ...
 * AUTHOR
        31/12/99 pragma
 * CHANGES
        29.04.2011 damir - аналог программы jabrw.i.
*/
 ...
 curflag = 1.
    ...
    if crec <> frec then do:
     ...
     {&postkey}
        ...
        run do_sysc_search.
        find first cods where string(cods.gl) matches s-mask and (s-entries = '' or lookup(cods.code, s-entries) > 0)
        use-index codegl_id no-lock no-error.
        trec = recid(cods).
        next upper.
     end.
        ...
        update s-mask label 'Введите маску' skip '(Последнюю * можно не писать)' with side-labels color messages overlay
        row 5 centered frame maskfr.
        hide frame maskfr.
        if s-mask <> '' and substr (s-mask, length(s-mask), 1) <> '*' then
        s-mask = s-mask + '*'.
        find first cods where cods.code matches s-mask and (s-entries = '' or lookup(cods.code, s-entries) > 0) use-index codegl_id no-lock no-error.
        if not avail cods then do:
            message 'Не найдены записи в cods по маске ' s-mask '. Возврат к default' view-as alert-box title ''.
            s-mask = '*'.
            find first cods where cods.code matches s-mask and (s-entries = '' or lookup(cods.code, s-entries) > 0)  use-index codegl_id no-lock no-error.
        end.
        trec = recid(cods).
