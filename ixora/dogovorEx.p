...
def input parameter s-param as char.
...
def var vr-mes1 as char.
def var vk-mes1 as char.
def var vr-period1 as char.
def var vk-period1 as char.
...
if avail bcit then v-kcity = bcit.chval. else v-kcity = v-city.
...
if s-param = "1" then do:
  if lookup(lgr.lgr,"151,153,171,157,176,152,154,172,158,177,173,175,174") > 0  then do:
     v-ifile = "/data/export/" + v-prefix + "dopik3.htm".
...
if s-param = "1" and lookup(lgr.lgr,"151,153,171,157,176,152,154,172,158,177,173,175,174") > 0 then do:
  run defdts(date(acvolt.x1), output vr-mes1, output vk-mes1).
  /*run defdts1(date(acvolt.x1), date(acvolt.x3), output vr-period1, output vk-period1).*/
end.
...
repeat:
   ...
   if s-param = "1" and lookup(lgr.lgr,"151,153,171,157,176,152,154,172,158,177,173,175,174") > 0 then do:
       run Sm-vrd(GetHoldAmount(aaa.aaa), output v-sumopnam).
       run Sm-vrd-kzopti(GetHoldAmount(aaa.aaa), output v-ksumopnam).
   end. else do:
   ...
   repeat:
     ...
     if v-str matches "*r1chs*" then do:
        v-str = replace (v-str, "r1chs", string(day(date(acvolt.x1)),"99") ).
        next.
     end.
     if v-str matches "*k1mes*" then do:
        v-str = replace (v-str, "k1mes", vk-mes1 ).
        next.
     end.

     if v-str matches "*r1mes*" then do:
        v-str = replace (v-str, "r1mes", vr-mes1 ).
        next.
     end.
     if v-str matches "*y1yyy*" then do:
        v-str = replace (v-str, "y1yyy", string(year(date(acvolt.x1)) )).
        next.
     end.

     if v-str matches "*std1*" then do:
        v-str = replace (v-str, "std1",  string(date(acvolt.x1), '99/99/9999') ).
        next.
     end.
     if v-str matches "*end1*" then do:
        v-str = replace (v-str, "end1",  string(date(acvolt.x3), '99/99/9999') ).
        next.
     end.
     ...
     if s-param = "1" and lookup(lgr.lgr,"151,153,171,157,176,152,154,172,158,177,173,175,174") > 0 then do:
         if v-str matches "*wjfnfkrfrj*" then do:
            v-str = replace (v-str, "wjfnfkrfrj", string(GetHoldAmount(aaa.aaa)) + " (" + v-sumopnam + ")").
            next.
         end.

         if v-str matches "*kvclsm*" then do:
            v-str = replace (v-str, "kvclsm", string(GetHoldAmount(aaa.aaa)) + " (" + v-ksumopnam + ")").
            next.
         end.
     end. else do:
         if v-str matches "*wjfnfkrfrj*" then do:
