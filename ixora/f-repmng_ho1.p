 ...
 * CHANGES
        ...
        13.08.2013 damir - Внедрено Т.З. № 1182,1258,1257,1650. Добавил в londoh_clients 4434; com_exp - getRMZExp (добавил CHF,AUD); поставил по lc группу счетов 6000; добавил по
                           com_inexp 5754; по SO_Adjust_provision_account заменил на 3305,3315; добавил по SO_depoGAR 2707; добавил по SO_rashod_deposit 5229; добавил по dueBanks 1264,1705;
                           добавил по securTrade 1745; добавил по lonast 1697; добавил по assets_other 1810,1890 и добавил getGL2; добавил по depov%,depoSM%,depoCORP% 2723;
...
for each t-period no-lock:
   if month(t-period.dtb) = 1 then do:
    run setKritVal("com_incom",t-period.pid, abs(getGroupGL("4601,4602,4603,4604,4606,4607,4608,4609,4610,4611,4612,4613,4614,4615,4616,4617,4618",'',t-period.dte))).
   end.
   else do:
...
/* Розница ФЛ */
    ...
    d_all = 0.
    d_all = getKritVal("SO_depov",t-period.pid) + getKritVal("SO_depov1",t-period.pid) + getKritVal("SO_depov2",t-period.pid) + getKritVal("SO_depov3",t-period.pid) + getKritVal("SO_depov%",t-period.pid) + getKritVal("SO_depoGAR",t-period.pid).
   ...
   run setKritVal("SO_allocated_branches", t-period.pid, - getStoredKritValAll("txb","CAPITAL", t-period.dtb )).
    ...
    d_all2 =  getKritVal("SO_Aksion_capital",t-period.pid)
           ...
           + getKritVal("SO_Ner_dohod_pred",t-period.pid)
           + getKritVal("SO_Ner_dohod_tekuch",t-period.pid)
...
for each t-period no-lock:
   ...
   run setKritVal("com_NetCommFXincome1003",t-period.pid,getKritVal("com_incom",t-period.pid) - getKritVal("com_exp",t-period.pid) + /*getKritVal("com_other1003",t-period.pid) +*/ getKritVal("com_FXincome1003",t-period.pid) - getKritVal("com_FXexpense1003",t-period.pid)).
