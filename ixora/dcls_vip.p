 ...
 * MODULE
        Закрытие опердня
 * DESCRIPTION
        Начисление процентов по депозитам "МЕТРО-КЛАССИК"
 * RUN
 ...
 * CHANGES
        ...
        29.03.2011 evseev - изменил Y и 18 на lgr.prefix для DoChgRate
...
fname1 = "acrvip" + substring(string(g-today),1,2) + substring(string(g-today),4,2) + ".txt".
...
for each lgr where lgr.led = "TDA" and lgr.feensf = 4 :
...
/*if aaa.aaa <> "199759283" then next account. */
        ...
        if lgr.intcal = "D" then do:
           ...
           if ev-date <> ? then do:
              ...
              if aaa.regdt < 04.01.2010 then do: /*условие согласно тз-685 */
                 run EventHandler(lgr.prefix, ev-date, date(acvolt.x1), date(acvolt.x3) - 1, output fire).   /*изменил Y Евсеев*/
              end.
              else do:
                 run EventHandler(lgr.prefix, ev-date, date(acvolt.x1), date(acvolt.x3) - 1, output fire).   /*изменил 18 Евсеев*/
              end.
             ...
             if aaa.regdt < 04.01.2010 then do: /*условие согласно тз-685 */
                ev-date = EventInRange(lgr.prefix, g-today, s-target - 1).  /*изменил Y Евсеев*/
             end. else
             do:
                ev-date = EventInRange(lgr.prefix, g-today, s-target - 1).  /*изменил 18 Евсеев*/
             end.
