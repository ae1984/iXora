 ...
 * AUTHOR
        12/12/03 kanat
 * CHANGES
        13/12/03 sasco исправил профит-центр 508 на 509
        07/01/04 kanat убрал проверку на депратамент при вызове jl-prcd.
        01/12/04 kanat добавил проверку на печать приходных ордеров для транзакций с касс для филиалов
...
/*    if (lookup(string(ljl.gl),sysc.chval) <> 0) and (substring(ljl.rem[1],1,5) = 'Обмен') then vcash = true.*/
    if (ljl.gl = sysc.inval) or ((ljl.gl = obmenGL2) and (substring(ljl.rem[1],1,5) = 'Обмен')) then vcash = true.
...
/* Печать ордеров только для филиалов */

if seltxb <> 0 and vcash = true then do:
   s-jh = jh.jh.
...
/*
find first ofc where ofc.ofc = jh.who and ofc.titcd = "509" no-lock no-error.
