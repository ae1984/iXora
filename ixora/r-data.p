 ...
 * MODULE
        ARP счета
 * DESCRIPTION
        Ненулевые остатки на счетах ARP на заданную дату по счету главной книги
 * RUN
 ...
 * AUTHOR
        31/12/99 pragma
 * CHANGES
        13/05/05 sasco Вывод через menu-prt
...
*/
for each arp where arp.gl eq (if varp ne 0 then varp else arp.gl) no-lock   break  by arp.gl by arp.crc:

/*    find first hisarp no-lock where hisarp.arp eq arp.arp and hisarp.fdt le
    v-asof and hisarp.ncrc[1] eq arp.crc no-error.     
    if available hisarp then do:*/
    ...
    if arp.crc ne 1 then do:
        ...
        v-balrate = v-bal.
    if /*arp.rdt le v-asof and */ v-bal ne 0 then 
    put
        arp.arp /*label "KARTES Nr."*/ " "
        arp.rdt /*label "   NO   "*/ format "99/99/9999" " "
        arp.duedt /*label "   L§DZ    "   */ " "
        arp.type /*label "TIPS"*/ format "999" " "
        arp.des /*label "APRAKSTS"*/ " "
        arp.geo /*label "GEO"*/ format "x(3)" " "
        " " arp.crc /*label "VAL"*/ " "
        v-bal /*label "NOMIN…LS" */ 
        format "z,zzz,zzz,zzz,zzz,zzz.99-" " "
        v-balrate /*label "NOMIN…LS Ls"*/
        format "z,zzz,zzz,zzz,zzz,zzz.99-" 
        skip.                                          
