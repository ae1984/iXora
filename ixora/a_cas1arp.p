 ...
 * DESCRIPTION
        Взнос наличных денег на ARP счет
 * BASES
        BANK comm
 * RUN
 ...
 * AUTHOR
        07.12.2011 Luiza
 * CHANGES
        11.03.2012 damir - добавил входной параметр в printord.p.
        ...
        28/03/2012 Luiza - отменила помощь при вводе арп счета и конвертацию дробных
        ...
        30/07/2012 Luiza - добавила обработку on "END-ERROR" of frame f-knp do:
        14/08/2012 Luiza - добавила запрос кода cif клиента при взносе погашения кредита согласно СЗ от 02/08/2012
...
define variable m_sub           as character initial "arp".
...
if v_bin  then v_label = " ИИН плательщика        :". else v_label = " РНН плательщика        :".
...
define frame f_mko
    v-cif label "Введите CIF-код клиента" validate(v-cif <> "", "Hеверное значение CIF-кода")
    with row 11 col 33 overlay side-label.
...
on help of v-joudoc in frame f_main do:
    if v-ek = 1 then run a_help-joudoc1 ("CS4"). else run a_help-joudoc1 ("EK4").
    ...
    if v_u = 2 then do:       /* update */
        ...
        if available joudoc then do:
            ...
            if available joudop then do:
                 if joudop.type <> "CS4"  and joudop.type <> "EK4" then do:
                    message substitute ("Документ не относится к типу взнос наличных денег на АРП счет ") view-as alert-box.
                    return.
                end.
                if v-ek = 1 and joudop.type = "EK4" then do:
                    message substitute ("Документ создан для ЭК ГК 100500") view-as alert-box.
                    return.
                end.
                if v-ek = 2 and joudop.type = "CS4" then do:
...
procedure save_doc:
    ...
    update  v-arp help "АРП счет; F2- помощь; F4-выход" with frame f_main.
    find first arp where arp.arp = v-arp no-lock no-error.
    if avail arp then do:
        v-arpname = arp.des.
        v-crc = arp.crc.
    end.
    displ v-arpname v-crc  v_code v_oper vj-label format "x(35)" no-label with frame f_main.
    pause 0.
    if v-ek = 2 then do:
    ...
    /*код для МКО*/
    if v-arp = "KZ14470192867A000216" or v-arp = "KZ47470192867A000204" or v-arp = "KZ04470192867A000202" then do:
    ...
    if v-ek = 2 then do:
     ...
     if v-ja then do:
        ...
        joudop.tim = time.
        joudop.lname = v-cif.
        if v-ek = 1 then joudop.type = "CS4". else joudop.type = "EK4".
...
procedure Delete_transaction:
    ...
    s-jh = joudoc.jh.
    do transaction:
        find last lnrkc where lnrkc.jh = s-jh exclusive-lock no-error. /* удаление записи из lnrkc cif код для погашения кредита */
        if avail lnrkc then do:
            if lnrkc.dtimp <> ? then do:
                message "Данные по заемщикам МКО уже прогружены: удаление невозможно." view-as alert-box.
                undo, return.
            end.
            else delete lnrkc.
        end.
