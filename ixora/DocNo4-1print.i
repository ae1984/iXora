 ...
 * DESCRIPTION
        Описание - 4 Продажа ин.валюты на следующий день
 * RUN
...
assign v-inputfile = "/data/export/statement2.htm".
...
repeat:
    ...
    repeat:
        ...
        if v-str matches "*+A*" then do:
            if V-KOD <> "" then v-str = replace (v-str,"+A",substr(V-KOD,1,1)).
            else v-str = replace (v-str,"+A","").
            next.
        end.
        if v-str matches "*+B*" then do:
            if V-KOD <> "" then v-str = replace (v-str,"+B",substr(V-KOD,2,1)).
            else v-str = replace (v-str,"+B","").
            next.
        end.
        if v-str matches "*+C*" then do:
            if V-KOD <> "" then v-str = replace (v-str,"+C",substr(V-KOD,1,1)).
            else v-str = replace (v-str,"+C","").
            next.
        end.
        if v-str matches "*+D*" then do:
            if V-KOD <> "" then v-str = replace (v-str,"+D",substr(V-KOD,2,1)).
            else v-str = replace (v-str,"+D","").
            next.
        end.
        if v-str matches "*vknp*" then do:
            v-str = replace (v-str,"vknp","223").
            next.
        end.
        if v-str matches "*fiorukovodname*" then do:
            v-str = replace (v-str,"fiorukovodname","").
            next.
        end.
        ...
        if v-str matches "*purpose*" then do:
            if trim(purpose) = "" then v-str = replace (v-str,"purpose","").
            else do:
                purpose = replace (purpose,'«', '"').
                purpose = replace (purpose,'»', '"').
                if dType = 1 or dType = 2 or dType = 3 or dType = 4 or dType = 6 then v-str = replace (v-str,"purpose",entry(2, purpose, '"')) no-error.
                else v-str = replace (v-str,"purpose"," ") no-error.
            end.
