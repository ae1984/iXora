 ...
 * AUTHOR
        26.12.2003 marinav
 * CHANGES
        05.01.2004 marinav - анализ обеспечения
        07.03.2004 sasco поменял все WORKFILE и WORK-TABLE на TEMP-TABLE
        31/05/2004 madiyar - добавил обработку схемы 4
        20/08/2004 madiyar - сохраняем автоматически рассчитанные рейтинги в t-klass.info[1],[2],[3]
        27/08/2004 madiyar - при сохранении новой класс-ции вставил pause - чтобы успели message разглядеть
        17/09/2004 madiyar - дату классификации риск-менеджером сохраняем в t-klass.info[4]
        20.05.05   marinav - добавила пункт - скопировать рейтинг на все счета данного клиента
        31/03/2006 madiyar - дни просрочки - из londebt
        12/10/2010 madiyar - статус "без просрочки" при просрочке до 14 дней включительно
*/
...
if not avail lon then do:
    message "Ссудный счет не найден" view-as alert-box.
    return.
end.
...
if v-select = 2 then do:
    ...
    for each lon where lon.cif = v-cif and lon.lon <> s-lon no-lock:
        for each t-klass:
            create kdlonkl.
            buffer-copy t-klass to kdlonkl.
            kdlonkl.kdlon = lon.lon.
        end.
    end.
end.
