...
form
    ...
    s-lc       label 'Reference Number  ' format "x(15)" validate(can-find(LC where LC.LC = s-LC and lc.bank = s-ourbank no-lock) or v-find,'Enter DC number!') skip
    v-lcsumorg label 'DC Original Amount' format ">,>>>,>>>,>>>,>>9.99" v-lccrc1 no-label format "x(3)" skip
    v-lcsumcur label 'DC Current  Amount' format ">,>>>,>>>,>>>,>>9.99" v-lccrc2 no-label format "x(3)" skip
    v-lcdtexp  label 'Expiry Date       ' format "99/99/9999" skip
    v-lcsts    label 'DC status         ' format "x(5)"       skip
...
on help of s-LC in frame frlc do:
    if not v-find or v-cif <> '' then do:
       {itemlist.i
         ...
         &flddisp = " LC.LC label 'Reference Number ' format 'x(15)' LC.LCsts label 'DC status' format 'x(15)' "
         &chkey = "LC"
         &index  = "lcrwhn"
     ...
     if v-find and v-cif = '' then do:
        {itemlist.i
         ...
         &flddisp = " LC.LC label 'Reference Number' format 'x(15)' LC.LCsts label 'DC status' format 'x(5)' LC.cif label 'Aplicant Code' format 'x(6)'"
         &chkey = "LC"
         &index  = "lcrwhn"
         &end = "if keyfunction(lastkey) = 'end-error' then return."
         }
