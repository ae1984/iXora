...
define new shared variable fcha as character extent 11 initial [
    ...
    "bol", "rez", "lie", "ven", "val" ].
define variable selec as character format "x(15)" extent 2 initial
    ["ATSKAITE", "SKAјO№ANA"].
...
display selec with no-labels row 4 attr-space frame zq centered no-box.
choose field selec with frame zq.

if frame-index eq 2 then do:
   hide all.
   run dinc-set(60,"DINCON").
end.
else do:
...
find sysc where sysc.sysc eq "bildin" no-lock no-error.
	...
	message "fails sysc nav atrasts  bildin (direktorijas nosaukums) !".
	return.
    end.
    else
	filename = sysc.chval.
...
{report1.i 60}
{report2.i 60}
vtitle = "BILANCES DINAMIKA  (KONSOLIDЁTA BILANCE)  PAR  " + string (ddate).
...
if sal then do:
	...
	if search (fname) eq ? then do:
	    ...
	    run dincon (input fname, input "SALASPILS").    end.
end.
if ola then do:
	...
	if search (fname) eq ? then do:
	    ...
	    run dincon (input fname, input "OLAINE").       end.
end.
if ces then do:
	...
	if search (fname) eq ? then do:
	    ...
	    run dincon (input fname, input "CЁSIS").        end.
end.
if jur then do:
	...
	if search (fname) eq ? then do:
	    ...
	    run dincon (input fname, input "J®RMALA").      end.
end.
if dgl then do:
	...
	if search (fname) eq ? then do:
	    ...
	    run dincon (input fname, input "DAUGAVPILS").   end.
end.
if jek then do:
	...
	if search (fname) eq ? then do:
	    ...
	    run dincon (input fname, input "JЁKABPILS").    end.
end.
if bol then do:
	...
	if search (fname) eq ? then do:
	    ...
	    run dincon (input fname, input "BOLDERAJA").    end.
end.
if rez then do:
	...
	if search (fname) eq ? then do:
	    ...
	    run dincon (input fname, input "RЁZEKNE").      end.
end.
if lie then do:
	...
	if search (fname) eq ? then do:
	    ...
	    run dincon (input fname, input "LIEP…JA").      end.
end.
if ven then do:
	...
	if search (fname) eq ? then do:
	    ...
	    run dincon (input fname, input "VENTSPILS").    end.
end.
if val then do:
	...
	if search (fname) eq ? then do:
	    ...
	    run dincon (input fname, input "VALMIERA").     end.
end.
