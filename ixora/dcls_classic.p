...
fname1 = "acrclassic" + substring(string(g-today),1,2) + substring(string(g-today),4,2) + ".txt".
...
for each lgr where lgr.led = "TDA" and lgr.feensf = 2 :
...
/*if aaa.aaa <> "199759791" then next account. */
        ...
        if lgr.intcal = "D" then do:
           ...
           if ev-date <> ? then do:
              ...
              run EventHandler(lgr.intpay, ev-date, date(acvolt.x1), date(acvolt.x3) - 1, output fire).
              if fire then DoPay = true.
...
/*Новые сроки при пролонгации*/
if DoPay then run PayInterests.
...
Procedure PayInterests.
   ...
   run tdasethold(aaa.aaa, v-sumchkamt).
