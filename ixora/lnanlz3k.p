 ...
 * MODULE
        Кредитный Модуль
 * DESCRIPTION
        Анализ кредитного портфеля для БЫСТРЫХ ДЕНЕГ ПЛЮС (казпочта)
 * RUN
 ...
 * MENU
        4-2-5-13 
 * AUTHOR
        02/10/2006 madiyar - скопировал из lnanlz3.p с небольшими изменениями
 * BASES
        bank, comm
 * CHANGES
*/  
...
/* текущий вид кредита */
define new shared var s-credtype as char no-undo init "6". /*быстрые деньги*/
{pk-sysc.i}
...
for each wrk where break by wrk.datot desc.
  ...
  if first-of(wrk.datot) then do:
    ...
    find last crchis where crchis.crc = 11 and crchis.regdt < wrk.datot no-lock no-error.
  ...
  if wrk.lon <> "" then do:
    ...
    if wrk.crc = 11 then crccur = crceur.
...
put stream rep unformatted
    "<b>Анализ программы БЫСТРЫЕ ДЕНЬГИ ПЛЮС на " d1 format "99/99/9999" "<BR>" skip
...
put stream rep unformatted "<tr><td colspan=2>Фонд страхования кредитных рисков на дату, KZT</td>" skip.
do i = 1 to 4:
...
put stream rep unformatted "<tr><td colspan=2>Фонд страхования кредитных рисков за период, KZT</td>" skip.
do i = 1 to 4:
  put stream rep unformatted "<td>" replace(replace(string(wrkrep.m-values[i], ">>>,>>>,>>>,>>>,>>9.99"),","," "),".",",") "</td>".
...
put stream rep unformatted "<tr><td colspan=2>Комиссия за ведение тек. счета на дату, KZT</td>" skip.
do i = 1 to 4:
  put stream rep unformatted "<td>" replace(replace(string(wrkrep.m-values[i], ">>>,>>>,>>>,>>>,>>9.99"),","," "),".",",") "</td>".
  itogo1[i] = itogo1[i] + wrkrep.m-values[i].
end.
...
put stream rep unformatted "<tr><td colspan=2>Комиссия за ведение тек. счета за период, KZT</td>" skip.
do i = 1 to 4:
  put stream rep unformatted "<td>" replace(replace(string(wrkrep.m-values[i], ">>>,>>>,>>>,>>>,>>9.99"),","," "),".",",") "</td>".
  itogo2[i] = itogo2[i] + wrkrep.m-values[i].
end.
