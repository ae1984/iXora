...
else if vselect eq "N" then do:
 {itemlist.i
         &var  = "def var vname like cif.sname."
         &updvar  = "{imesg.i 2808} update vname."
         &where = "trim(trim(cif.prefix) + ' ' + trim(cif.sname)) matches '*' + trim(vname) + '*' "
         &frame = "row 5 centered scroll 1 12 down overlay "
         &index = "sname"
         &chkey = "cif"
         &chtype = "string"
