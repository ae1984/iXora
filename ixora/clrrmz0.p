...
if depart = '' then m_pid = 'LB'.
else m_pid = 'LB-'  + depart.*/

m_pid = 'LB'.
ddat = g-today.
clear frame ans.
update "Дата:" ddat with row 3 no-label frame dat.
find first clrdoc where clrdoc.rdt = ddat no-lock no-error.
/******************/

if available clrdoc then do:                              
    repeat:
      find first clrdoc where clrdoc.rdt = ddat and clrdoc.pr = vnum no-lock no-error.
          if available clrdoc then  vnum = vnum + 1. 
          else leave.
    end. 
    
    v_num = vnum -  1.

    update "Номер пачки по SCLEAR00 ?" vnum format "zz9  " with row 3 column 19 no-label frame ans. 

    find first clrdoc where clrdoc.rdt = ddat and clrdoc.pr = vnum no-lock no-error.
    if available clrdoc and vnum = v_num then do:
...
if vvans = true  then do:   
    ...
    find first clrdoc where clrdoc.rdt = ddat and clrdoc.pr = vnum no-lock no-error.      
    if available clrdoc then do:
        if clrdoc.maks = true then do:
            update "SCLEAR00 сегодня уже сформирован. Переформировать ?" otv
   ...
   for each clrdoc where clrdoc.rdt = ddat and clrdoc.pr = vnum: 
     /*do transact:*/
        find remtrz where remtrz.remtrz = clrdoc.rem exclusive-lock.
        find que where que.remtrz = clrdoc.rem exclusive-lock.
        que.pid = m_pid.
        que.con = "W" .
   ...
   for each que where que.pid = m_pid 
    ...
    find first clrdoc where clrdoc.rdt = ddat 
        and clrdoc.rem eq remtrz.remtrz  use-index rem no-lock no-error.
    if not available clrdoc then do: /*transaction*/
        k1 = k1 + 1.
        disp "Обработано " remtrz.remtrz k1 "код очереди = " + m_pid with no-label centered no-box frame aaa.
        pause 0.
        create clrdoc.
        clrdoc.rem = remtrz.remtrz. 

        clrdoc.bank = remtrz.rbank.
        clrdoc.amt = remtrz.payment.
        if remtrz.ba begins '/'  then clrdoc.tacc = substring(remtrz.ba,2). else clrdoc.tacc = remtrz.ba.
        clrdoc.facc = remtrz.sacc.
        clrdoc.rdt = ddat.
        clrdoc.pr = vnum.
        clrdoc.maks = false.
     ...
     do transaction:
        ...
        if remtrz.rbank begins bbcod then 
         ...
         if remtrz.rbank begins 'lat' then
          clrdoc.bank = substr(remtrz.rbank,4,3).
         else clrdoc.bank =  remtrz.rbank.
        end. 
         clrdoc.amt = remtrz.payment.
         clrdoc.facc = remtrz.sacc.
         if remtrz.ba begins '/'  then
          clrdoc.tacc = substring(remtrz.ba,2).
         else clrdoc.tacc = remtrz.ba.
          clrdoc.rdt = ddat.
          clrdoc.pr = vnum.
          clrdoc.maks = false.
          s-remtrz = remtrz.remtrz.
...
mt102_cnt = 0.
for each clrdoc where clrdoc.rdt = ddat and clrdoc.pr = vnum 
    and substr(clrdoc.tacc,1,9) = '000080000' and clrdoc.bank = '190501008':
    mt102_cnt = mt102_cnt + 1.
    if mt102_cnt > mt102_max then clrdoc.pr = vnum + 1.
end.
find first clrdoc where clrdoc.rdt = ddat and clrdoc.pr = vnum + 1 no-lock no-error.
if available clrdoc then 
...
for each clrdoc where clrdoc.rdt = ddat and clrdoc.pr = vnum no-lock break by clrdoc.bank:     
    nbank = nbank + 1.
    vsum = vsum + clrdoc.amt.
    if last-of(clrdoc.bank) then do:
       nk = nk + 1.
       create ree.
       ree.npk = nk.
       ree.bank = clrdoc.bank.
       ree.quo = nbank.
       ree.kopa = vsum.
       vvsum = vvsum + vsum.
       nnsum = nnsum + nbank.
...
&postkey = "else if keyfunction(lastkey) = 'RETURN' then do:
             run clrrmz1(ree.bank, ddat, vnum).
              vvsum = vvsum - ree.kopa.
              nnsum = nnsum - ree.quo.
              for each clrdoc where clrdoc.rdt = ddat and clrdoc.pr = vnum and
                clrdoc.bank = ree.bank no-lock:
                nbank = nbank + 1.
                vsum = vsum + clrdoc.amt.
              end.
                ree.quo = nbank.
