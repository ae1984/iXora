...
run savelog('dcls_ForteProfitable','73').
find first aaa where lookup(aaa.lgr,"B01,B02,B03,B04,B05,B06,B07,B08") > 0 no-lock no-error.
if not avail aaa then return.
fname1 = 'ForteProfitable_' + substring(string(g-today),1,2) + substring(string(g-today),4,2) + '.txt'.
...
run savelog('dcls_ForteProfitable','89').
for each lgr where lgr.intcal = "D" use-index intcal:
    if lookup(lgr.lgr,"B01,B02,B03,B04,B05,B06,B07,B08") = 0 then next.
    vcnt = 0. vtotaip = 0. vtotaipc = 0.
    run savelog('dcls_ForteProfitable','93. ' + lgr.lgr).
    c-aaa:
    for each aaa where aaa.lgr = lgr.lgr  break by aaa.crc:
        run savelog('dcls_ForteProfitable','96. ' + aaa.aaa).
        ...
        run savelog('dcls_ForteProfitable','109. ' + aaa.aaa).
        ...
        if not avail crc then do:
           run mail("id00787@metrocombank.kz", "METROCOMBANK <abpk@metrocombank.kz>", "ОШИБКА: в dcls_ForteProfitable",
        ...
        intrat = aaa.rate.
        vbal = IntBase().
        run savelog('dcls_ForteProfitable','145. ' + aaa.aaa).
        if DoPay then do:
           if ev-date <> ? then do:
              ...
              run savelog('dcls_ForteProfitable','152. ' + aaa.aaa).
              create accr-m10.
              assign
              ...
              /*if v-intday2 > 0 then do:
                  ...
                  run savelog('dcls_ForteProfitable','177. ' + aaa.aaa).
                  create accr-m10.
                  assign
                      accr-m10.aaa     = aaa.aaa
                      accr-m10.rate    = aaa.rate
             ...
             if DoUpr then do:
                if uv-date <> ? then do:
                   ...
                   v-intday2 = s-intday - v-intday1.
                   v-m10 =  vbal * aaa.rate * v-intday1 / (aaa.base * 100).
                   run savelog('dcls_ForteProfitable','206. ' + aaa.aaa).
                   create accr-m10.
                   assign
                       accr-m10.aaa     = aaa.aaa
                       accr-m10.rate    = aaa.rate
                   ...
                   if v-intday2 > 0 then do:
                      ...
                      v-m10 =  v-intday2 * vbal * aaa.rate / (aaa.base * 100).
                      run savelog('dcls_ForteProfitable','235. ' + aaa.aaa).
                      create accr-m10.
                      assign
                          accr-m10.aaa     = aaa.aaa
                          accr-m10.rate    = aaa.rate
                ...
                run savelog('dcls_ForteProfitable','261. ' + aaa.aaa).
                create accr-m10.
                assign
                    accr-m10.aaa     = aaa.aaa
                    accr-m10.rate    = aaa.rate
        ...
        if v-accrued > 0  then do:
           run savelog('dcls_ForteProfitable','288. ' + aaa.aaa).
           ...
           run savelog('dcls_ForteProfitable','316. ' + aaa.aaa).
        end.
  ...
  if lgr.crc ne 1 then do:
    ...
    run savelog('dcls_ForteProfitable','394. ' + aaa.aaa).
end. /* for each lgr */
    ...
    def var d_tax as decimal decimals 2 init 0.
      ...
      if v-pay > 0 then do:
         ...
         if rcode ne 0 then do:
            run savelog('dcls_ForteProfitable', '556. ' + aaa.aaa + ' ' + string(rcode) + ' ' + rdes).
         end.
         else do:
                 create aad.
                 ...
                 if avail urdpinfo and urdpinfo.rem1 = "1" then do:
                    if d_tax > 0 then do:
                       ...
                       if rcode ne 0 then do:
                          run savelog('dcls_ForteProfitable', '627. ' + aaa.aaa + ' ' + string(rcode) + ' ' + rdes).
                       end.
                       else do:
                          if not s-bday then do:
                             find b-jh where b-jh.jh = v-jh exclusive-lock.
