...
def input parameter m-loncat like lon.loncat.
...
if m1-hs <> ? then do:
	...
	glbl.cgr = m-cgr  and glbl.loncat = m-loncat no-lock no-error.
	if not available glbl then do:
	    /*
	    find first glbl where glbl.gl = m-gl and  glbl.sts = yes and
	    glbl.cgr = ? and glbl.loncat = m-loncat no-lock no-error.
	    if not available glbl then do:
		*/
		find first glbl where glbl.gl = m-gl and  glbl.sts = yes and
		glbl.cgr = m-cgr  and glbl.loncat = ? no-lock no-error.
		/*
		if not available glbl then do:
		    find first glbl where glbl.gl = m-gl
		    and glbl.sts = yes and glbl.cgr = ? and
		    glbl.loncat = ? no-lock no-error.
		*/
...
if m-key3 then do:
	...
	if glbl.SignSum = true then MenBil.summa = MenBil.summa - m-sum .
	...
	MenBil.stabs = m-ind.
	MenBil.kod-val = m-crccode.
	if glbl.SignSum = true then MenBil.summa = - m-sum .
...
if m-key3 and glbl.period = yes then do:
...
and glbl.loncat = m-loncat no-lock no-error.
if not available glbl then do:
    /*
    find first glbl where glbl.gl = m-gl and  glbl.sts = yes and
    ...
    */
	find first glbl where glbl.gl = m-gl and  glbl.sts = yes and
	glbl.cgr = m-cgr and  integer(glbl.geo) = m-geo
	and glbl.loncat = ? no-lock no-error.
	/*
	if not available glbl then do:
	    find first glbl where glbl.gl = m-gl and  glbl.sts = yes and
	    glbl.cgr = ? and integer(glbl.geo) = m-geo
...
if m-key3 then do:
	...
	if glbl.SignSum then MenBilP.summa = MenBilP.summa - m-sum.
	...
	MenBilP.stabs = m-ind.
	MenBilP.kod-val = m-crccode.
	if glbl.SignSum then MenBilP.summa = - m-sum.
