 ...
 * CALLER
        Список процедур, вызывающих этот файл - vcrepdus.p.
 * SCRIPT
...
def var v-filename as char init "vccomcred.htm".
def var v-title as char.

v-title = "ПРЕДОСТАВЛЕНИЮ<BR>АКТОВ ВЫПОЛНЕННЫХ РАБОТ ПО<BR>ИМПОРТУ<BR>Сумма проплат превышает сумму актов, и<BR>ПО ЭКСПОРТУ<BR>
Сумма актов превышает сумму проплат<br>(ИМПОРТ,ЭКСПОРТ)".

def stream vcrpt.
output stream vcrpt to value(v-filename).

{html-title.i &stream = "stream vcrpt" &title = "Задолжники по предоставлению актов выполненных работ"}
