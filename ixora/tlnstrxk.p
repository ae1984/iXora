...
def var m-count as integer initial 3.
...
def var vappend as logical initial false format "Добавить/Переписать".
...
{tlatrx0.f}
/*
find first aal where aal.regdt eq g-today
      and aal.crc = crc.crc no-lock no-error.
if available aal then 
do:
  for each aal where aal.regdt eq g-today
      and aal.crc = crc.crc no-lock break by aal.aah by aal.ln:
     find aah where aah.aah = aal.aah no-lock no-error.

        m-stn = 0.
        if aal.jh = ? or aal.jh = 0 then do:
            find aah where aah.aah = aal.aah no-lock no-error.
