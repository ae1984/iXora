/* sysc-ed2.p
 * MODULE
        Настройки приложений по расходам по сотрудникам банка
 * DESCRIPTION
        Настройки приложений по расходам по сотрудникам банка
 * RUN
...
define variable s-mask as character format "x(20)" initial "*" no-undo.

define variable s-entries as character initial "1,2,3,4,5,6,7,8,9" no-undo. 
define variable srch as character format "x(40)" label "Строка поиска" initial "" no-undo.
define variable srch2 as character no-undo.
...
&formname  = "sysc"
&framename = "sysc"
&where     = " (sysc.sysc begins  'pr' and lookup(substr(sysc.sysc,3,1),s-entries) > 0) or sysc.sysc begins  'k-' "  /** nataly **/
...
&postkey   = "else if keyfunction(lastkey) = 'RETURN'
              ...
              else if keyfunction(lastkey) = 'F' then /** SASCO **/
                      do:
              ...
              else if keylabel(lastkey) = 'ctrl-f' or keylabel(lastkey) = 'ctrl-а' then
                      do: 
                         run do_sysc_search.
