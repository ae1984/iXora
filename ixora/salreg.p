 ...
 * MODULE
        Клиентские операции
 * DESCRIPTION
        Регистрация з/п платежей Salary
 * RUN
 ...
 * BASES
        BANK COMM
 * CHANGES
        19.07.2013 damir - Changing copy spreg.p. Внедрено Т.З. № 1931.
*/
{mainhead.i}
...
def browse b1
    query q1 no-lock
    display salary_p.whn label "Дата" format "99/99/99"
...
ON CHOOSE OF bedt IN FRAME f1
    do:
        run salregzp (false, rowid(salary_p), dat).
        b1:refresh().
    end.

ON CHOOSE OF bnew IN FRAME f1
    do:
        run salregzp (true, rowid(salary_p),dat).
