 ...
 * AUTHOR
        31/12/99 pragma
 * CHANGES
        16.04.2001 - все платежи или по одному менеджеру
        20.02.2002 - по одному менеджеру -> только тенговые
        04.08.2003 sasco - Вывод наименования клиента в списке RMZ
        07.07.2004 tsoy  - Срочные платежи показывать *
        12.07.2004 tsoy  - Убрал коннект к БД IB
        14.07.2004 tsoy  - Убрал сортировку по срочности
        22.09.2004 dpuchkov - ограничение просмотра платежей.
        30.09.2004 saltanat - Добавила признаки Валютного(vk) и Кредитного Администрирования(dk) контроля
        18.12.2005 tsoy     - добавил время создания платежа.
        03.07.2012 evseev - отструктурировал код. ТЗ-1391
*/

/* h-rmz8i.p
   F2 - для интернетовских платежей в очереди 3А 22.05.2000
*/
...
message "1)По одному менеджеру 2)Все тенговые 3)Все валютные " update choice .

find first que where que.pid = '3a' and que.con <> 'F' and  (can-find(remtrz where remtrz.remtrz = que.remtrz and (remtrz.source  = 'IBH') )) use-index fprc no-lock no-error.
if avail que then do:
   for each que where que.pid = '3a' and que.con <> 'F' and (can-find(remtrz where remtrz.remtrz = que.remtrz and (remtrz.source  = 'IBH') )) use-index fprc no-lock:
       ...
       if (choice = 1 and remtrz.tcrc = 1 and (substr(cif.fname,1,8)) = v-ofc) or (choice = 2 and remtrz.tcrc = 1) or (choice = 3 and remtrz.tcrc <> 1) then do:
          find aaa where aaa.aaa = remtrz.dracc no-lock no-error.
          ...
          find first sub-cod where sub-cod.acc = que.remtrz and sub-cod.d-cod = "urgency" no-lock no-error.
          if avail sub-cod then w-remtrz.urgency = substr(sub-cod.ccode,1,1). else w-remtrz.urgency = "o".
          ...
          if avail vkdkcon then do:
             if vkdkcon.vk = "vk" then w-remtrz.vkc = "*". else w-remtrz.vkc = "".
             if vkdkcon.dk = "dk" then w-remtrz.dkc = "*". else w-remtrz.dkc = "".
          end.
       end.
   end.
end.
...
{browpnp.i
 ...
 &disp = "  w-remtrz.vkc column-label ""ВК"" format 'x(1)'
            ...
            /*w-remtrz.ref column-label ""Nr."" format 'x(19)' */
