...
def shared  temp-table temp
     ...
     field cname      like txb.cif.sname
     field acc       as   char
     field opf       as   char
     ...
     field sumtval   like txb.garan.sumtreb
...
for each txb.cif no-lock,
    ...
    assign temp.cif     = txb.cif.cif
        ...
        temp.npsbal  = '6555' + v-r + v-cgr + v-hs.
	...
	if avail txb.garan then do:
        assign temp.ref      = txb.garan.info[1]
               ...
               temp.sumtval  = txb.garan.sumtreb
...
for each lc where lc.bank = v-bank and lc.lctype = 'i' and lookup(lc.lcsts,'fin,cls,cln') > 0 no-lock:
    ...
    /*--------------------------------поиск 20-значного счета-------------------------------------*/
    if v-gar then find first b-lch where b-lch.lc = lch.lc and b-lch.kritcode = 'DepAcc'  no-lock no-error.
    else find first b-lch where b-lch.lc = lch.lc and b-lch.kritcode = 'CollAcc'  no-lock no-error.
    v-acc = "".
    if available b-lch then v-acc = b-lch.value1.
    if not available b-lch or v-acc = "" then do:
        for each lcres where lcres.lc = lch.lc no-lock.
            if length(trim(lcres.dacc)) = 20 then v-acc = trim(lcres.dacc).
        end.
    end.
    /*---------------------------------------------------------------------------------------------*/
    ...
    if v-dt2 <> ? and v-dt2 < today then next.
    ...
    if avail txb.cif then do:
        assign temp.cif    = txb.cif.cif
            ...
            v-r  = " ".
            v-cgr = " " .
            find last txb.crchs where txb.crchs.crc eq txb.crc.crc no-lock no-error.
            if txb.crchs.hs eq "L" then v-hs = "1".
            else if txb.crchs.hs eq "H" then v-hs = "2".
                else if txb.crchs.hs eq "S" then v-hs = "3".
            if substring(string(integer(txb.cif.geo),"999"),3,1) eq "1" then v-r = "1".
            ...
            find last txb.sub-cod where txb.sub-cod.sub = 'cln' and txb.sub-cod.acc = txb.cif.cif and txb.sub-cod.d-cod = 'secek' use-index sub-cod-idx3 no-lock no-error.
            if available txb.sub-cod then v-cgr = txb.sub-cod.ccode.
            temp.npsbal  = temp.nps + v-r + v-cgr + v-hs.
