 ...
 * CHANGES
       ...
       29/10/2008 madiyar - исходник исчез из откомпиленной библиотеки, перекомпиляция
...
put stream v-out unformatted
    "<P align=left>ЗАКРЫТЫЕ СЧЕТА С  " string(v-dbeg,"99/99/9999") + "  ПО  " + string(v-dend,"99/99/9999") ".</P>" skip
...
put stream v-out unformatted
    ...
    "<TD>КТО <br> ЗАКРЫЛ</TD>" skip
    "<TD>КОГДА <br> ЗАКРЫЛ</TD>" skip
    "</FONT></TR>" skip.

for each cif where (s-type = " " or cif.type = s-type) no-lock break by cif.type by cif.cif:
    ...
    for each aaa where aaa.cif = cif.cif  no-lock :
        if aaa.sta = "C" and
        aaa.cltdt >= v-dbeg and aaa.cltdt <= v-dend then do:
            if v-first then do:
                for each otl :
                    delete otl.
                end.
    ...
    then display stream m-out fill("-",130) format "x(130)"
