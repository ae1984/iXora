 ...
 * MODULE
        Потреб. кредитование
 * DESCRIPTION
        Формирование доп. соглашения по реструктуризации/пролонгации - пролонгация без отсрочки
 * RUN
...
put stream rep unformatted
    ...
    "<tr style=""FONT-SIZE: 9pt; FONT-FAMILY: 'Times New Roman CYR'""><td width=50%></td><td width=50% align=""right"">Регистрационный N 119</td></tr>" skip
...
put stream rep unformatted
    ...
    "Срок пользования Кредитом по Договору продлевается до " + date2str(v-dtend) + " г.</P>" skip
    "</td></tr>" skip.
...
put stream rep unformatted
    ...
    "а также Вознаграждение в размере, предусмотренном действующим Графиком платежей, за 3 (три) полных месяца.</P>" skip
    "</td></tr>" skip.
...
put stream rep unformatted
    "<tr style=""page-break-before:always;FONT-SIZE: 9pt; FONT-FAMILY: 'Times New Roman CYR'""><td width=50%></td><td width=50% align=""right"">Тіркеу N 119</td></tr>" skip
    "<tr style=""font:bold"" align=""center"">" skip
...
put stream rep unformatted
    "<tr><td colspan=2>" skip
    "<P style=""TEXT-ALIGN: justify"">&nbsp;<br>" + string(v-num) + '.' skip
    "Шарт бойынша Несиені пайдалану мерзімі " + date2str_kz(1,v-dtend) + " дейін ўзартылады.</P>" skip
    "</td></tr>" skip.
