 ...
 * CHANGES
        ...
        26/03/2009 madiyar - в библиотеке был старый исходник, переподкинул
        26/05/2009 madiyar - классификация валютных кредитов
        08/07/2009 madiyar - подправил расчет прогнозного ОД, обнуление кол-ва дней просрочки при выходе из просрочки
        24/08/2009 madiyar - штрафы не учитываются в просрочке; пролонгации проставляются всегда "юез пролонгации"; при пересчете признаков редактируется логин(поле "who")
        30/09/2009 madiyar - изменения в классификации; изменил расчет прогнозного ОД
...
for each txb.lon where txb.lon.grp = 90 or txb.lon.grp = 92 no-lock:
    ...
    if v-dt < g-today then do:
        for each kdlonkl where kdlonkl.bank = s-ourbank and kdlonkl.kdcif = txb.lon.cif and kdlonkl.kdlon = txb.lon.lon and kdlonkl.rdt = v-dt no-lock:
            create b-kdlonkl.
            buffer-copy kdlonkl to b-kdlonkl.
            assign b-kdlonkl.rdt = g-today
                   b-kdlonkl.who = g-ofc.
        end.
        release b-kdlonkl.
    end.
    ...
    find first kdlonkl where kdlonkl.bank = s-ourbank and kdlonkl.kdcif = txb.lon.cif and kdlonkl.kdlon = txb.lon.lon and kdlonkl.rdt = g-today and kdlonkl.kod = 'prosr' exclusive-lock no-error.
    if not avail kdlonkl then do:
        create kdlonkl.
        assign kdlonkl.bank = s-ourbank
               kdlonkl.kdcif = txb.lon.cif
               kdlonkl.kdlon = txb.lon.lon
               kdlonkl.rdt = g-today
               kdlonkl.kod = 'prosr'
               kdlonkl.who = g-ofc
               kdlonkl.whn = g-today.
    end.
    if v-maxpr <= 14 then kdlonkl.val1 = '01'.
    if v-maxpr > 14 and v-maxpr <= 30 then kdlonkl.val1 = '02'.
    if v-maxpr >= 31 and v-maxpr <= 60 then kdlonkl.val1 = '03'.
    if v-maxpr >= 61 and v-maxpr <= 90 then kdlonkl.val1 = '04'.
    if v-maxpr >= 91 then kdlonkl.val1 = '05'.

    find bookcod where bookcod.bookcod = 'kdprosr' and bookcod.code = kdlonkl.val1 no-lock no-error.
    if avail bookcod then
        assign kdlonkl.valdesc = bookcod.name
               kdlonkl.rating = deci(trim(bookcod.info[1])).
    if kdlonkl.val1 = '01' then do:
        ...
        */
        ...
        if v-maxpr_old > 14 then kdlonkl.rating = 0.
    end.
    find current kdlonkl no-lock.

    /* 4. количество пролонгаций */
    /*
    find first kdlonkl where kdlonkl.bank = s-ourbank and kdlonkl.kdcif = txb.lon.cif and kdlonkl.kdlon = txb.lon.lon and kdlonkl.rdt = g-today and kdlonkl.kod = 'long' exclusive-lock no-error.
    if not avail kdlonkl then do:
        create kdlonkl.
        assign kdlonkl.bank = s-ourbank
               kdlonkl.kdcif = txb.lon.cif
    ...
    if avail bookcod then assign kdlonkl.valdesc = bookcod.name
                                 kdlonkl.rating = decimal(kdlonkl.val1) * deci(trim(bookcod.info[1])).
    find current kdlonkl no-lock.
    */
    /* 1. финансовое состояние */
    find first kdlonkl where kdlonkl.bank = s-ourbank and kdlonkl.kdcif = txb.lon.cif and kdlonkl.kdlon = txb.lon.lon and kdlonkl.rdt = g-today and kdlonkl.kod = 'finsost1' exclusive-lock no-error.
    if not avail kdlonkl then do:
        create kdlonkl.
        assign kdlonkl.bank = s-ourbank
               kdlonkl.kdcif = txb.lon.cif
               kdlonkl.kdlon = txb.lon.lon
               kdlonkl.rdt = g-today
               kdlonkl.kod = 'finsost1'
               kdlonkl.who = g-ofc
               kdlonkl.whn = g-today.
    end.
    if v-pprc <= 30 then do:
        if txb.lon.crc = 1 then kdlonkl.val1 = '01'. /* стабильное */
        else kdlonkl.val1 = '02'. /* по валютным кредитам фин. состояние понижаем на 1 пункт */
    end.
    else
    if v-pprc >= 31 and v-pprc <= 90 then kdlonkl.val1 = '02'. /* удовл. фин. сост. */
    else
    if v-pprc >= 90 and v-pprc <= 180 then kdlonkl.val1 = '03'. /* неуд. фин. сост. */
    else kdlonkl.val1 = '05'. /* крит. фин. сост. */
    ...
    find first kdlonkl where kdlonkl.bank = s-ourbank and kdlonkl.kdcif = txb.lon.cif and kdlonkl.kdlon = txb.lon.lon and kdlonkl.rdt = g-today and kdlonkl.kod = 'obesp1' exclusive-lock no-error.
    if not avail kdlonkl then do:
        create kdlonkl.
        assign kdlonkl.bank = s-ourbank
               kdlonkl.kdcif = txb.lon.cif
    ...
    kdlonkl.val1 = '05'. /* без обеспечения */
    ...
    /*
    find first kdlonkl where kdlonkl.bank = s-ourbank and kdlonkl.kdcif = txb.lon.cif and kdlonkl.kdlon = txb.lon.lon and kdlonkl.rdt = g-today and kdlonkl.kod = 'prosr_ob1' exclusive-lock no-error.
    if not avail kdlonkl then do:
        create kdlonkl.
        assign kdlonkl.bank = s-ourbank
               kdlonkl.kdcif = txb.lon.cif
    ...
    */
    ...
    find first kdlonkl where kdlonkl.bank = s-ourbank and kdlonkl.kdcif = txb.lon.cif and kdlonkl.kdlon = txb.lon.lon and kdlonkl.rdt = g-today and kdlonkl.kod = 'ispakt' exclusive-lock no-error.
    if not avail kdlonkl then do:
        create kdlonkl.
        assign kdlonkl.bank = s-ourbank
               kdlonkl.kdcif = txb.lon.cif
    ...
    /*
    find first kdlonkl where kdlonkl.bank = s-ourbank and kdlonkl.kdcif = txb.lon.cif and kdlonkl.kdlon = txb.lon.lon and kdlonkl.rdt = g-today and kdlonkl.kod = 'spisob1' exclusive-lock no-error.
    if not avail kdlonkl then do:
        create kdlonkl.
        assign kdlonkl.bank = s-ourbank
               kdlonkl.kdcif = txb.lon.cif
    ...
    */
    ...
    find first kdlonkl where kdlonkl.bank = s-ourbank and kdlonkl.kdcif = txb.lon.cif and kdlonkl.kdlon = txb.lon.lon and kdlonkl.rdt = g-today and kdlonkl.kod = 'rait1' exclusive-lock no-error.
    if not avail kdlonkl then do:
        create kdlonkl.
        assign kdlonkl.bank = s-ourbank
               kdlonkl.kdcif = txb.lon.cif
    ...
    kdlonkl.val1 = '04'. /* Ниже рейтинга РК и без рейтинга */
    ...
    for each kdlonkl where kdlonkl.bank = s-ourbank and kdlonkl.kdcif = txb.lon.cif and kdlonkl.kdlon = txb.lon.lon and kdlonkl.rdt = g-today no-lock:
        if lookup(kdlonkl.kod,kod_list) = 0 then next.
        v-rat = v-rat + kdlonkl.rating.
        if kdlonkl.kod = 'prosr' then v-prosr = kdlonkl.val1.
    end.

    find first kdlonkl where kdlonkl.bank = s-ourbank and kdlonkl.kdcif = txb.lon.cif and kdlonkl.kdlon = txb.lon.lon and kdlonkl.rdt = g-today and kdlonkl.kod = 'klass' exclusive-lock no-error.
    if not avail kdlonkl then do:
        create kdlonkl.
        assign kdlonkl.bank = s-ourbank
               kdlonkl.kdcif = txb.lon.cif
    ...
    if txb.lon.crc <> 1 and txb.lon.rdt >= 09/01/2009 then do:
        if v-rat <= 2 then kdlonkl.val1 = '03'.
    end.
    ...
    */
    ...
    if avail bookcod then kdlonkl.valdesc = bookcod.name.
    find current kdlonkl no-lock.

    wrk.dtcl = ?.
    find last kdlonkl where kdlonkl.bank = s-ourbank and kdlonkl.kdcif = txb.lon.cif and kdlonkl.kdlon = txb.lon.lon use-index bclrdt no-lock no-error.
    if avail kdlonkl then wrk.dtcl = kdlonkl.rdt.

    if wrk.dtcl <> ? then do:
        for each kdlonkl where kdlonkl.bank = s-ourbank and kdlonkl.kdcif = txb.lon.cif and kdlonkl.kdlon = txb.lon.lon and kdlonkl.rdt = wrk.dtcl no-lock:
            i = lookup(kdlonkl.kod,kod_list).
            if i = 0 then next.
            assign wrk.sts[i] = kdlonkl.val1
                   wrk.sts_des[i] = kdlonkl.valdesc
                   wrk.rating[i] = kdlonkl.rating.
        end.
        find first kdlonkl where kdlonkl.bank = s-ourbank and kdlonkl.kdcif = txb.lon.cif and kdlonkl.kdlon = txb.lon.lon and kdlonkl.rdt = wrk.dtcl and kdlonkl.kod = 'klass' no-lock no-error.
        if avail kdlonkl then do:
            assign wrk.rating_s = kdlonkl.rating
                   wrk.class = kdlonkl.val1.
