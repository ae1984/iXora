...
find rem where rem.rem = s-rem no-lock no-error.
if available rem then do:
svc5 = rem.svc.
find crc where crc.crc = rem.crc1 no-lock no-error.

find tarif2 where tarif2.str5 = substring(rem.tcby,1,4) and tarif2.stat = "r" no-lock no-error.
...
find tarif2 where tarif2.str5 = substring(brem.tcby,1,4) and tarif2.stat = "r" no-lock no-error.
if available tarif2 then v-pakal = tarif2.pakalp.
...
put "Komisija: "   svc5 " " crc.code "     " v-pakal format "x(33)" skip(1).
put
"------------------------------------------------------------------------------"
skip.
put skip(18).
