 ...
 * CHANGES
        ...
        08/12/2010 evseev - добавил вывод отчета с Переченем шифров объектов кодирования (с разбивкой) lntgt_1
...
def var bilance  as decimal format 'zzz,zzz,zzz,zz9.99' init 0.
...
def var balanspr as deci.

def shared variable v-dt  as date format "99/99/9999".
def shared variable v-d-cod like txb.sub-cod.d-cod.
...
def shared temp-table  wrk
    ...
    field balansprsr like txb.lon.opnamt
    ...
    field sub11  as char
    field sub71   like txb.sub-cod.ccode
...
for each txb.lon by txb.lon.gl:
  ...
  run lonbal_txb('lon',txb.lon.lon,v-dt,'1',yes,output summa). /* чистый ОД */
  balanspr = 0.
  run lonbal_txb('lon',txb.lon.lon,v-dt,'7,8',yes,output balanspr). /* просроченный ОД */

  if summa = 0 and balanspr = 0 then next.
  ...
  create wrk.
   ...
   find last txb.crchis where txb.crchis.crc = txb.lon.crc and txb.crchis.rdt le v-dt no-error.
    if avail txb.crchis then do.
       ...
       wrk.balansprsr = balanspr * txb.crchis.rate[1].
    end.
...
/*nataly*/
  ...
  find txb.sub-cod where txb.sub-cod.sub = 'LON' and txb.sub-cod.acc = txb.lon.lon and d-cod = v-d-cod no-lock no-error.
...
/*marinav*/
  ...
  wrk.sub71 = txb.sub-cod.ccode.
  find txb.sub-cod where txb.sub-cod.sub = 'cln' and txb.sub-cod.acc = txb.lon.cif and d-cod = 'ecdivis' no-lock no-error.
    ...
    /**/
  find txb.sub-cod where txb.sub-cod.sub = 'LON' and txb.sub-cod.acc = txb.lon.lon and d-cod = 'lnsrok' no-lock no-error.
     wrk.sub8 = txb.sub-cod.ccode.
     ...
     find first b-aaa where b-aaa.aaa = txb.aaa.craccnt  no-lock no-error.
      if available b-aaa then do.
         ...
         if avail txb.aab and txb.aab.bal <> 0 then do:
            find last txb.crchis where txb.crchis.crc = txb.aaa.crc and txb.crchis.rdt le v-dt no-error.
            create wrk.
              wrk.lon = txb.aaa.aaa.
              wrk.crc = txb.aaa.crc.
              wrk.gl = '1401'.
