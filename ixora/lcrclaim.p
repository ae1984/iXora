...
def new shared var s-event  like lcevent.event init 'rclaim'.
def new shared var s-number like lcevent.number.
def new shared var s-sts    like lcevent.sts.
def new shared var s-ftitle as char init ' REIMBURSEMENT CLAIM '.
...
{mainlc.i
 ...
 &prefind    = " assign v-find     = yes
              ...
              if avail lch and trim(lch.value1) <> '' then do:
                 ...
                 if avail lcamendh then assign v-lcsumcur = deci(lcamendh.value1)
                                               /*v-lcsumorg = deci(lcamendh.value1)*/.
              end.
 ...
 &postfind = " if avail LC then v-lcsts = LC.LCsts.
               ...
               do on error undo,return:
                   v-yes = no.
                   if v-lcsts = 'FIN' then
                   message 'Do you want to create a new Event (Reimbursement Claim)?' VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO TITLE ' QUESTION !' update v-yes.
                    ...
                    if v-yes then do transaction:
                        ...
                        if avail lcevent then do:
                            if lcevent.sts <> 'FIN' then do:
                                message 'The status of last event (Reimbursement Claim number ' + string(lcevent.number) +  ') is not FIN, it is impossible create a new Event!' view-as alert-box error.
                                assign v-yes    = no
                                       s-sts    = lcevent.sts
