 ...
 * DESCRIPTION
        Назначение программы, описание процедур и функций - Провайдеры, по которым необходим выбор типа платежа
 * RUN
 ...
 * AUTHOR
        26/03/09 id00205
 * CHANGES
        07.11.2012 damir - It's changes copying version compay2.p.
        07.11.2012 damir - Внедрено Т.З. № 1365,1481,1538.
        14.11.2012 damir - Добавление к изменению 07.11.2012. Контроль отправки платежей (вручную или автоматически).
...
/*Провайдеры - Нурсат,ШыгысЭнергоТрейд и т.д.*/
...
DEFINE FRAME MainFrame
        ...
        "_____________________________________________________________________________" skip(1)
         ...
         typeplat as character FORMAT "x(2)" label "Тип платежа        " validate(Find_First('TYP',typeplat),v-errmess)
         help "<F2> - выбор типа платежа" space(25)
         codereg as character FORMAT "x(3)" label "Регион" validate(Find_First('REG',codereg),v-errmess)
         help "<F2> - выбор региона" skip(1)
...
/***********************************************************************************************************/
on help of typeplat in FRAME MainFrame do:
    {itemlist.i
       &file = "t-Type"
       &frame = "row 6 centered scroll 1 20 down overlay "
       &where = " t-Type.choise = 'TYP' "
       &flddisp = " t-Type.code label 'Тип' format 'x(2)'
                    t-Type.name label 'Название' format 'x(50)'
                  "
       &chkey = "code"
       &chtype = "string"
       &index  = "idx"
    }
    typeplat = t-Type.code.
    displ typeplat with FRAME MainFrame.
end.

on help of codereg in FRAME MainFrame do:
    {itemlist.i
       &file = "t-Type"
       &frame = "row 6 centered scroll 1 20 down overlay "
       &where = " t-Type.choise = 'REG' "
       &flddisp = " t-Type.code label 'Код' format 'x(3)'
                    t-Type.name label 'Регион' format 'x(25)'
                  "
       &chkey = "code"
       &chtype = "string"
       &index  = "idx"
    ...
    codereg = t-Type.code.
    displ codereg with FRAME MainFrame.
end.
...
/***********************************************************************************************************/
      ...
      REPEAT on  ENDKEY UNDO  , leave :
        ...
        CASE pos:
          ...
          WHEN 1 THEN
          DO:
               set typeplat with frame MainFrame.
               if typeplat entered then do:

                 if Doc:ap_code = 530 then set codereg with frame MainFrame.
                 ...
                 do:
                   ...
                   do: /*Наличие онлайн проверки по авангард плат*/
                     ...
                     Usr:type_pay = typeplat.
                     Usr:code_reg = codereg.

                     run ap_check( Usr ,output err, output errdes).
                     if err <> 0 then do: message errdes view-as alert-box. undo. end.

                     if Doc:ap_code = 530 or Doc:ap_code = 536 then do: /*Шыгысэнерготрейд,Нурсат*/
                         ...
                         Doc:type_pay = trim(typeplat).
                         Doc:code_reg = trim(codereg).
                      pos = 3.
                     end.
                     else do:
          ...
          DO:
                   ...
                   if summ > 0 and summ >= Doc:minsum then SMess = "                [F1]-выполнить транзакцию, [F4]-отмена".
                   else SMess =  "                           [F4]-отмена".
...
procedure ShowFrame:
           ...
           typeplat  = Doc:type_pay.
           codereg   = Doc:code_reg.
           ...
           DISPLAY docNo docJH suppname  knp
                       ...
                       payacc summ State
                       comm_summ summall SMess typeplat codereg WITH  FRAME MainFrame.
