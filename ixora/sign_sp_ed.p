...
def shared var s-sp as integer.
...
v-who = "sp_" + string(s-sp) + "_" + string(t-faces.code).
...
do i = 1 to num-entries(spr_list):
    ...
    find first codfr where codfr.codfr = entry(i,spr_list) and codfr.code = v-who no-lock no-error.
...
on choose of bsave in frame ft do:
    for each b-data no-lock:
        find first codfr where codfr.codfr = b-data.dcode and codfr.code = v-who exclusive-lock no-error.
        if not avail codfr then do:
            create codfr.
