...
procedure AddRecord :
  ...
  if param1 <> "" then do:
      ...
      if avail xml_det then v-str = xml_det.val.
      put unformatted "<TR>" skip "<TD width=20>"  "</TD>" skip "<TD>" replace(v-str,";","; ") "</TD>" skip.
      v-str = "".
      find first xml_det where xml_det.xml_id = xml_id and xml_det.par matches param1 + " value" no-lock no-error.
      if avail xml_det then v-str = xml_det.val.
      put unformatted "<TD>" replace(v-str,";","; ") "</TD>" skip.
  end. else do:
  ...
  if param2 <> "" then do:
      ...
      if avail xml_det then v-str = xml_det.val.
      put unformatted "<TD width=20>"  "</TD>" skip "<TD>" replace(v-str,";","; ") "</TD>" skip.
      v-str = "".
      find first xml_det where xml_det.xml_id = xml_id and xml_det.par matches param2 + " value" no-lock no-error.
      if avail xml_det then v-str = xml_det.val.
      put unformatted "<TD>" replace(v-str,";","; ") "</TD>" skip "</TR>" skip.
  end. else do:
...
procedure AddRecordColor :
  ...
  if param1 <> "" then do:
      ...
      find first xml_det where xml_det.xml_id = xml_id and xml_det.par matches param1 + " title" no-lock no-error.
      if avail xml_det then v-str = xml_det.val.
      put unformatted "<TR bgcolor=#D8D8D8>" skip "<TD width=20>"  "</TD>" skip "<TD>" replace(v-str,";","; ") "</TD>" skip.
      v-str = "".
      find first xml_det where xml_det.xml_id = xml_id and xml_det.par matches param1 + " value" no-lock no-error.
      if avail xml_det then v-str = xml_det.val.
      put unformatted "<TD>" replace(v-str,";","; ") "</TD>" skip.
...
procedure AddRecord2 :
  ...
  if param1 <> "" then do:
      ...
      if avail xml_det then v-str = xml_det.val.
      put unformatted "<TR>" skip "<TD width=20>"  "</TD>" skip "<TD>" v-str "</TD>" skip.
      v-str = "".
      ...
      if avail xml_det then v-str = xml_det.val.
      put unformatted "<TD>" v-str "</TD>" skip.
  end. else do:
      put unformatted "<TR>" skip "<TD width=20>"  "</TD>" skip "<TD>"  "</TD>" skip "<TD>" "</TD>" skip.
  end.
  if param2 <> "" then do:
      ...
      if avail xml_det then v-str = xml_det.val.
      put unformatted "<TD width=20>"  "</TD>" skip "<TD>" v-str "</TD>" skip.
      v-str = "".
      ...
      if avail xml_det then v-str = xml_det.val.
      put unformatted "<TD>" v-str "</TD>" skip "</TR>" skip.
  end. else do:
      put unformatted "<TD width=20>"  "</TD>" skip "<TD>"  "</TD>" skip "<TD>"  "</TD>" skip "</TR>" skip.
  end.
end procedure.
...
procedure AddRecord2Color :
  ...
  if param1 <> "" then do:
      ...
      find first xml_det where xml_det.xml_id = xml_id and xml_det.line >= st and xml_det.line <= en and xml_det.par matches param1 + " title" no-lock no-error.
      if avail xml_det then v-str = xml_det.val.
      put unformatted "<TR bgcolor=#D8D8D8 >" skip "<TD width=20>"  "</TD>" skip "<TD>" v-str "</TD>" skip.
      v-str = "".
      find first xml_det where xml_det.xml_id = xml_id and xml_det.line >= st and xml_det.line <= en and xml_det.par matches param1 + " value" no-lock no-error.
      if avail xml_det then v-str = xml_det.val.
      put unformatted "<TD>" v-str "</TD>" skip.
...
do i = 1 to v-count:
    ...
    put unformatted "<TR><TD colspan=6 bgcolor=#D8D8D8> " + xml_det.val + " </TD></TR>" skip.
...
do i = 1 to v-count:
   ...
   put unformatted "<TR bgcolor=#D8D8D8><TD colspan=3> Текущие </TD><TD colspan=2> Последняя дата записи в систему </TD><TD> " + xml_det.val + " </TD></TR>" skip.
...
if v-count <> 0 then do:
    ...
    do i = 1 to v-count:
        find first xml_det where xml_det.xml_id = xml_id and xml_det.line >= vstart[i] and xml_det.line <= vend[i]
               and xml_det.par matches "*NegativeData NegativeStatus NegativeStatusOfContract RegistrationDate value" no-lock no-error.
       put unformatted "<TR bgcolor=#D8D8D8><TD colspan=3> Исторический </TD><TD colspan=2> Последняя дата записи в систему </TD><TD> " + xml_det.val + " </TD></TR>" skip.
       run AddRecord2("*NegativeData NegativeStatus NegativeStatusOfContract NegativeStatusOfContract","",vstart[i],vend[i]).
    end.
end.
...
do i = 1 to v-count:
   ...
   for each t-xml_det where t-xml_det.par matches "*ExistingContracts Contract PaymentsCalendar Payment".
      find first b-xml where b-xml.line = t-xml_det.line + 1 no-error.
   ...
   put unformatted "<TR>" skip.
   for each t-xml_det where t-xml_det.par matches "*ExistingContracts Contract PaymentsCalendar Payment".
      find first b-xml where b-xml.line = t-xml_det.line + 2 no-error.
      put unformatted "<TD>" + b-xml.val +  "</TD>" skip.
   end.
   put unformatted "</TR>" skip.
...
put unformatted "<TR bgcolor=gray><TD colspan=4 height = 20 > КОНЕЦ КРЕДИТНОГО ОТЧЕТА</TD><TD>Дата формирования кредитного отчета</TD><TD>" + xml_det.val + "</TD></TR>" skip.

find first xml_det where xml_det.xml_id = xml_id and xml_det.par matches "*Footer ContactInfo value" no-lock no-error.
put unformatted "<TR><TD colspan=6 bgcolor=gray> " + xml_det.val + " </TD></TR>" skip.
