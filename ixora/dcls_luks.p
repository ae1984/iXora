...
fname1 = "acrluks" + substring(string(g-today),1,2) + substring(string(g-today),4,2) + ".txt".
...
for each lgr where lgr.led = "TDA" and lgr.feensf = 3 :
...
/*if aaa.aaa <> "199759872" then next account. */
        ...
        if lgr.intcal = "D" then do:
           ...
           if ev-date <> ? then do:
             ...
             if lookup(aaa.lgr,"A34,A35,A36") <> 0 then do: /*Если депозит пенсионный*/
             ...
             if lookup(aaa.lgr,"A34,A35,A36") <> 0 then do: /*Если депозит пенсионный*/
                 if aaa.regdt < 04.01.2010 then do: /*условие согласно тз-685 */
                    ev-date = EventInRange(lgr.prefix, g-today, s-target - 1). /*изменил Евсеев*/
                 end.
                 else
                 do:
