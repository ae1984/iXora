 ...
 * CALLER
        Список процедур, вызывающих этот файл - vcrepdex.p
 * SCRIPT
...
def var v-filename as char init "vcrepdex.htm".
def var v-title as char.

v-title = "КОНСИГНАЦИИ<BR>сумма ГТД превышает сумму проплат<BR>Приложение 14, строки 3, 14 по экспорту<br>(ЭКСПОРТ)".

def stream vcrpt.
output stream vcrpt to value(v-filename).

{html-title.i &stream = "stream vcrpt" &title = "Задолжники по консигнации"}
