 ...
 * MODULE
        Trade Finance
 * DESCRIPTION
        EXLC - Advice of Discrepancy (MT750)
 * RUN
...
def new shared var s-event    like lcevent.event init 'discr'.
def new shared var s-number   like lcevent.number.
def new shared var s-sts      like lcevent.sts.
def new shared var s-ftitle   as char init ' ADVICE of DISCREPANCY '.
...
{mainlc.i
 ...
 &postfind = " if avail LC then v-lcsts = LC.LCsts.
               ...
               do on error undo,return:
                   message 'Do you want to create a new Event (Advice of Discrepancy)?' VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO TITLE ' QUESTION !' update v-yes.
                    ...
                    if v-yes then do transaction:
                        ...
                        if avail lcevent then do:
                            if lcevent.sts <> 'FIN' then do:
                                message 'The status of last event (Advice of Discrepancy number ' + string(lcevent.number) +  ') is not FIN, it is impossible create a new Event!' view-as alert-box error.
                                assign v-yes    = no
                                       s-sts    = lcevent.sts
