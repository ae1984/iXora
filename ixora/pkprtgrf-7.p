...
if not avail lon then do:
    if v-inet then run fmsg-w(pkanketa.bank,pkanketa.credtype,pkanketa.ln,"pkprtgrf-7 - Ссудный счет N " + pkanketa.lon + " не найден!").
...
if avail pksysc then v-comved = pkanketa.summa * pksysc.deval / 100. else v-comved = pkanketa.summa * 0.5 / 100.
*/
/*if avail pksysc then v-comved = pkanketa.summa * (pksysc.deval / 2 + pksysc.deval) / 100. else v-comved = pkanketa.summa * 0.8 / 100.*/
...
for each wrk.
    if wrk.com then do:
         ...
         put stream m-out unformatted "<tr align=""right"" style=""font-size:x-small"">"
                   ...
                   "<td> " wrk.proc format '>>>,>>>,>>9.99' "</td>" skip
                   "<td> </td>" skip
                   "<td> " wrk.od + wrk.proc format '>>>,>>>,>>9.99' "</td></tr>" skip.
    end.
end.
