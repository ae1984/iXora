 ...
 * MODULE
        Кредитный модуль
 * DESCRIPTION
       Долг процентов на любую дату по 2 уролвню (без просрочки %) 
 * RUN
 ...
 * AUTHOR
        26/09/2003 marinav
 * CHANGES
        31/05/2005 madiyar - подправил для 5 схемы
        11/03/2007 madiyar - подправил для 4 схемы
*/
...
for each lonres where lonres.lon = p-lon and lonres.lev = 2 no-lock:
    if lonres.dc = "d" and lonres.rem <> "" then do:
        ...
        do i = 0 to v-cnt:
            ...
            if i = v-cnt then do:
                if v-intbal <> lonres.amt then wt.amt = wt.amt + (lonres.amt - v-intbal).
            end.
        end.
    end.
end.
