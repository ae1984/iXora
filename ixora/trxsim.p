 ...
 * CHANGES
       ...
       05.07.2005 suchkov - исправил косяк с кассой в пути
       08/04/08 marinav - crc.rate[5] -> crc.rate[3], crc.rate[4]  -> crc.rate[2]
       06/01/2011  Luiza добавила параметр v-dochid (номер документа ) для создания записи в табл docl (проводки для акцептования).
       25/02/2011 madiyar - перенес процедуру trxglacc из trxgen0
       ...
       28/10/2013 Luiza  - ТЗ 1937 для нулевых сумм исключаем линии провод
*/

{global.i}

def input parameter v-dochid as char.
...
/* create record for docl*/

if v-dochid <> "" then do:
    find first docl EXCLUSIVE-LOCK no-error.
    for each tmpl no-lock where tmpl.amt > 0.
        create docl.
        docl.docid = substring(v-dochid,1,9).
        docl.gl = tmpl.drgl.
        docl.sub = tmpl.drsub.
        docl.acc = tmpl.dracc.
        docl.crc = tmpl.crc.
        ...
        docl.cam = 0.
        do k = 1 to 5:
            docl.rem[k] = tmpl.rem[k].
        end.
        docl.cods = substring(v-dochid,11,9).
        create docl.
        docl.docid = substring(v-dochid,1,9).
        docl.gl = tmpl.crgl.
        docl.sub = tmpl.crsub.
   ...
   find current docl no-lock no-error.
end. /*end create record for docl */
...
procedure rateauto.
    ...
    if tmpl.rate-f = "a" then do:
        find crc where crc.crc = tmpl.crc no-lock.
        if isConvGL(tmpl.crgl)  /* tmpl.crgl = buy.inval*/ then do:
            if tmpl.drgl = cas.inval or tmpl.drgl = 100200 or tmpl.drgl = 100500 then do:
        ...
        if isConvGL(tmpl.drgl)  /* tmpl.drgl = sel.inval*/ then do:
            if tmpl.crgl = cas.inval or tmpl.drgl = 100200 or tmpl.drgl = 100500 then do:
    ...
    if vsub = "arp" then do:
        ...
        if available arp then do:
            if vlev = 1 then vgl = arp.gl.
                ...
                find trxlevgl where trxlevgl.gl = arp.gl and trxlevgl.level = vlev no-lock no-error.
                if available trxlevgl then vgl = trxlevgl.glr.
                else do:
                    rcode = 28.
                    rdes = errlist[rcode] + " Субсчет = " + vsub + "; уровень = " + string(vlev,"z9") + "; Г/К(1) = " + string(arp.gl,"999999") + ".".
                    return.
                end.
            end.
        end.
            ...
            rcode = 20.
            rdes = trim(errlist[rcode]) + ":Субсчет=" + vsub + ";счет=" + vacc + "Линия=" + string(tmpl.ln,"99") + ";Г/К <- счет.".
            return.
        end.
    end.
    else
    if vsub = "ast" then do:
        ...
        if available ast then do:
            if vlev = 1 then vgl = ast.gl.
                ...
                find trxlevgl where trxlevgl.gl = ast.gl and trxlevgl.level = vlev no-lock no-error.
                if available trxlevgl then vgl = trxlevgl.glr.
                else do:
                    rcode = 28.
                    rdes = errlist[rcode] + " Субсчет = " + vsub + "; уровень = " + string(vlev,"z9") + "; Г/К(1) = " + string(ast.gl,"999999") + ".".          return.
                end.
            end.
        end.
        else do:
        ...
        if available aaa then do:
            if vlev = 1 then vgl = aaa.gl.
                ...
                if available trxlevgl then vgl = trxlevgl.glr.
                    ...
                    rcode = 28.
                    rdes = errlist[rcode] + " Субсчет = " + vsub + "; уровень = " + string(vlev,"z9") + "; Г/К(1) = " + string(aaa.gl,"9999999") + ".".
                    return.
                end.
            end.
        end.
        ...
        if available dfb then do:
            if vlev = 1 then vgl = dfb.gl.
                ...
                if available trxlevgl then vgl = trxlevgl.glr.
                    ...
                    rcode = 28.
                    rdes = errlist[rcode] + " Субсчет = " + vsub + "; уровень = " + string(vlev,"z9") + "; Г/К(1) = " + string(dfb.gl,"999999") + ".".
                    return.
                end.
            end.
        end.
    ...
    if vsub = "eps" then do:
        ...
        if available eps then do:
            if vlev = 1 then vgl = eps.gl.
                ...
                find trxlevgl where trxlevgl.gl = eps.gl and trxlevgl.level = vlev no-lock no-error.
                if available trxlevgl then vgl = trxlevgl.glr.
                else do:
                    rcode = 28.
                    rdes = errlist[rcode] + " Субсчет = " + vsub + "; уровень = " + string(vlev,"z9") + "; Г/К(1) = " + string(eps.gl,"999999") + ".".         return.
                end.
            end.
        end.
        else do:
    ...
    if vsub = "fun" then do:
        ...
        if available fun then do:
            if vlev = 1 then vgl = fun.gl.
                ...
                find trxlevgl where trxlevgl.gl = fun.gl and trxlevgl.level = vlev no-lock no-error.
                if available trxlevgl then vgl = trxlevgl.glr.
                else do:
                    rcode = 28.
                    rdes = errlist[rcode] + " Субсчет = " + vsub + "; уровень = " + string(vlev,"z9") + "; Г/К(1) = " + string(fun.gl,"999999") + ".".      return.
                end.
            end.
        end.
        else do:
    ...
    if vsub = "scu" then do:
        ...
        if available scu then do:
            if vlev = 1 then vgl = scu.gl.
                ...
                find trxlevgl where trxlevgl.gl = scu.gl and trxlevgl.level = vlev no-lock no-error.
                if available trxlevgl then vgl = trxlevgl.glr.
                else do:
                    rcode = 28.
                    rdes = errlist[rcode] + " Субсчет = " + vsub + "; уровень = " + string(vlev,"z9") + "; Г/К(1) = " + string(scu.gl,"999999") + ".".      return.
                end.
            end.
        end.
        else do:
    ...
    if vsub = "tsf" then do:
        ...
        if available tsf then do:
            if vlev = 1 then vgl = tsf.gl.
                ...
                find trxlevgl where trxlevgl.gl = tsf.gl and trxlevgl.level = vlev no-lock no-error.
                if available trxlevgl then vgl = trxlevgl.glr.
                else do:
                    rcode = 28.
                    rdes = errlist[rcode] + " Субсчет = " + vsub + "; уровень = " + string(vlev,"z9") + "; Г/К(1) = " + string(tsf.gl,"999999") + ".".      return.
                end.
            end.
        end.
        else do:
    ...
    if vsub = "lcr" then do:
        ...
        if available lcr then do:
            if vlev = 1 then vgl = lcr.gl.
                ...
                find trxlevgl where trxlevgl.gl = lcr.gl and trxlevgl.level = vlev no-lock no-error.
                if available trxlevgl then vgl = trxlevgl.glr.
                else do:
                    rcode = 28.
                    rdes = errlist[rcode] + " Субсчет = " + vsub + "; уровень = " + string(vlev,"z9") + "; Г/К(1) = " + string(lcr.gl,"999999") + ".".         return.
                end.
            end.
        end.
        else do:
    ...
    if vsub = "lon" then do:
        ...
        if available lon then do:
            if vlev = 1 then vgl = lon.gl.
                ...
                find trxlevgl where trxlevgl.gl = lon.gl and trxlevgl.level = vlev no-lock no-error.
                if available trxlevgl then vgl = trxlevgl.glr.
                else do:
                    rcode = 28.
                    rdes = errlist[rcode] + " Субсчет = " + vsub + "; уровень = " + string(vlev,"z9") + "; Г/К(1) = " + string(lon.gl,"999999") + ".".            return.
                end.
            end.
        end.
        else do:
        ...
        if available ock then do:
            if vlev = 1 then vgl = ock.gl.
                ...
                if available trxlevgl then vgl = trxlevgl.glr.
                    ...
                    rcode = 28.
                    rdes = errlist[rcode] + " Субсчет = " + vsub + "; уровень = " + string(vlev,"z9") + "; Г/К(1) = " + string(ock.gl,"999999") + ".".      return.
                end.
            end.
        end.
        else do:
