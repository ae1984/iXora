 ...
 * AUTHOR
        11.03.2004 marinav 
 * CHANGES
        04/06/2004 - добавил возможность формирования отчета задним числом
*/
...
{global.i}
{lonlev.i}
def input param v-cif like cif.cif.
def input param v-indat as date.
...
if avail zllon then do:
   put stream s1 'Учредители:             ' zllon.stholder[1] format "x(60)" skip.
   put stream s1 '                        ' zllon.stholder[2] format "x(60)" skip.
   put stream s1 '                        ' zllon.stholder[3] format "x(60)" skip.
   put stream s1 '                        ' zllon.stholder[4] format "x(60)" skip.
   put stream s1 '                        ' zllon.stholder[5] format "x(60)" skip.
   put stream s1 '                        ' zllon.stholder[6] format "x(60)" skip.
   put stream s1 '                        ' zllon.stholder[7] format "x(60)" skip.
end. 

put stream s1 'Обслуживает:            ' cif.fname ''skip.
put stream s1 skip(1).
put stream s1 '--------------------------------------------------------------------------------------' skip.
put stream s1 '                               Обороты по счетам клиента ' skip.
put stream s1 '--------------------------------------------------------------------------------------' skip.
put stream s1 '  Год         Расчет.счета        Расчет.счета      Ссудные счета      Ссудные счета ' skip.
put stream s1 '                 в KZT               в USD              в KZT              в USD     'skip.
put stream s1 '--------------------------------------------------------------------------------------' skip.

repeat god = 1999 to year(v-indat) by 1:   
...
/* 01.01.2004 nadejda - изменила расчет даты на 3 месяца назад с учетом количества дней в прошлых месяцах */
v-day = day(v-indat).
v-mc = month(v-indat) - 3.
v-year = year(v-indat).
...
run mondays(v-mc, v-year, output mdays).
if v-day > mdays then v-day = mdays.

datums = date(v-mc, v-day, v-year).


  cnt[1] = 0. cnt[2] = 0. cnt[3] = 0. cnt[4] = 0.  
  for each aaa where aaa.cif = v-cif and not aaa.lgr begins '5' no-lock.
      for each jl where jl.acc = aaa.aaa  and jl.jdt >= datums and jl.jdt <= v-indat and jl.crc = 1 and (jl.lev = 1 or jl.lev = 2) no-lock.
        accumulate jl.cam (TOTAL).
      end.
      cnt[1] = cnt[1] + accum total jl.cam.
      for each jl where jl.acc = aaa.aaa  and jl.jdt >= datums and jl.jdt <= v-indat and  jl.crc = 2 and (jl.lev = 1 or jl.lev = 2) no-lock.
        accumulate jl.cam (TOTAL).
      end.
      cnt[2] = cnt[2] + accum total jl.cam.
  end.
  for each lon where lon.cif = v-cif no-lock.
      for each jl where jl.acc = lon.lon  and jl.jdt >= datums and jl.jdt <= v-indat and jl.crc = 1 and (jl.lev = 1 or jl.lev = 2) no-lock.
        accumulate jl.dam (TOTAL).
      end.
      cnt[3] = cnt[3] + accum total jl.dam.
      for each jl where jl.acc = lon.lon  and jl.jdt >= datums and jl.jdt <= v-indat and jl.crc = 2 and (jl.lev = 1 or jl.lev = 2) no-lock.
        accumulate jl.dam (TOTAL).
      end.
      cnt[4] = cnt[4] + accum total jl.dam.
  end.
...
/***************/
datums = date(string(day(v-indat)) + "/" + string(month(v-indat)) + "/" + string(year(v-indat) - 1)).

put stream s1 '------------------------------------------------------------------------' skip.
put stream s1 '         Доходность клиента с ' datums ' по ' v-indat skip.
put stream s1 '------------------------------------------------------------------------' skip.
put stream s1 '  Счет             Наименование                             Сумма' skip.
put stream s1 '------------------------------------------------------------------------' skip.

  for each aaa where aaa.cif = v-cif no-lock.
    for each jl where jl.acc = aaa.aaa and jl.lev = 1 and  jdt >= datums and jdt <= v-indat no-lock. 
  ...
  summa = 0.
  put stream s1 skip(1).
  put stream s1 'От кредитных операций' skip.
  for each temp_jl. delete temp_jl. end.
  for each temp_jl1. delete temp_jl1. end.

  for each lon where lon.cif = v-cif no-lock.
    for each jl where jl.acc = lon.lon and jl.lev = 2 and jdt >= datums and jdt <= v-indat no-lock. 
...
/**************/
put stream s1 '   КРЕДИТНАЯ ИСТОРИЯ КЛИЕНТА' skip(2).


    for each lon where lon.cif = v-cif break by lon.cif by lon.crc:
    run atl-dat (lon.lon,v-indat,output bilance). /* остаток  ОД*/                        
    if bilance > 0 then next.

    if first-of (lon.cif) then
    do:
      put stream s1 'Погашенные кредиты' skip(1).
      put stream s1 '------------------------------------------------------------------------------------------------------------' skip.
      put stream s1 'Нпп   Кредит  Грп.   Дата     срок          Сумма      Вал.    %       Вид     Код     Валюта      Сумма  ' skip.
      put stream s1 '                    регист. погашен        кредита          ставка   кредита                               ' skip.                          
      put stream s1 '                    кредит  кредита                                           залога   залога      залога  ' skip.
      put stream s1 '                                                                                                             ' skip.
      put stream s1 '------------------------------------------------------------------------------------------------------------ ' skip.
    end.
...
put stream s1 'Действующие кредиты' skip(1).

for each lon where lon.cif = v-cif break by lon.crc by lon.cif:
    run atl-dat (lon.lon,v-indat,output bilance). /* остаток  ОД*/                        
    if bilance <= 0 then next.
put stream s1 '------------------------------------------------------------------------------------------------------------------------------------' skip.
put stream s1 'Нпп   Кредит     Дата     срок          Сумма      Вал.    %            Задолженность                 Факт. погашено          Вид   ' skip.
put stream s1 '                регист. погашен        кредита           ставка ------------------------------------------------------------- кред. ' skip.                          
put stream s1 '                кредит  кредита                                    осн. долга          %%          осн. долга         %%            ' skip.
put stream s1 '                                                                                                                                    ' skip.
put stream s1 '------------------------------------------------------------------------------------------------------------------------------------' skip.
        ...
        run atl-dat(lon.lon,v-indat,output bilance). /* остаток  ОД*/                        
        run atl-prcl(lon.lon,v-indat - 1, output vint, output vint1, output vint1).  /* остаток % */
...
for each zldog where zldog.lon = lon.lon break by zldog.lon.
   sumzal = 0.
    ...
    put stream s1 v-cntz format '>9' skip.
    put stream s1 " Номер договора залога:  " zldog.nomz skip.
    put stream s1 " Дата договора залога :  " zldog.regdt skip.          
    put stream s1 " Залогодатель         :  " zldog.zal_name skip.
    put stream s1 " Руководитель зал-ля  :  " zldog.chief_zal skip.
    put stream s1 " Учредитель зал-ля    :  " zldog.stholder_zal[1] format "x(60)" skip.
    put stream s1 "                         " zldog.stholder_zal[2] format "x(60)" skip.
    put stream s1 "                         " zldog.stholder_zal[3] format "x(60)" skip.
    put stream s1 "                         " zldog.stholder_zal[4] format "x(60)" skip.
    put stream s1 "                         " zldog.stholder_zal[5] format "x(60)" skip.
    put stream s1 "                         " zldog.stholder_zal[6] format "x(60)" skip.
    put stream s1 "                         " zldog.stholder_zal[7] format "x(60)" skip.
    if zldog.longdt[1] <> ? then 
         put stream s1 "Пролонгация              " zldog.longdt[1] skip(1).
    v-cnt = 1.
    v-cntz = v-cntz + 1.
...
for each zlzalog where zlzalog.lon = lon.lon and zlzalog.nomz = zldog.nomz break by zlzalog.lon.
  ...
  do:
   put stream s1 "-----------------------------------------------------------------------------------------------------------------------------------------------------------------"  skip.
   put stream s1 "п/п|                      Предмет залога                                             |                      Документы                     | Вал.|      Сумма      " skip.
   put stream s1 "-----------------------------------------------------------------------------------------------------------------------------------------------------------------"  skip.
  end.
...
v-cnt = v-cnt + 1.
      ...
      if zlzalog.crc = 1 then do:
            find last crchis where crchis.crc = 2 and crchis.regdt le v-indat no-lock no-error.
      ...
      if zlzalog.crc = 11 then do:
            find last crchis where crchis.crc = 11 and crchis.regdt le v-indat no-lock no-error.
...
if length(trim(des[3])) > 0 then 
   put stream s1 '   |' des[3] format "x(80)" ' | '.
   else put stream s1 '   |' space(80) ' | '.
if length(trim(docs[3])) > 0 then 
   put stream s1 docs[3] format "x(50)" ' | ' skip.
   else put stream s1 space(50) ' | ' skip.

run rin-dal(input-output gal,output des[4],input 80).
run rin-dal(input-output gal1,output docs[4],input 50).
if length(trim(des[4])) > 0 then 
   put stream s1 '   |' des[4] format "x(80)" ' | '.
   else put stream s1 '   |' space(80) ' | '.
if length(trim(docs[4])) > 0 then 
   put stream s1 docs[4] format "x(50)" ' | ' skip.
   else put stream s1 space(50) ' | ' skip.

run rin-dal(input-output gal,output des[5],input 80).
run rin-dal(input-output gal1,output docs[5],input 50).
if length(trim(des[5])) > 0 then 
   put stream s1 '   |' des[5] format "x(80)" ' | '.
   else put stream s1 '   |' space(80) ' | '.
if length(trim(docs[5])) > 0 then 
   put stream s1 docs[5] format "x(50)" ' | ' skip.
   else put stream s1 space(50) ' | ' skip.

run rin-dal(input-output gal,output des[6],input 80).
run rin-dal(input-output gal1,output docs[6],input 50).
if length(trim(des[6])) > 0 then 
   put stream s1 '   |' des[6] format "x(80)" ' | '.
   else put stream s1 '   |' space(80) ' | '.
if length(trim(docs[6])) > 0 then 
   put stream s1 docs[6] format "x(50)" ' | ' skip.
   else put stream s1 space(50) ' | ' skip.

run rin-dal(input-output gal,output des[7],input 80).
run rin-dal(input-output gal1,output docs[7],input 50).
if length(trim(des[7])) > 0 then 
   put stream s1 '   |' des[7] format "x(80)" ' | '.
   else put stream s1 '   |' space(80) ' | '.
if length(trim(docs[7])) > 0 then 
   put stream s1 docs[7] format "x(50)" ' | ' skip.
   else put stream s1 space(50) ' | ' skip.

run rin-dal(input-output gal,output des[8],input 80).
if length(trim(des[8])) > 0 then 
   put stream s1 '   |' des[8] format "x(80)" ' | ' skip.

run rin-dal(input-output gal,output des[9],input 80).
if length(trim(des[9])) > 0 then 
   put stream s1 '   |' des[9] format "x(80)" ' | ' skip.

run rin-dal(input-output gal,output des[10],input 80).
if length(trim(des[10])) > 0 then 
   put stream s1 '   |' des[10] format "x(80)" ' | ' skip.

run rin-dal(input-output gal,output des[11],input 80).
if length(trim(des[11])) > 0 then 
   put stream s1 '   |' des[11] format "x(80)" ' | ' skip.

run rin-dal(input-output gal,output des[12],input 80).
if length(trim(des[12])) > 0 then 
   put stream s1 '   |' des[12] format "x(80)" ' | ' skip.

run rin-dal(input-output gal,output des[13],input 80).
if length(trim(des[13])) > 0 then 
   put stream s1 '   |' des[13] format "x(80)" ' | ' skip.

run rin-dal(input-output gal,output des[14],input 80).
if length(trim(des[14])) > 0 then 
   put stream s1 '   |' des[14] format "x(80)" ' | ' skip.

run rin-dal(input-output gal,output des[15],input 80).
if length(trim(des[15])) > 0 then 
   put stream s1 '   |' des[15] format "x(80)" ' | ' skip.

run rin-dal(input-output gal,output des[16],input 80).
if length(trim(des[16])) > 0 then 
   put stream s1 '   |' des[16] format "x(80)" ' | ' skip.

run rin-dal(input-output gal,output des[17],input 80).
if length(trim(des[17])) > 0 then 
   put stream s1 '   |' des[17] format "x(80)" ' | ' skip.

run rin-dal(input-output gal,output des[18],input 80).
if length(trim(des[18])) > 0 then 
   put stream s1 '   |' des[18] format "x(80)" ' | ' skip.

run rin-dal(input-output gal,output des[19],input 80).
if length(trim(des[19])) > 0 then 
   put stream s1 '   |' des[19] format "x(80)" ' | ' skip.
  put stream s1 "----------------------------------------------------------------------------------------------------------------------------------------------------------------"  skip.
  put stream s1 skip(1).
end.     
put stream s1 "ИТОГО в долл США" space(130) sumzal format ">>>,>>>,>>>.99" skip.
put stream s1 skip(1).
...
/*****************/
                ...
                if v-indat = g-today then do:
                  ...
                  run atl-dat (input lon.lon, input v-indat, output vcu[1]).
                  run atl-dat (input lon.lon, input date(month(v-indat),1,year(v-indat)) - 1, output vmost[1]).
                  run atl-dat (input lon.lon, input date(1,1,year(v-indat)) - 1, output vyrst[1]).
                end.
                
                run atl-prcl(input lon.lon, input date(month(v-indat),1,year(v-indat)) - 1, output vmost[3], output vmost[4], output vmost[2]).
                
                run atl-prcl(input lon.lon, input date(1,1,year(v-indat)) - 1, output vyrst[3], output vyrst[4], output vyrst[2]).
                
                
                run atl-prcl(input lon.lon, input v-indat, output vcu[3], output vcu[4], output vcu[2]).
                
                
                run atl-prov(input lon.lon, input date(month(v-indat),1,year(v-indat)) - 1, output vmost[3]).
                
                run atl-prov(input lon.lon, input date(1,1,year(v-indat)) - 1, output vyrst[3]).                                                               
                
                
                run atl-prov(input lon.lon, input v-indat, output vcu[3]).                
                ...
                vcu[4]   format ">>>,>>>,>>>.99-" skip(3).                                             
                 ...
                 for each w-amk by w-amk.nr:
                     if w-amk.dt > v-indat then do: w-amk.dt = ?. w-amk.amt2 = 0. end.
...
*/
                 ...
                 for each w-amk where w-amk.fdt <= v-indat or w-amk.amt2 > 0 by w-amk.nr:
                     run atl-prcl(input lon.lon, input w-amk.fdt - 1,                                      
                                 output vcu[3], output vcu[4], output vcu[2]).                                   
                     w-amk.amt1 = vcu[3].  
                     if w-amk.fdt > v-indat and w-amk.amt2 > 0 then     
                        put stream s1 '                               '
                                      w-amk.dt 
                                      w-amk.amt2 skip.
                     else
      ...
      /*           find last w-amk where w-amk.amt2 > 0 no-lock no-error.
                 ...
                 find last w-amk where w-amk.fdt < v-indat no-lock no-error.
                 if avail w-amk then do:
                    f-dat1 = w-amk.fdt.
                    v-am2 = w-amk.amt1.
                 end.
...
/*Обороты по счету - основная сумма*/
      ...
      g1:
         for each lnscg where lnscg.lng = lon.lon and
             lnscg.f0 > - 1 and lnscg.fpn = 0 and lnscg.flp > 0 and lnscg.stdat <= v-indat
         ...
         for each lnsch where lnsch.lnn = lon.lon and lnsch.flp > 0 and lnsch.stdat <= v-indat
    ...
    find last bal_cif where bal_cif.cif = v-cif and bal_cif.nom begins 'a' and bal_cif.rdt <= v-indat
      ...
      define var w-lonp like bal_cif.amount extent 22.
      define var w-lond like bal_cif.amount extent 17.
      define var w-lonaold like bal_cif.amount extent 27.
      define var w-lonpold like bal_cif.amount extent 22.
      def var vk1 as deci.
      def var vk2 as deci.
      def var vk3 as deci.
      def var vk4 as deci.
...
/* Коэффициент оборачиваемости т.м.з.   */
...
put stream s1 'Коэффициент надежности ' sum1sum2 format '->>,>>9.99' skip.
end.

output stream s1 close.
/*run menu-prt('rpt.img').*/
unix silent cptwin rpt.img winword.
