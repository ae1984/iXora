 ...
 * MODULE
        Социальные платежи
 * DESCRIPTION
        Формирование реестра отправленных социальных платежей в Excel
 * RUN
...
"<TABLE width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" skip
  ...
  "<B>ПОЛУЧАТЕЛИ ПИСЕМ - ВОЗВРАТЫ СОЦИАЛЬНЫХ ПЛАТЕЖЕЙ</B><BR>" skip
...
do vd = v-d1 to v-d2:
   for each letters where letters.rdt = vd and letters.bank = ourbnk and letters.type = "pmprmz" no-lock use-index rdt:
       create tmp.
       buffer-copy letters to tmp.
   end.
   for each letters where letters.rdt = vd and letters.bank = ourbnk and letters.type = "pmpcas" no-lock use-index rdt:
       create tmp.
       buffer-copy letters to tmp.
   end.
end.
