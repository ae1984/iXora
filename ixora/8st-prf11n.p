...
do i =  4 to NUM-ENTRIES(s-gl[j]):
 ...
 if entry(i ,s-gl[j]) = 'd1t' then do:
  for each wf1 where trim(wf1.wkod) matches '*035*' . 
   temp.val =  temp.val + round(wf1.wsumpr / 1000, 0)  + round(wf1.wsumLs / 1000, 0)  .
  end.
 end.
 if entry(i ,s-gl[j]) = 'd2t' then do:
  for each wf2 where trim(wf2.wkod) matches '*035*' . 
 ...
 when '131' then do:
  ...
  temp.rem = entry(i ,s-gl[j]).
 if entry(i ,s-gl[j]) = 'd1t' then do:
  for each wf1 where trim(wf1.wkod) matches '*058*' . 
   temp.val =  temp.val + round(wf1.wsumpr / 1000, 0)  + round(wf1.wsumLs / 1000, 0)  .
  end.
 end.
 if entry(i ,s-gl[j]) = 'd2t' then do:
  for each wf2 where trim(wf2.wkod) matches '*058*' . 
   temp.val =  temp.val + round(wf2.wsumpr / 1000, 0)  + round(wf2.wsumLs / 1000, 0) .
  end.
 end.
 ...
 if not available 
      sthead then do: /*message 'Нет данных по отчету  7pn за '  vasof .  */ return. end. 
  ...
  if rez = '*' then do:
   for each txb.stdata where stdata.referid = sthead.referid
       ...
       substr(stdata.fun,1,4) matches entry(i ,s-gl[j])
         and   substr(stdata.fun,6,1) eq trim(priz) .
   ...
   for each txb.stdata where stdata.referid = sthead.referid
       ...
       substr(stdata.fun,1,4) matches entry(i ,s-gl[j])
         and   substr(stdata.fun,6,1) eq trim(priz) and
          substr(stdata.fun,5,1) eq trim(rez)  .
         num = R-INDEX ( stdata.fun, ',' ) .
         temp.val = temp.val + decimal(substr(stdata.fun,num + 1)).
   end.
 end. /*if rez <> '*' */
