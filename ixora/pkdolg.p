 ...
 * AUTHOR
        15/03/2005 madiar
 * CHANGES
        19/03/2005 madiar - исправил ошибку - не попадали кредиты без просрочек по ОД и %, но со штрафами
        23/03/2005 madiar - переставил колонки; округление платежа и штрафов вверх до целых
        08/06/2005 madiar - полностью переделал отчет - скопировал из отчета по задолжникам 4-4-3-9
        09/08/2005 madiar - добавил колонки "Выд. сумма" и "Остаток ОД"
        01/11/2005 madiar - добавил внебаланс
        02/11/2005 madiar - внебалансовые кредиты отсеивались, исправил
        13/02/2006 madiar - дни просрочки - из londebt
        08/03/2006 Natalya D. - добавила поля по начисленным за балансом % и штрафам (bal4 & bal5)
...
def var lst_non as char init "10,15,30,35,50,55,70,90,92". /* группы кредитов юридических лиц и БД */
...
for each lon where lon.cif matches v-cif no-lock:
     ...
     run lonbalcrc('lon',lon.lon,datums,"4",yes,lon.crc,output v-amt4).
     run lonbalcrc('lon',lon.lon,datums,"5",yes,1,output v-amt5).

     dlong = lon.duedt.
     if lon.ddt[5] <> ? then dlong = lon.ddt[5].
     if lon.cdt[5] <> ? then dlong = lon.cdt[5].

     if dlong > lon.duedt and dlong > datums and v-am16 <= 0 and v-bal[1] <= 0 and v-bal[2] <= 0 and v-bal[3] <= 0 and v-amt4 <= 0 and v-amt5 <= 0 then next. /* если есть пролонгация, дата пролонгации еще впереди и нет штрафов - пропускаем */
     ...
     v-am0 = 0. v-am1 = 0. v-am2 = 0. v-am3 = 0. prosr_od = 0. v-amt4 = 0. v-amt5 = 0.
      /* просрочка % */
...
define stream rep.
output stream rep to pkdolg.htm.
...
put stream rep unformatted "<tr align=""center""><td><h3>Задолженность по ссудным счетам клиентов за " string(datums) "<BR>".
...
put stream rep unformatted "</h3></td></tr><br><br>" skip.
       ...
       put stream rep unformatted "<br><tr><td><table border=""1"" cellpadding=""10"" cellspacing=""0"" style=""border-collapse: collapse"">" skip
                  ...
                  "<td>Валюта</td>"
                  "<td>Штрафы<br>к оплате<br>KZT</td>"
                  "<td>Штрафы KZT</td>"
                  "<td>Начисленные штрафы<BR>за балансом</td>"
                  "<td>Штрафы<BR>списанные за<br>баланс KZT</td>"
                  "<td>Дней просрочки</td>"
                  "<td>Просрочка ОД</td>"
                  "<td>Просрочка %</td>"
                  "<td>Индекс ОД</td>"
                  "<td>Индекс %%</td>"
                  "<td>ОД,<BR>списанный<br>за баланс</td>"
                  "<td>начисленные %<BR>за балансом</td>"
                  "<td>%,<BR>списанные<br>за баланс</td>"
...
/* Все остальные кредиты */
  ...
  for each wrk no-lock break by wrk.segm by wrk.crc.
        ...
        if first-of(wrk.crc) then do:
          message wrk.crc view-as alert-box buttons ok.
        ...
        */
        ...
        put stream rep unformatted "<tr align=""right"">"
               ...
               "<td>" replace(trim(string((wrk.bal16 + wrk.bal5 + wrk.bal30), "->>>>>>>>>>>9.99")),".",",") "</td>" skip
