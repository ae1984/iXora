...
find last {&file} use-index {&index} where {&where} exclusive-lock no-error.
...
do {&j} = 1 to {&n}:
   {&max-key} = {&file}.{&key}.
   find prev {&file} use-index {&index} where {&where} exclusive-lock no-error.
   if not available {&file}
   then leave.
end.
find first {&file} use-index {&index} where {&where} exclusive-lock.
{&postfind}.
{&min-key} = {&file}.{&key}.
{&i} = 2.
readkey pause 0.
repeat with frame {&frame}:
   ...
   then do:
        {&i} = {&i} + 1.
        find next {&file} use-index {&index} where {&where} exclusive-lock
             no-error. 
        if not available {&file}
        then do:
             find last {&file} use-index {&index} where {&where} exclusive-lock.
             {&postfind}.
             display {&display} with frame {&frame}.
             pause 0.
             {&i} = {&n}.
   ...
   then do :
        if {&i} >= {&n} and lastkey <> keycode("CURSOR-DOWN")
        then do:
             {&dispkopa}.
             display {&display} with frame {&frame}.
             message "Поиск".
             ...
             then do:
                  ...
                  do {&j} = 1 to {&n}:
                     ...
                     find prev {&file} use-index {&index}
                          where {&where} exclusive-lock no-error.
                     if not available {&file}
                     then leave.
                  end.
             end.
