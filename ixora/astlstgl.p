...
{mainhead.i FRBG}  /* PRINT FIXED ASSET LIST */

def var vdt as date.
def var vmo as int.
def var vdp  like jl.dam.
...
def var tast as int format "zz,zz9" label "# ITEMS".
...
for each crc where crc.sts ne 9 break by crc.crc:
  ...
  for each ast where ast.crc eq crc.crc break by ast.crc by ast.gl by ast.ast:
  ...
  */
    ...
    texpa = texpa + vexpa.
    trem = trem + vrem.
    display ast.gl ast.fag  /* ast.ofc ast.cont   */
	    ast.ast
	    ast.name
	    ast.rdt label "PUR-DATE" ast.noy vins skip
	    vrem (sub-total by ast.crc by ast.gl) format "z,zzz,zzz,zzz,zz9.99-"
	    ast.qty (sub-total by ast.crc by ast.gl)
	    vamt (sub-total by ast.crc by ast.gl)
				  format "z,zzz,zzz,zzz,zz9.99-"
	    vbook (sub-total by ast.crc by ast.gl)
				  format "z,zzz,zzz,zzz,zz9.99-"
	    vdepa (sub-total by ast.crc by ast.gl)
				  format "z,zzz,zzz,zzz,zz9.99-"
	    vexpa (sub-total by ast.crc  by ast.gl)
				  format "z,zzz,zzz,zzz,zz9.99-"
	    with width 132 down frame ast no-box.
  end.
  display trem tast tamt tbook tdepa texpa
