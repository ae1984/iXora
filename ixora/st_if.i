 ...
 * MODULE
        Клиентская база
 * DESCRIPTION
        общая процедура печати выписок по ОТКРЫТЫМ счетам клиентов
 * RUN
 ...
 * CHANGES
        ...
        02.09.2004 dpuchkov - добавил ограничение доступа на просмотр реквизитов клиентов
        06.03.2012 damir - добавил возможность выбора группирования - хронология, по сумме.
        28.03.2012 damir - корректировка выбора группирования...
...
with overlay no-label no-box at row 5 column 55 frame yy.
...
form
    o-label format "x(11)"
with overlay no-label no-box row 5 column 60 frame t.

form
        df
        dt
        st label " Статус" skip
        hronol skip
        dksum skip
...
run r-vichk.
if stat-us then do: message "Запрещено!". pause 5. return. end.
end. /* ... transaction ... */
...
display st with frame opt.
update hronol dksum with frame opt.
if hronol = yes and dksum = yes then update hronol = yes dksum = no with frame opt.
...
display df dt hronol dksum with frame opt.
...
repeat:
update dt with side-label row 9 overlay frame c-opt.
...
if krja <> 0 then do:
   ...
   with no-label at row 9 column 20 frame afr.
