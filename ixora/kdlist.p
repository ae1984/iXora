 ...
 * MODULE
     ...
     Список всех досье 
 * DESCRIPTION
        Вывод списка всех досье в разрезе статуса по филиалу
 * RUN
        без параметров
 * CALLER
 ...
 * AUTHOR
        01.12.03 marinav
 * CHANGES
        12.01.04 marinav - работа над ошибками
        30/04/2004 madiar - отбор по датам не работал
                          в ГБ можно выбрать досье ГБ или досье филиалов
        18/05/04 madiar - добавил no-lock.
...
if s-ourbank = "TXB00" then do:
  ...
  run sel2 ("Выбор :", " 1. Список досье, принятых в ГБ | 2. Список досье, принятых в филиалах | 3. Выход ", output v-sel).
...
put stream m-out "<tr align=""center""><td><h3>Заявки, принятые с "
 ...
 put stream m-out "<br><br><tr></tr>".
       ...
       if s-ourbank = "TXB00" and gb = no then put stream m-out "<td bgcolor=""#C0C0C0"" align=""center"">Филиал</td>".
       
       put stream m-out "<td bgcolor=""#C0C0C0"" align=""center"">Наименование заемщика</td>"
                  "<td bgcolor=""#C0C0C0"" align=""center"">Номер КД</td>"
                  "<td bgcolor=""#C0C0C0"" align=""center"">Дата регистрации</td>"
                  ...
                  "<td bgcolor=""#C0C0C0"" align=""center"">Статус досье</td>" 
                  "</table>" skip.

if s-ourbank <> "TXB00" or (s-ourbank = "TXB00" and gb = yes) then do:

 for each kdlon where kdlon.bank = s-ourbank and kdlon.whn >= dt and kdlon.whn <= dt1 no-lock break by kdlon.who by kdlon.kdcif.
...
*/
    put stream m-out "<tr align=""right"">"
               ...
               "<td align=""left""> " trim(kdcif.prefix) + " " + kdcif.name format "x(60)" "</td>"
               "<td align=""left""> " kdlon.kdlon format 'x(8)' "</td>"
               "<td align=""center""> " kdlon.whn "</td>"
 ...
 for each kdlon where kdlon.bank <> s-ourbank and kdlon.whn >= dt and kdlon.whn <= dt1 no-lock break by kdlon.who by kdlon.kdcif.
   ...
   if avail kdcif then do:
    ...
    find txb where txb.bank = kdlon.bank and consolid = yes no-lock no-error.
/*    find first kdaffil where kdaffil.bank = s-ourbank and kdaffil.code = '21' and kdaffil.kdcif = s-kdcif and kdaffil.kdlon = s-kdlon no-error.
    if avail 
*/
    put stream m-out "<tr align=""right"">"
               "<td align=""center""> " coun "</td>"
               "<td align=""center""> " txb.name "</td>"
               "<td align=""left""> " trim(kdcif.prefix) + " " + kdcif.name format "x(60)" "</td>"
               "<td align=""left""> " kdlon.kdlon format 'x(8)' "</td>"
               "<td align=""center""> " kdlon.whn "</td>"
               "<td align=""right""> " kdlon.amountz format "->>>,>>>,>>9.99" "</td>"
