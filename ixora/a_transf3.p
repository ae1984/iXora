 ...
 * AUTHOR
        07.12.2011 Luiza
 * CHANGES
        06/02/2012 Luiza добавила поля  v_oper v_sum для редактирования при импорте ЗК
        ...
        10/07/2012 Luiza  - при отмене проверяем статус док Doc:TransferStatus <> 7
        ...
        08/11/2012 Luiza - входящий параметр Docu для Юнистрим
        ...
        28/11/2012 Luiza - ТЗ 1580 импорт Юнистрим
...
form
    ...
    v_oper1    no-label format "x(70)" colon 19  skip
...
on help of v-joudoc in frame f_main do:
    if v-ek = 1 then run a_help-joudoc1 ("TR3" + string(v-select4)). else run a_help-joudoc1 ("RT3" + string(v-select4)).
...
if new_document then do:  /* создание нового документа  */
    ...
    v_title = " БЫСТРЫЕ ПЕРЕВОДЫ (отмена перевода)" + v-sys.
    ...
    if v_u = 2 then do:       /* update */
        ...
        if available joudoc then do:
            ...
            if available joudop then do:
                if joudop.type <> "TR3" + string(v-select4)  and joudop.type <> "RT3" + string(v-select4) then do:
                    message substitute ("Документ не относится к типу отмена быстрых перевода ") + v-sys view-as alert-box.
                    return.
                end.
                if v-ek = 1 and joudop.type = "RT3" + string(v-select4) then do:
                    message substitute ("Документ создан для ЭК ГК 100500") view-as alert-box.
                    return.
                end.
                if v-ek = 2 and joudop.type = "TR3" + string(v-select4) then do:
...
procedure import_docu:
 if Docu:TransferStatus <> 42 or Docu:PayeeSname <> Docu:PayerSname or Docu:PayeeMname <> Docu:PayerMname then do:
   message "Неверный тип документа.~nДолжен быть документ на возврат!" view-as alert-box.
   return.
 end.
  v-cifmin = "".
  ...
  v_doc_num =  Docu:PayeeSerialNumber + " " + Docu:PayeeCNumber.
  ...
  v_docdtf = DATE(int(substring(Docu:receiver_docexp,6,2)), int(substring(Docu:receiver_docexp,9,2)), int(substring(Docu:receiver_docexp,1,4))).
  ...
  displ v_rnn v_rez v_lname v_name v_mname
   ...
   if v_rnn <> "" then do:
    ...
    /* 655044976 */
    update v_rnn v_countr /* v_doctype*/ v_public v_sum with frame f_main.
    v_lname1  = v_lname.
    v_name1  = v_name.
    v_mname1 =  v_mname.
    /*v_rez1 = v_rez.
    v_countr1 = v_countr.*/
    displ v_lname1 v_name1 v_mname1 with frame f_main.
...
procedure import_doc:
 /* 274596948 */
    if Doc:TransferStatus <> 7 then do:
   message "Неверный тип документа.~nДолжен быть документ на отмену!" view-as alert-box.
   return.
  end.
  ...
  displ v_rnn v_rez v_lname v_name v_mname
        v_countr v_doctype v_doc_num v_docwho
        v_docdt v_docdtf v_public v-bplace v-bdt1 v_addr
        v_tel v_knp v_oper v_oper1 v_rez1 v_lname1 v_name1 v_mname1
        v_countr1 v-transf v_crc v_sum  with frame f_main.
   ...
   if v_rnn <> "" then do:
    ...
    /* 655044976 */
    update v_lname v_name v_mname v_countr v_docdtf  v_public  v-bplace v_rez1 v_oper v_oper1  v_sum with frame f_main.
    v_lname1  = v_lname.
    v_name1  = v_name.
    v_mname1 =  v_mname.
    displ v_lname1 v_name1 v_mname1 with frame f_main.
...
procedure save_doc:
    if v_u = 2 and v-select4 = 3 and VALID-OBJECT(Doc) then do:
        ...
        v_name1  = v_name.
        v_mname1 =  v_mname.
        displ v_lname1 v_name1 v_mname1 vj-label no-label with frame f_main.
        run chksum.  /* расчет суммы внесения  */
        if v-cur then undo, return.
    end.
    else do:
        if v_u = 2 and v-select4 = 4 and VALID-OBJECT(Docu) then do:
            ...
            if trim(v_rnn) <> "-" then do:
                ...
                if available cifmin then do:
                    ...
                    if cifmin.res = "1" then v_rez = "19". else v_rez = "29".
                    /*update v_iin with frame f-iin.*/
                    displ v-joudoc v-label v_rnn  v_lname v_name v_mname v_rez  with frame f_main.
                end.
                else do:
                    /*v_iin = "".
                    update v_iin with frame f-iin.*/
...
procedure view_doc:
    ...
    v_oper = substring(joudoc.remark[1],1,70).
    v_oper1 = substring(joudoc.remark[1],71,length(joudoc.remark[1]) - 70).
    ...
    if v_countr1 = "CN" and v-select4 = 3 then do:
    ...
    displ v-label format "x(18)" no-label v_rnn format "x(12)" no-label v_trx v_lname v_name v_mname   v_doc_num v_docdt v_docdtf
          ...
          v_oper v_oper1 v-transf v_dt  v_kt  v_arp v_arpname  with frame f_main.
    if v_countr1 = "CN" and v-select4 = 3 then displ v-sw v-bank v-branch v-chet with frame f_main.
...
procedure Create_transaction:
    ...
    if v-ek = 2 then do:
        ...
        if (v_sumv = v_sum and v_crc <> 1) or v_crc = 1 then do:
            ...
            v-param = v-joudoc + vdel + string(v_sum - v_sumv) + vdel + string(v_crc) + vdel + v-chEK + vdel + "обмен валюты" +
                    vdel + substring(v_rez,1,1) + vdel + substring(v_rez,1,1) + vdel + "9" + vdel + "9" + vdel + "213" + vdel +
                    "1" + vdel + v-chEK1 .
            run trxgen (v-tmpl, vdel, v-param, "jou", v-joudoc, output rcode, output rdes, input-output s-jh).
    ...
    if v-ek = 1 then do:
            v-tmpl = "JOU0001".
            v-param = v-joudoc + vdel + string(v_sum) + vdel + string(v_crc) + vdel + v_arp + vdel + v_oper + v_oper1 + vdel + v_r1 + vdel + v_r + vdel + "9" + vdel + "9" + vdel + v_knp.
            run trxgen (v-tmpl, vdel, v-param, "jou", v-joudoc, output rcode, output rdes, input-output s-jh).
            if rcode ne 0 then do:
                message rdes.
                pause.
    ...
    for each sendod no-lock.
        ...
        "Добрый день!\n\n Необходимо отконтролировать отмену быстрого перевода \n Сумма: " + string(v_sum) +
        "  " + v-crc_val + "\n документ :" + v-joudoc + "\n создал :" + g-ofc + "\n " + string(g-today) + "  " +
        string(time,"HH:MM"), "1", "","" ).
    end.
    hide all.
...
procedure save_date:
    ...
    joudop.amt = v_sumv.
    joudop.amt1 = v_sumt.
    if v_countr1 = "CN" and v-select4 = 3 then joudop.mname = trim(v-sw) + "^" + trim(v-bank) + "^" + trim(v-branch) + "^" + trim(v-chet).
    if v-ek = 1 then joudop.type = "TR3" + string(v-select4). else joudop.type = "RT3" + string(v-select4).
    displ v-joudoc with frame f_main.
end procedure.
...
/*--------------------------------------------------------------*/
    ...
    v-res111 = TCIFNAME + "&" + TINN + "&" + TKOD + "&" + TSUMM + "&" + TCRC + "&" + TCOMSUMM + "&" + TCOMCRC + "&" + TRECNAME + "&" + TKBE + "&" + TRECAAA + "&" + TRBANK + "&" + TREM + "&" + TKNP + "&" + TSYSTEM.
