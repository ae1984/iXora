...
for each txb.lon no-lock:
  ...
  if avail txb.cif then vgeo = txb.cif.geo.

  if vgeo = '021' then do:
      find first lnpr where lnpr.id = txb.lon.crc and lnpr.gid = 1 no-lock no-error.
      ...
      find first lnpr where lnpr.id = txb.lon.crc and lnpr.gid = 2 no-lock no-error.
      if avail lnpr then do:
        lnpr.nsum = lnpr.nsum + (v-bal1 + v-bal7 + v-bal2 + v-bal9) * rates[txb.lon.crc].
      end.
      else do:
