 ...
 * CHANGES
        ...
        20/06/2011 evseev - из-за перехода на ИИН/БИН, ink2 переименован в ink2_1
        21/06/2011 evseev - перекомпиляция
        22/06/2011 evseev - удалил и добавил в src
        05/07/2011 evseev - логирование в добавлении и удалении  run savelog ("ink2_1"...
        20/07/2011 lyubov - исключила из выводимого списка счетов счета О/Д
        15.08.2011 ruslan - изменил payee
...
/* блок объявления переменных ->*/
    ...
    def browse b1 query q1 displ
       ...
       aas.aaa format "x(21)" label "Счет"
       aas.regdt format "99.99.99" label "Дата рег.  "
       aas.payee  format "x(40)" label "Примечание"
       with 12 down title "Приостановление операций за искл. плат в бюджет(действующие)" overlay.
    def browse b4 query q4 displ
        ...
        with 12 down title "История спец. инструкций (текущие и удаленые)" overlay.
    ...
    def frame getlist1
       ...
       aas.nkbin  format "x(16)"     label           "БИН              " validate((chk12_innbin(aas.bnf)),'Неправильно введён БИН/ИИН') skip
       aas.payee /*aas.docprim*/ label "Примечание       " skip
    ...
    def frame aacc
       ch_acc label "Счет" format "x(21)" validate(ch_acc <> "", "Введите номер счета ")
       with side-labels centered row 8.
...
on help of aas.bnf in frame getlist1 do:
      ...
      {itemlist.i
       &file = "t-ln"
       &frame = "row 6 centered scroll 1 12 down overlay title 'Выберите налоговый комитет' "
       &where = " true "
...
on choose of bt_AddNew in frame a2 do:
    ...
    if avail aaa then do:
       ...
       if avail cif then do:
          message "Наименование клиента:" cif.name  skip "БИН клиента:" cif.bin view-as alert-box question buttons ok title "Визуальный контроль".
       end.
       create aas.
       ...
       aas.aaa = aaa.aaa.
       aas.payee = "Приостановлении р/х операций за искл.отдельных платежей в бюджет".
       displ aas.aaa with frame getlist1.
       aas.whn = g-today.
       update aas.regdt aas.docnum aas.docdat aas.bnf /*aas.nkbin*/ /*aas.kbk*/ aas.payee with frame getlist1.
       ...
       if avail cif then aas.cif = cif.cif.
       aas.sic = 'HB'.
       run savelog ("ink2_1", "bt_AddNew 1: " + aaa.aaa + " aaa.hbal = " + string(aaa.hbal) + " " + aas.sic).
       s-aaa = aaa.aaa.

       find first aaa where aaa.aaa = s-aaa exclusive-lock.
       run savelog("aaahbal", "ink2_1 ; " + aaa.aaa + " ; " + string(aaa.hbal) + " ; " + string(aaa.hbal + aas.chkamt) + " ; " + string(aas.chkamt)).
       aaa.hbal = aaa.hbal + aas.chkamt.
       run savelog ("ink2_1", "bt_AddNew 2: " + aaa.aaa + " aaa.hbal = " + string(aaa.hbal) + " " + aas.sic).

       run savelog ("ink2_1", "bt_AddNew 3: " + aaa.aaa + " aaa.hbal = " + string(aaa.hbal) + " " + aas.sic).
...
on choose of bt_Find in frame a2 do:
    ...
    ON CHOOSE OF bhistory IN frame fr1 do:
       ...
       p-aaa = ''.
       p-ln = 0.
       for each aas_hist where aas_hist.aaa = aas.aaa and aas_hist.sta = 2 NO-LOCK USE-INDEX aasprep:
    ...
    ON CHOOSE OF bRedakt IN frame fr1 do:
       ...
       if avail buf and avail aaa then do:
          display aas.who aas.regdt aas.nkbin aas.payee with frame getlist5.
          update aas.nkbin  aas.payee with frame getlist5.
          s-aaa = aas.aaa.
          op_kod = 'E'.
          aas.mn = substr(aas.mn,1,3) + "28".
          RUN aas2his.
          if (aas.nkbin entered) or (aas.payee entered) then op_kod= 'E'.
          hide frame getlist5.
       end.
    end.
    ON CHOOSE OF bprint IN frame fr1 do:
       ...
       put unformatted "----------------------------------------------------------------------------------------------------------------------" skip.
       if v-dep = 1 then
           for each aas where aas.sta = 2 and aas.aaa = s_FindAcc and aas.ln <> 7777777 use-index aaaln:
             find last b-cif where b-cif.cif = aas.cif no-lock no-error.
             put unformatted "  " aas.cif " " b-cif.sname format 'x(20)' string(aas.regdt,"99/99/99") "  " aas.aaa format 'x(21)' " " aas.docnum format 'x(7)' "  " string(aas.docdat,"99/99/99") "   " aas.nkbin format 'x(15)'  "   " aas.payee  skip.
           end.
       if v-dep = 2 then
           for each aas where aas.sta = 2 and aas.cif = s_FindCIF and aas.ln <> 7777777 use-index aaaln:
             find last b-cif where b-cif.cif = aas.cif no-lock no-error.
             put unformatted "  " aas.cif " " b-cif.sname format 'x(20)' string(aas.regdt,"99/99/99") "  " aas.aaa format 'x(21)' " " aas.docnum format 'x(7)' "  " string(aas.docdat,"99/99/99") "   " aas.nkbin format 'x(15)'  "   " aas.payee  skip.
           end.
       if v-dep = 3 then
           for each aas where aas.sta = 2 and aas.who = s_FindLogin and aas.ln <> 7777777 use-index aaaln:
             find last b-cif where b-cif.cif = aas.cif no-lock no-error.
             put unformatted "  " aas.cif " " b-cif.sname format 'x(20)' string(aas.regdt,"99/99/99") "  " aas.aaa format 'x(21)' " " aas.docnum format 'x(7)' "  " string(aas.docdat,"99/99/99") "   " aas.nkbin format 'x(15)'  "   " aas.payee  skip.
           end.
       if v-dep = 4 then
           for each aas where aas.sta = 2 and aas.regdt >= dt_FindDateBegin and aas.regdt <= dt_FindDateEnd and aas.ln <> 7777777 use-index aaaln:
             find last b-cif where b-cif.cif = aas.cif no-lock no-error.
             put unformatted "  " aas.cif " " b-cif.sname format 'x(20)' string(aas.regdt,"99/99/99") "  " aas.aaa format 'x(21)' " " aas.docnum format 'x(7)' "  " string(aas.docdat,"99/99/99") "   " aas.nkbin format 'x(15)'  "   " aas.payee  skip.
           end.
       output close.
    ...
    ON CHOOSE OF bdetail IN frame fr1 do:
       ...
       if avail buf then do:
          ...
          display aas.who aas.docnum aas.docdat aas.bnf  aas.nkbin  with frame getlist3.
          hide frame getlist3.
       end.
    end.
    /* Снять ограничение */
           ...
           if yes-no ("Внимание!", "Вы действительно хотите снять ограничение?") then do:
              ...
              if not aas.contr then do:
                 ...
                 MESSAGE "Необходим контроль Директором/ЗамОД в 1.3.1.6" VIEW-AS ALERT-BOX QUESTION buttonS OK.
                  ...
                  find first aaa where aaa.aaa = aas.aaa exclusive-lock no-error.
                  if avail aaa then do:
                     run savelog ("ink2_1", "brem 1: " + aaa.aaa + " aaa.hbal = " + string(aaa.hbal)).
                     run savelog("aaahbal", "ink2_1 ; " + aaa.aaa + " ; " + string(aaa.hbal) + " ; " + string(aaa.hbal - aas.chkamt) + " ; " + string(aas.chkamt)).
                     aaa.hbal = aaa.hbal - aas.chkamt.
                     run savelog ("ink2_1", "brem 2: " + aaa.aaa + " aaa.hbal = " + string(aaa.hbal)).
                  end.
                  op_kod= 'D'.
                  aas.who = g-ofc. aas.whn = g-today. aas.tim = time.
                  s-aaa = aaa.aaa.
                  aas.mn = substr(aas.mn,1,3) + "29".
                  RUN aas2his.
                  if lookup (aaa.lgr , "138,139,140,143,144,145") > 0 then do:
    ...
    repeat:
        ...
        case v-dep:
              when 1 then do: /*По номеру счета*/
                   ...
                   if i_indx <> 0 then do:
                      open query q1 for each aas where aas.sta = 2 and aas.aaa = s_FindAcc and aas.ln <> 7777777 use-index aaaln.
                      ...
                      if frame-index = 1 then do:
                             ...
                             p-aaa = ''.
                             p-ln = 0.
                             for each aas_hist where aas_hist.aaa = aaa.aaa and aas_hist.sta = 2 NO-LOCK USE-INDEX aasprep:
                                 if aas_hist.ln <> p-ln then do transaction:
                                    p-aaa = aas_hist.aaa.
                                    p-ln = aas_hist.ln.
                                    create t-aashist.
              ...
              when 2 then do: /*По CIF коду    */
                   ...
                   open query q1 for each aas where aas.sta = 2 and  aas.cif = s_FindCIF and aas.ln <> 7777777 use-index aaaln.
                   b1:SET-REPOSITIONED-ROW (1, "CONDITIONAL").
                   ENABLE all with frame fr1 centered overlay top-only.
                   apply "value-changed" to b1 in frame fr1.
                   WAIT-FOR WINDOW-CLOSE of frame fr1.
              ...
              when 3 then do: /* По менеджеру */
                   ...
                   if v-dp3 = 1 then do:
                      open query q1 for each aas where aas.sta = 2 and aas.who = s_FindLogin and aas.ln <> 7777777 use-index aaaln.
                   end.
                   if v-dp3 = 2 then do:
                      ...
                      open query q1 for each aas where aas.sta = 2 and aas.who = s_FindLogin and aas.regdt >= dt_1 and aas.regdt <= dt_2 and aas.ln <> 7777777 use-index aaaln.
                   end.
                   b1:SET-REPOSITIONED-ROW (1, "CONDITIONAL").
                   ENABLE all with frame fr1 centered overlay top-only.
                   apply "value-changed" to b1 in frame fr1.
              ...
              when 4 then do: /*За период      */
                   ...
                   open query q1 for each aas where aas.sta = 2 and aas.regdt >= dt_FindDateBegin and aas.regdt <= dt_FindDateEnd and aas.ln <> 7777777 use-index aaaln.
                   b1:SET-REPOSITIONED-ROW (1, "CONDITIONAL").
                   ENABLE all with frame fr1 centered overlay top-only.
                   apply "value-changed" to b1 in frame fr1.
                   WAIT-FOR WINDOW-CLOSE of frame fr1.
