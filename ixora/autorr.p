 ...
 * CHANGES
        ...
        16.08.2004 tsoy Добавил Группу
        20.08.2004 tsoy поиск по всем не закрытым кредитам
...
define stream m-out.
output stream m-out to autorr.html.
...
for each lon no-lock.
  ...
  find loncon  where loncon.lon = lon.lon and index(loncon.rez-char[10],"&") > 0  no-lock no-error.  
     if avail loncon then  do:
           if substring(loncon.rez-char[10],index(loncon.rez-char[10],"&") + 1,3) = "yes"  then 
                  paraksts = yes.
           else   
                  paraksts = no.

           if not paraksts then do:
               for each ln%his where ln%his.lon = lon.lon and ln%his.cif = lon.cif no-lock.
                 create t-data.
                     assign
                     t-data.cif     = lon.cif              
                     t-data.lon     = lon.lon            
...
put stream m-out unformatted "<h3>Отчет по неподписанным выданным кредитам. <br>" skip.
       ...
       put stream m-out unformatted "<tr style=""font:bold"">"
                         ...
                         "<td bgcolor=""#C0C0C0"" align=""center"">Оформ/редакт      </td>"
...
for each t-data break by t-data.cif by t-data.lon:

if first-of(t-data.lon) then do:
put stream m-out  unformatted "<tr>"
                   "<td colspan =""22""><b style=""font:bold;color:red"">" t-data.cif-name " " t-data.cif "</b></td>"  skip
                   "</tr>" skip.
end.
...
unix silent cptwin autorr.html excel.
