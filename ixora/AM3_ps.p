 ...
 * MENU
        Пункт меню
 * AUTHOR
        21/09/2010 galina - скопировала из AMLOFF_ps.p
 * BASES
        BANK COMM
 * CHANGES
        19/11/2010 madiyar - выгрузка удаленных операций
        27/11/2010 madiyar - добавил проверку на запуск именно на боевом сервере
...
def var v-property as char no-undo.
def var v-propertynumber as char no-undo.
def var v-sdp as char no-undo.                   /*Золотая корона*/
...
for each amloffline where amloffline.sts = "new" no-lock:
    ...
    */
    ...
    if v-out = 0 then do:
        run setStringProperty in messageh("Property", v-property) no-error.
        if error-status:error then v-out = 4.
    end.
    if v-out = 0 then do:
        run setStringProperty in messageh("PropertyNumber", v-propertynumber) no-error.
        if error-status:error then v-out = 4.
    end.

    if v-out = 0 then do:
        run setStringProperty in messageh("SDP", v-sdp) no-error.
        if error-status:error then v-out = 4.
    end.
