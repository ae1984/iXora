 ...
 * DESCRIPTION
        Вывод списка всех одобренных заявок 
 * RUN
        без параметров
 * CALLER
...
put stream m-out "<tr align=""center""><td><h3>Заявки, одобренные с "
 ...
 put stream m-out "<br><br><tr></tr>".
       ...
       put stream m-out "<td bgcolor=""#C0C0C0"" align=""center"">Наименование заемщика</td>"
                  "<td bgcolor=""#C0C0C0"" align=""center"">Дата заседания КК</td>"
...
for each kdlon where (kdlon.bank = s-ourbank or s-ourbank = "TXB00") and (kdlon.resume = '02' or kdlon.resume = '03')
                     and kdlon.datkk >= dt and kdlon.datkk <= dt1 no-lock
                     break by kdlon.who by kdlon.kdcif.
   
   if s-ourbank = "TXB00" and kdlon.bank <> "TXB00" and kdlon.resume = '02' then next. /* для ГБ исключить заявки, одобренные КК филиала */
...
*/
    ...
    if s-ourbank = "TXB00" then put stream m-out "<td align=""left""> " txb.name "</td>".

    put stream m-out "<td align=""left""> " trim(kdcif.prefix) + " " + kdcif.name format "x(60)" "</td>"
               "<td align=""left""> " kdlon.datkk  "</td>"
               "<td align=""right""> " kdlon.amount format "->>>,>>>,>>9.99" "</td>"
