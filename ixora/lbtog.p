 ...
 * CHANGES
	...
	03.10.2006 u00121 выдается сообщение при некорректном завершении lb100g.p      
        24/01/2008 marinav - rcp на scp
...
find sysc where sysc.sysc = "lbtog" no-lock no-error .
if not avail sysc or sysc.chval = "" then 
do:
	v-text = " ERROR !!! There isn't record LBTOG in sysc file !! ".
...
find sysc where sysc.sysc = "lbtoga" no-lock no-error .
if not avail sysc or sysc.chval = "" then 
do:
	v-text = " ERROR !!! There isn't record LBTOGA in sysc file !! ".
	message v-text view-as alert-box.
	run lgps.
	return .
end.
...
do :
	v-text = " ERROR !!! Records LBTOG and LBTOGA are equal !! ".
	message v-text view-as alert-box.
	run lgps.
	return .
end .
...
do:
	...
	do:
		...
		for each t-qarc. 
			...
			do:
				yn = false .
				Message " Clearing " + string(vnum) + " archive has been already done. Continue  ?  " update yn .
				if not yn then 
					leave .
			end.
		end.
		...
		if return-value = "1" then
			message "Обратитесть к Администраторам АБПК!" skip 
				"Пачка  GROSS # " vnum " выгрузилась не корректно!" view-as alert-box title "Произошла ошибка выгрузки!". 
	end.
...
do:
	...
	for each t-qarc.
		...
		do:
			yn = false .
			Message " Clearing " + string(vnum) + " archive has been already done. Overwrite it ?  " update yn .  
			if yn then leave.
		end.
	end.
