 ...
 * MODULE
        Платежная система
 * DESCRIPTION
        автоматическая регистрация входящих платежей в тенге по CLEAR в течении дня
 * RUN
 ...
 * AUTHOR
        16/02/2008 marinav
 * CHANGES
        04.06.10 marinav- изменился список казначейских платежей
        09.07.2010 marinav - Счет-карточка ARP по дебиторской задолженности падает на полочку
...
/* Создание списка файлов для загрузки */
       ...
       repeat : 
          ...
          if search(num[9]) eq num[9]  then do: 
               find first lbinf where lbinf.rdt = g-today and lbinf.name = num[9] and lbinf.gc = 'gross' no-lock no-error.
               if not avail lbinf then do:  
                    ...
                    if t-gro = 'CLEAR' or t-str = "970" then do:
                           if t-str = "970" then l-fl = yes.
                           create t-qin.
...
/* i - создание REMTRZ по списку */
    ...
    if l-fl = no then do:
      message skip " Финальная выписка 970 еще не принята! " skip(1) view-as alert-box .
      return.
    end. 

    Message "Обработать платежи CLEAR ? " update yn . 
    if yn then  do:
        ...
        v-ok = false.
        run lb-check ('clear').   /* сверка */

        MESSAGE "" . pause 0 . 
                                                                                                                                           
        v-text = " Сверка CLEARING проведена : " + 
        ...
        Message " Сделать платежи ? " update yn .
      ...
      /* SEND */  
       ...
       for each que where que.pid = "LBI" or que.pid = "STW" exclusive-lock,  
           each remtrz where remtrz.remtrz = que.remtrz and remtrz.cover = 1 and remtrz.fcrc = 1 exclusive-lock ,
            ...
            if remtrz.ptype = "7" then do :
              ...
              if remtrz.racc = v-nostro then do:
                ...
                if avail bankl and bankl.name matches "*казнач*" and 
                  lookup(remtrz.sacc, v-kazn) > 0 and 
                     ...
                     if v-bin = no then do:
                           ...
                           then do :
                            ...
                            if not avail clfilials then do:
                               ...
                               remtrz.crgl = 0.
                               remtrz.cracc = "".
                               v-text = remtrz.remtrz + " Ошибка в ИИН/БИН.".
                               run lgps.
                            end. 
                           end.
                      end.

                     if que.rcod ne "1" then do:
                  ...
                  if avail arp then do:
                    ...
                    if not avail sub-cod or sub-cod.ccode <> 'msc' then do.
                      ...
                      if (string(remtrz.crgl) = card-gl or lookup(arp.arp, v-arplbi) > 0) then do:
                         ...
                         find first debgrp where debgrp.arp = arp.arp no-lock no-error.
                            if avail debgrp then do:
                               que.rcod = "1".
                               remtrz.crgl = 0.
                               remtrz.cracc = ''.
                               remtrz.rsub = 'arp'.
                               v-text = remtrz.remtrz + " Счет-карточка ARP по дебиторской задолженности".
                               run lgps.
                            end.
                      end.
                    end.
