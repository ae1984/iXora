 ...
 * MODULE
        Клиентская база
 * DESCRIPTION
        Снятие с текущего счета в другом филиале
 * RUN
 ...
 * AUTHOR
        24.06.2010 marinav
 * CHANGES
        23.08.2010 marinav - платежи отдельно по сумме изъятия и комиссии
        15.09.2010 marinav - шаблон jou0041 с КНП и Кбе
		...
		13.12.2012 id00477 - для next-value явно указал базы
*/
...
def new shared var v-chk as inte.
...
if v-sel = '1' then do:
      ...
      v-tit = 'Снятие с текущего счета на филиале '.
      v-type = 'DDA,SAV'.
     ...
     /*********************************************************************/
       ...
       if v-ja then do:
              ...
              if v-fu = 'B' then  do:
               ...
               create filpayment.
                 filpayment.id = 'fil' + string(next-value(filp,comm)).
                 filpayment.type = 'getdda'.
                 ...
                 filpayment.info[5] = v-fu.
                 filpayment.info[6] = string(v-chk).
    ...
    /* платежи отдельно по сумме изъятия и комиссии*/
        if filpayment.amountcom > 0 then do:
          ...
          do transaction:
          if v-rmz ne "" then  do:
                                  displ "Платеж " v-rmz " на снятие средств со счета " filpayment.iik " сделан. Транзитный счет будет пополнен через 5 минут!" with frame fr title ""  overlay centered row 13.
                                  pause 8. hide frame fr.
                                  ...
                                  run mail   (filpayment.info[3],
                                              ...
                                              "Добрый день!\n\n ФИО: " + filpayment.name + "\n ИИК: " + filpayment.iik + "\n Снятие средств с текущего счета \n " +
...
if v-sel = '2' then do:
       ...
       find first filpayment where filpayment.bankfrom = s-ourbank and filpayment.type = 'getdda' and filpayment.jou = '' no-lock no-error.
       ...
       def new shared var v-id as char.
        ...
        {itemlist.i
           &file = "filpayment"
           &where = "filpayment.bankfrom = s-ourbank and filpayment.type = 'getdda' and filpayment.jou = '' "
           ...
           &frame = "row 7 centered scroll 1 8 down overlay "
        ...
        if v-ja then do:
          ...
          v-glrem = v-npl + " " + v-npl1.
            ...
            if trim(filpayment.info[5]) = 'B' then  do:
                   find sysc where sysc.sysc = "transu" no-lock no-error.
                   if not avail sysc or sysc.chval = "" then do:
                      display " В настройках нет записи transu  !!".   pause.   return.
                   end.
            ...
            find current jh no-lock.
               /*
            if trim(filpayment.info[5]) = 'B' then  run setcsymb (s-jh, 430).
                                              else  run setcsymb (s-jh, 420).
                 */
            ...
            if avail joudoc then do:
                   ...
                   joudoc.chk = integer(filpayment.info[6]). joudoc.kfmcif = filpayment.cif.
...
/****marinav*****/
            ...
            if not avail acheck then do:
                ...
                v-ch = "".
                v-ch = string(NEXT-VALUE(krnum,bank)).
                create acheck.
            ...
            if filpayment.amountcom > 0 then do:
                ...
                if avail tarif2 then do:
                    ...
                    if jh.sts ne 6 then jh.sts = 6.
                    for each jl of jh:
                            if jl.sts ne 6 then jl.sts = 6.
                    end.
                    find current jh no-lock.
                    find first joudoc where joudoc.docnum = v_doc no-error.
                    if avail joudoc then do:
