...
*/
 ...
 if avail que then
  do:
   ...
   find first jh where remtrz.jh2 = jh.jh no-lock no-error .
   if not avail jh then
   do:
     v-text = " 2 проводка не найдена в jh файле для " + remtrz.remtrz .
     run lgps.
     nxt = "10".
     que.con = "F".
  ...
  if jh.sts = 6 then do:
    ...
    v-text = "2 проводка акцептована " + jl.teller + " для " + remtrz.remtrz .    run lgps.
    que.rcod = "0".
    que.con = "F".
    que.pri = integer(que.pvar).
    que.pvar = "" .
