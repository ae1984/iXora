...
find last acheck where acheck.jh = string(v-jh) and acheck.dt = g-today no-lock no-error.
if avail acheck then do:
  ...
  find last ofc where ofc.ofc = g-ofc.
    ...
    if v-nm = 1 then
       put "                     ПРИХОДНЫЙ НОМЕР  "  g-today   "  " v-jh /*  format "99/99/99" */ skip.
    else
       put "                     РАСХОДНЫЙ НОМЕР  "  g-today   "  " v-jh /*  format "99/99/99" */ skip.
