 ...
 * BASES
        BANK COMM
 * CHANGES
        31.08.2010 MARINAV - новые шаблоны jou0027 jou0040
        ...
        12.03.2012 damir - отменил печать операционных и кассовых ордеров на матричный принтер,keyord.i.
*/
...
form  skip(1)
  ...
  '----------------------------Назначение платежа---------------------------' at 5 skip
    ...
    v-dog label "Номер и дата договора " format "x(45)" validate(trim(v-dog) ne "", "Введите данные по договору") skip
    v-acc label "Номер транзитного счета в Kassa Nova " format "x(20)" validate(trim(v-acc) ne "", "Введите счет") skip(2)
...
on help of v-knp in frame fr1 do:
  ...
  displ v-kod v-kbe v-fio v-sum_com v-ja with frame fr1.
  update v-sum v-crc with frame fr1.
  ...
  find pksysc where pksysc.sysc = "kasnov" no-lock no-error.
  if not avail pksysc or pksysc.chval = "" then do:
      display " В настройках нет записи kasnov  !!".
      pause.
      return.
  end.
  v-iik = entry(v-crc,trim(pksysc.chval)).
  displ v-crc_val v-iik with frame fr1.
  ...
  update v-fio v-rnn v-dog v-acc with frame fr1.
  v-ja = no.
...
/*********************************************************************/
  ...
  if v-ja then do:
       ...
       v-glrem = "Погашение кредита " + v-fio + " РНН " + v-rnn + " по договору займа " + v-dog + " Номер транзитного счета в Kassa Nova " + v-acc.
            ...
            if s_account_a = string(c-gl) and s_account_b1 = '' then do:
               ...
               v-tmpl = "jou0040".
               v-param = string(v-sum) + vdel + string(v-crc) + vdel + s_account_b1 + vdel + entry(v-crc,trim(sysc.chval)) + vdel + v-glrem + vdel +
                                     "1" + vdel + "1" + vdel + "4" + vdel + v-knp .
            end.
