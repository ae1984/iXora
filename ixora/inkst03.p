...
v-text = ":77E:FORMS/P03/" + string(year(g-today) mod 1000,'99') + string(month(g-today),'99') + string(day(g-today),'99') + entry(1, string(time, "hh:mm"), ":") + entry(2, string(time, "hh:mm"), ":") + "/Помещение инк. распоряжений в картотеку".
put stream mt400 unformatted v-text skip.
...
for each t-inc where t-inc.vo <> '07' and t-inc.vo <> '09' no-lock:
    if v-bin then v-text = "//07/" + t-inc.bin + "/" + string(t-inc.iik,'x(20)') + "/" + t-inc.crc + "/" + replace(trim(string(t-inc.sum, ">>>>>>>>>>>>9.99")), ".", ",") + "/" + string(t-inc.num) + "/" + t-inc.ref + "/" + t-inc.stat2.
    else v-text = "//07/" + t-inc.jss + "/" + string(t-inc.iik,'x(20)') + "/" + t-inc.crc + "/" + replace(trim(string(t-inc.sum, ">>>>>>>>>>>>9.99")), ".", ",") + "/" + string(t-inc.num) + "/" + t-inc.ref + "/" + t-inc.stat2.
    put stream mt400 unformatted v-text skip.
