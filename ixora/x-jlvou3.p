...
/* x-jlvou3.p
  24.12.02   Вывод названия клиента и назначение платежа   
*/
...
for each jl of jh use-index jhln break by jl.crc by jl.ln:
  ...
  if jl.acc <> '' then  do:
      ...
      vname = trim(trim(cif.prefix) + " " + trim(cif.name)).
  end.
  if jl.dc = "C" then
    disp jl.ln jl.gl gl.sname jl.acc vname format 'x(20)'
         crc.code xamt xco
         with down width 132 frame jlprt no-label no-box.
  else 
    disp jl.ln jl.gl gl.sname jl.acc vname format 'x(20)'
         crc.code xamt xco jl.rem[1] format 'x(30)'
         with down width 132 frame jlprt no-label no-box.
    if last-of(jl.crc) then do:
        put vcha2 space(20) xdam crc.code skip vcha3 space(20) xcam crc.code skip.
        xcam = 0. xdam = 0. 
    end.
