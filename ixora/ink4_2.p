...
/* блок объявления переменных ->*/
    ...
    def var v-bnfbin         as char      no-undo.
    ...
    def frame getlock
        ...
        t-aashist1.nkbin format "x(12)" label  "БИН налог. управления  " validate((chk12_innbin(t-aashist1.nkbin)) ,'Неправильно введён БИН/ИИН') skip
    ...
    def frame getjust
        ...
        aas.binben  format "x(12)" label "БИН бенефициара        " validate((chk12_innbin(aas.binben)) ,'Неправильно введён БИН/ИИН') skip
    ...
    def frame get_hist1
        ...
        t-aashist1.binben  format "x(12)" label  "БИН  бенефициара       "  validate((chk12_innbin(t-aashist1.binben)) ,'Неправильно введён БИН/ИИН') skip
    ...
    def frame getlist1
        ...
        aas.nkbin format "x(12)" label "БИН налог. управления  " validate((chk12_innbin(aas.nkbin)) ,'Неправильно введён БИН/ИИН') skip
    ...
    def frame getlist33
        ...
        t-aashist1.nkbin format "x(16)"     label           "БИН налогового управления     " validate((chk12_innbin(t-aashist1.nkbin)) ,'Неправильно введён БИН/ИИН') skip
...
on help of aas.bnf in frame getlist1 do:
   ...
   {itemlist.i
       ...
       &where = " true "
       &flddisp = " t-ln.bin label 'БИН' format 'x(12)'
   ...
   if avail taxnk then assign aas.nkbin = taxnk.bin  aas.bnf = taxnk.name.
   display aas.nkbin aas.bnf with frame getlist1.
end.
...
/*
on help of aas.nkbin in frame getlist1 do:
   str_p = "".
   find first taxnk use-index bin1 no-lock no-error.
   if avail taxnk where taxnk.bin <> "" then do:
       for each taxnk no-lock use-index bin1:
           str_p = str_p + string (taxnk.bin) + "|".
       end.
       str_p = SUBSTR (str_p, 1, LENGTH(str_p) - 1).
       run sel ("Выберите БИН налогового управления", str_p).
       i-ind = 0.
       for each taxnk use-index bin1 no-lock :
           i-ind = i-ind + 1.
       ...
       display aas.nkbin with frame getlist1.
   end. else do:
   ...
   repeat:
       ...
       if avail aaa then do:
           if v-dep = 1 then do:
               ...
               if avail cif then message "Наименование клиента:" cif.name  skip "БИН клиента:" cif.bin view-as alert-box question buttons ok title "Визуальный контроль".
               ...
               do transaction on error undo, return:
                   ...
                   update  aas.regdt aas.docprim aas.fnum aas.docdat aas.bnf /*aas.nkbin*/ aas.docnum with frame getlist1.
                   ...
                   displ aas.regdt aas.docprim aas.fnum aas.docdat aas.bnf aas.nkbin aas.docnum  aas.payee aas.kbk with frame getlist1.
                   ...
                   if lookup (aaa.lgr , "138,139,140,143,144,145") > 0 then do:
                       run mail("DPC@fortebank.com", "METROCOMBANK <abpk@metrocombank.kz>", "ИР",
                           "ИР №" + aas.fnum + " от " + string(aas.docdat,"99/99/9999") + " на " + aas.docprim + ". \n Зарегистрировал " + aas.who + " по счету " + aas.aaa + ", ИИН " + cif.bin + ", дата " + string(g-today,"99/99/9999")
                           , "1", "", "").
           ...
           if v-dep = 2 then do:
               ...
               do transaction on error undo, return:
                   ...
                   if v-mn begins "423" then update aas.regdt aas.docprim aas.fnum aas.docdat aas.bnf aas.binben aas.iikben aas.bicben with frame getjust.
                   else update aas.regdt aas.docprim aas.fnum aas.docdat aas.bnf aas.binben aas.iikben aas.bicben with frame getjust1.
           ...
           if v-dep = 3 then do:
               ...
               if v-dp3 = 1 then do:
                  ...
                  do transaction on error undo, return :
                     ...
                     update aas.regdt aas.docprim aas.fnum aas.docdat aas.bnfname aas.binben aas.bicben aas.bankben aas.docnum aas.iikben aas.knp aas.payee with frame getother1.
               ...
               if v-dp3 = 2 then do:
                   ...
                   do transaction on error undo, return :
                       ...
                       update aas.regdt aas.docprim aas.fnum aas.docdat aas.bnfname aas.binben aas.bicben aas.bankben aas.iikben aas.knp aas.payee with frame getother2.
                       find last bankl where bankl.bic = aas.bicben no-lock no-error.
                       if avail bankl and aas.bicben <> "" then aas.bankben = bankl.name.
                       aas.chkamt = 100000000000.00.
                       aas.sta = 9.
                       ...
                       RUN aas2his.
                       if lookup (aaa.lgr , "138,139,140,143,144,145") > 0 then do:
                           run mail("DPC@fortebank.com", "METROCOMBANK <abpk@metrocombank.kz>", "ПТП",
                               "ПТП №" + aas.fnum + " от " + string(aas.docdat,"99/99/9999") + " на " + aas.docprim + ". \n Зарегистрировал " + aas.who + " по счету " + aas.aaa + ", ИИН " + cif.bin + ", дата " + string(g-today,"99/99/9999")
                               , "1", "", "").
                           message "Вам необходимо выслать сканированную копию данного распоряжения на группу ДПК" view-as alert-box question buttons ok title "Сообщение".
                       end.
                   end. /*transaction*/
               ...
               repeat:
                   ...
                   repeat:
                       ...
                       if v-str matches "*jss*" then do:
                           v-str = replace (v-str, "jss", cif.bin ).
                           next.
                       end.
                       find first cmp no-lock no-error.
...
on choose of bt_Find in frame a2 do:
   ...
   on choose of b-rmz in frame fr1 do:
      ...
      if aas.sta = 4 or aas.sta = 5  then do:
          if aaa.crc = 1 then aasfsum = deci(aas.docprim).
          else aasfsum = exchange(deci(aas.docprim), currrate).
      ...
      if aas.sta = 9 and (aas.knp = '010' or aas.knp = '019' or aas.knp = '017' or aas.knp = '012') then do:
          if aaa.crc = 1 then aasfsum = deci(aas.docprim). else aasfsum = exchange(deci(aas.docprim), currrate).
          if (aaa.cr[1] - aaa.dr[1]) - (v-olds + decimal(d_arsum) + decimal(d_arsummy)) < aasfsum then do:
      ...
      if aas.sta = 9 and (aas.knp = '421' or aas.knp = '423' or aas.knp = '429') then do:
          ...
          if (aaa.cr[1] - aaa.dr[1]) - (v-olds + decimal(d_arsum) + decimal(d_arsummy)) < aasfsum then do:
              ...
              v-rmzsum = deci(aas.docprim).
              if aaa.crc <> 1 then v-rmzsumval = exchange(deci(aas.docprim), currrate).
          end.
      ...
      if aas.sta = 9 and (aas.knp = '421' or aas.knp = '423' or aas.knp = '429') then do:
          ...
          t-sum = 0.
          for each b-aas where b-aas.aaa = v-inkaaa and lookup(string(b-aas.sta), "11,2,4,5,15,6,7,9,16,17") <> 0 or
                                  b-aas.aaa = v-inkaaa and b-aas.mn = "30037" no-lock:
          ...
          if (aaa.cr[1] - aaa.dr[1]) - d_arsum  > 0 then do: /* деньги на счете есть производим оплату инкассовых распоряжений */
              ...
              if aas.sta = 4 or aas.sta = 5  then do:
                  ...
                  if aas.nkbin = "адм. судов" then do:
                      v-bnfiik = aas.iikben.
                      v-bnfbin = aas.binben.
                      v-bicbnf = aas.bicben.
                  end.
              ...
              if aas.sta = 9 and (aas.knp = '010' or aas.knp = '019' or aas.knp = '017' or aas.knp = '012') then do:
                  find first p_f_list where p_f_list.bin = aas.binben  and p_f_list.acnt = aas.iikben and p_f_list.bic = aas.bicben no-lock no-error.
              ...
              if trim(v-bnfbin) = "" then do:
                 message "Отсутствует БИН бенефициара" view-as alert-box.
                 leave.
              end.
              if trim(cif.bin) = "" then do:
                 message "Отсутствует БИН отправителя" view-as alert-box.
                 leave.
              end.
              d-tmpSum = 0.
              d_sum = 0.
              d_sum = aaa.hbal.
              t-sum = 0.
              for each b-aas where b-aas.aaa = v-inkaaa and lookup(string(b-aas.sta), "2,4,5,6,7,9") <> 0 or
                                  b-aas.aaa = v-inkaaa and b-aas.mn = "30037" no-lock:
                  t-sum = t-sum + b-aas.chkamt.
              end.
   ...
   on choose OF bhistory IN FRAME fr1 do:
           ...
           ON CHOOSE OF bdethis IN FRAME fr4 do:
              ...
              if avail buf-t-aashist and avail aaa then do:
                  ...
                  ON CHOOSE OF bdtlst IN FRAME fr5 do:
                     ...
                     if avail buf-t-aashist1 then do:
                         ...
                         if t-aashist1.chgoper = "T" then do: /*приостановлено*/
                            display t-aashist1.fnum t-aashist1.docdat t-aashist1.who t-aashist1.bnf t-aashist1.nkbin t-aashist1.kbk t-aashist1.knp t-aashist1.payee t-aashist1.dtbefore t-aashist1.docnum1 t-aashist1.docdat1 t-aashist1.docprim1 with frame getlist33.
                         end. else if t-aashist1.chgoper = "O" or t-aashist1.chgoper = "X" then do: /*отозвано*/
                            display t-aashist1.fnum t-aashist1.docdat t-aashist1.who t-aashist1.bnf t-aashist1.nkbin t-aashist1.kbk t-aashist1.knp t-aashist1.payee t-aashist1.dtbefore t-aashist1.docnum1 t-aashist1.docdat1 t-aashist1.docprim1 with frame getlist34.
                         end. else do:
                            if t-aashist1.sta <> 9 and t-aashist1.sta <> 15 then
                               display t-aashist1.aaa  t-aashist1.regdt t-aashist1.nkbin t-aashist1.kbk t-aashist1.bnf t-aashist1.fsum t-aashist1.fnum t-aashist1.docdat t-aashist1.knp t-aashist1.payee with frame getlock.
                            else display t-aashist1.aaa t-aashist1.regdt t-aashist1.docprim t-aashist1.fnum t-aashist1.docdat t-aashist1.bnfname t-aashist1.binben t-aashist1.bicben t-aashist1.bankben t-aashist1.iikben t-aashist1.knp t-aashist1.payee with frame get_hist1.
                         end.
                     end.
                  end.
                  for each t-aashist1.
                  ...
                  for each aas_hist where aas_hist.aaa = t-aashist.aaa  and aas_hist.ln = t-aashist.ln and lookup(string(aas_hist.sta), "4,5,6,9,15") <> 0  exclusive-lock USE-INDEX aasprep break by aas_hist.tim DESCENDING:
                      ...
                      t-aashist1.nkbin   = aas_hist.nkbin  .
                      find first taxnk where taxnk.bin = aas_hist.nkbin no-lock no-error.
                      ...
                      t-aashist1.binben   = aas_hist.binben  .
           ...
           for each aas_hist where aas_hist.aaa = aaa.aaa and lookup(string(aas_hist.sta), "4,5,6,9,15") <> 0  NO-LOCK USE-INDEX aasprep break by  aas_hist.ln  by aas_hist.chgdat DESCENDING  by aas_hist.tim DESCENDING:
               if aas_hist.ln <> p-ln then do transaction:
                   ...
                   t-aashist.binben  = aas_hist.binben.
   ...
   ON CHOOSE OF bprint IN FRAME fr1 do:
      ...
      if v-dep = 1 then
          for each aas where lookup(string(aas.sta), "4,5,15,6,7,9") <> 0 and aas.cif = s_FindCIF and aas.ln <> 7777777 /*use-index aaaln*/ exclusive-lock:
              put unformatted aas.cif " " aas.aaa format 'x(20)' " " aas.fnum format 'x(7)' "  " string(aas.docdat,"99/99/99") "  " string(aas.regdt,"99/99/99") "  " aas.nkbin format 'x(15)'   "     " aas.payee skip.
          end.
      if v-dep = 2 then
   ...
   ON CHOOSE OF bdetail IN FRAME fr1 do:
      ...
      find first taxnk where taxnk.bin = aas.nkbin no-lock no-error.
      if  avail taxnk then aas.bnf = taxnk.name.
      find last ofc where ofc.ofc = aas.who no-lock no-error.
      if avail ofc then  do:
         if aas.sta <> 9 and aas.sta <> 15 then display aas.aaa  aas.regdt aas.docprim aas.fnum aas.docdat aas.bnf  aas.nkbin aas.docnum aas.payee aas.kbk aas.knp aas.rgref with frame getlist1.
         else display aas.aaa  aas.regdt aas.docprim aas.fnum aas.docdat aas.bnfname aas.binben aas.bicben aas.bankben aas.iikben aas.knp aas.payee with frame getother1.
      end.
      hide frame getother1.
   end.
   /*Снять ограничение*/
      ...
      if frame-index = 1 or /*frame-index = 3 or*/ frame-index = 2 or v-flv = 1 or v-flv = 2 /*or v-flv = 3*/ then do:
          ...
          if v-yn = True or (v-yn = False and v-flv <> 0) then do:
              ...
              if not aas.contr then do:
                  ...
                  if lookup (aaa.lgr , "138,139,140,143,144,145") > 0 then do:
                       if aas.mn begins "432" then v-tmpstr = "ПТП". else v-tmpstr = "ИР".
                       find first aaa where aaa.aaa = aas.aaa no-lock no-error.
                       find first cif where cif.cif = aaa.cif no-lock no-error.
                       run mail("DPC@fortebank.com", "METROCOMBANK <abpk@metrocombank.kz>", "Отзыв/удаление " + v-tmpstr,
                           "Отзыв/удаление " + v-tmpstr + " №" + aas.fnum + " от " + string(aas.docdat,"99/99/9999") + " на " + aas.docprim + ". \n Зарегистрировал " + aas.who + " по счету " + aas.aaa + ", ИИН " + cif.bin + ", дата " + string(aas.whn,"99/99/9999")
                           , "1", "", "").
                       message "Вам необходимо выслать сканированную копию данного распоряжения на группу ДПК" view-as alert-box question buttons ok title "Сообщение".
                  end.
                             ...
                             ON CHOOSE OF bdtlst IN FRAME fr5 do:
                                ...
                                if avail buf-t-aashist1 then do:
                                    ...
                                    displ t-aashist1.aaa t-aashist1.regdt t-aashist1.fnum t-aashist1.fsum t-aashist1.docdat t-aashist1.bnf
                                          t-aashist1.nkbin t-aashist1.payee t-aashist1.kbk t-aashist1.knp with frame getlock.
                                    hide frame getlock.
                                end.
                             end.
                             for each t-aashist1 .
