...
if yes-no('',' Вы действительно хотите вернуть запись? ') then do:
   ...
   if avail tarifex then do:
      ...
      open query q1 for each tarifex where tarifex.stat = 'h' by tarifex.cif. 
   end.
...
update i no-label
  ...
  if i <> '' then do:     
     find first tarifex where tarifex.cif begins i and tarifex.stat = 'h' no-lock no-error.
     if not avail tarifex then do:
       i = ''.
       message ('Такого клиента здесь нет ! ').
     end.
     else do:   
         reposition q1 to rowid rowid(tarifex) no-error.
         browse b1:refresh().         
     end.
  end. /* if */
end.
...
open query q1 for each tarifex where tarifex.stat = 'h' by tarifex.cif by tarifex.str5. 
