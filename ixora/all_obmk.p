 ...
 * DESCRIPTION
        Отчет обменного пункта о конвертации ин валюты
 * RUN
...
for each jl where jl.sub = 'cif' and jl.crc > 1 and string(jl.gl) begins '2'  and jl.jdt >= v-dtb and jl.jdt <= v-dte no-lock.
   if jl.rem[1] begins 'Комиссия' then next.
   find dealing_doc where dealing_doc.jh = jl.jh or dealing_doc.jh2 = jl.jh no-lock no-error.
   if avail dealing_doc then 
           do:
             ...
             if jl.dc = 'd' and jl.dam <> 0 then run add_to_TS(jl.dam, dealing_doc.rate, crchis.rate[1]).   /*продажа*/
             if jl.dc = 'c' and jl.cam <> 0 then run add_to_TS(jl.cam, dealing_doc.rate, crchis.rate[1]).    /*покупка*/
           end.   
end.
...
find cmp no-lock.
put unformatted cmp.name skip.

put unformatted padc ('ОТЧЕТ', pwidth, ' ') skip.
put unformatted padc ('о конвертации,реконвертации иностранной валюты ', pwidth, ' ') skip.
...
if avail tot_sum then do:
    put "|   |" " Куплено " '|' at 17 ' USD ' 
                            ...
                            '|' at 103 skip.
    put "| 1 |" "  валюты " '|' at 17 ' ' '|' at 30 '|' at 47  '|' at 68 '|' at 86 '|' at 103 skip.
end.
else do:
    put "|   |" " Куплено " '|' at 17 ' USD ' '|' at 30  '|' at 47 '|' at 68 '|' at 86 '|' at 103 skip.
    put "| 1 |" "  валюты " '|' at 17 '|' at 30  '|' at 47 '|' at 68 '|' at 86 '|' at 103 skip.
end.
...
if avail tot_sum then do:
    ...
    put "|   |" " Продано " '|' at 17 ' USD ' '|' at 30 '|' at 47 '|' at 68 '|' at 86 '|' at 103 skip.
    put "| 2 |" "  валюты " '|' at 17 '|' at 30 '|' at 47 '|' at 68 '|' at 86 '|' at 103 skip.
end.
...
if avail tot_sum then   
   put "| 3 |" " Всего  " '|' at 17 ' USD ' '|' at 30 v_amt + ts_dam - ts_cam format '-z,zzz,zzz,zz9.99' '|' at 47 '|' at 68 '|' at 86 '|' at 103 skip.
else
   put "| 3 |" " Всего  " '|' at 17 ' USD ' '|' at 30 '|' at 47 '|' at 68 '|' at 86 '|' at 103 skip.


find tot_sum where tot_sum.ts_crc = 3 no-lock no-error.
if not avail tot_sum then 
   put "|   |" " валюты " '|' at 17 ' EUR ' '|' at 30  '|' at 47 '|' at 68 '|' at 86 '|' at 103 skip.
else 
   put "|   |" " валюты " '|' at 17 ' EUR ' '|' at 30 eur_amt + ts_dam - ts_cam format '-z,zzz,zzz,zz9.99' '|' at 47 '|' at 68 '|' at 86 '|' at 103 skip.
