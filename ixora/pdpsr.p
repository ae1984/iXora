...
if remtrz.sbank = ourbcode then do :
  ...
  if  remtrz.sbank ne ourbank then do:
  ...
  find bankl where bankl.bank = remtrz.sbank no-lock no-error.
  v-bm1 = bankl.name.
  v-bm2 = trim(bankl.addr[1]) + " " + trim(bankl.addr[2]) + " " +
          trim(bankl.addr[3]).
end.
  if remtrz.sbank begins "RKB" then 
   v-kbm = ourbcode.
  else v-kbm = remtrz.sbank .
  v-crccode = crc.code.

/*
  v-km = trim(remtrz.sacc).
  v-km1 = trim(remtrz.dracc).
  if v-km1 = v-km then v-km1 = ''.
*/
...
if index(v-km1,"/") ne 0 then do :
 ...
 if remtrz.rbank = ourbcode or remtrz.rbank begins "RKB" then
 v-kbs = ourbcode.
 else do :
 ...
 if available bankl then do :
     if remtrz.bb[1] begins v-kbs then 
     v-bs1 = substr(remtrz.bb[1],length(v-kbs) + 1) + " " + remtrz.bb[2] 
  + " " + remtrz.bb[3] .
  else 
  v-bs1 = remtrz.bb[1] + " " + remtrz.bb[2] + " " + remtrz.bb[3] .
 end.
 else
 v-bs1 = substr(remtrz.actins[1],2) + " " + remtrz.actins[2] + " "
        + remtrz.actins[3] + " " + remtrz.actins[4] + " ".
