...
def var lop as inte.
def var vans as logi format "да/нет".
def var d as char.
curflag = 1.
...
find last {&head} use-index {&index} no-error.
if available {&head} then do:
  lrec = recid({&head}).
find first {&head} use-index {&index} no-error.
  frec = recid({&head}).
  crec = frec.
  if clin = 0 then do:
    trec = frec.
    ...
    find {&head} where recid({&head}) = trec use-index {&index} no-error.
  end.
  dlin = frame-down({&framename}).
    clear frame {&framename} all no-pause.
end.
else do:
  if {&addcon} then do transaction:
     {&precreate}
     create {&head}.
     {&postcreate}
     {&postadd}
...
{&reframe}
    ...
    repeat v-cnt = 1 to dlin:
      ...
      find next {&head} use-index {&index} no-error.
    ...
    brec = recid({&head}).
  find {&head} where recid({&head}) = crec use-index {&index} no-error.
end.
  ...
  if vans then do:
   ...
   if clin = 1 then do:
      if trec = frec then do:
        ...
        find prev {&head} use-index {&index} no-error.
        nrec = recid({&head}).
      end.
   end.
     do transaction:
      find first {&head} where recid({&head}) = crec exclusive-lock use-index {&index} no-error.
      {&prevdelete}
      delete {&head}.
      {&postdelete}.
     end.
     ...
     if crec <> frec then do:
      ...
      if clin > 1 then do:
       ...
       if blin < dlin then blin = blin + 1.
        ...
        find {&head} where recid({&head}) = brec use-index {&index} no-error.
        find prev {&head} use-index {&index} no-error.
        brec = recid({&head}).
        find {&head} where recid({&head}) = crec use-index {&index} no-error.
       end.
    ...
    if crec <> frec then do:
        ...
        color disp messages {&highlight} with frame {&framename}.
         ...
         repeat v-cnt = 1 to dlin:
          find prev {&head} use-index {&index} no-error.
          if not available {&head} then do:
            find {&head} where recid({&head}) = frec use-index {&index} no-error.
            leave.
          end.
         end.
           trec = recid({&head}).
     ...
     if keyfunction(lastkey) = "RIGHT-END" then do:
       ...
       if crec = lrec then do: bell. next inner. end.
          color disp normal {&highlight} with frame {&framename}.
          find {&head} where recid({&head}) = lrec use-index {&index} no-error.
       if brec = lrec then do:
          down blin - clin with frame {&framename}.
          crec = brec. clin = blin.
          color disp messages {&highlight} with frame {&framename}.
