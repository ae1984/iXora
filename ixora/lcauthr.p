 ...
 * MODULE
        Trade Finance
 * DESCRIPTION
        IMLC - Authorization to Reimburse (MT740)
 * RUN
...
def new shared var s-event    like lcevent.event init 'authr'.
def new shared var s-number   like lcevent.number.
def new shared var s-sts      like lcevent.sts.
def new shared var s-ftitle   as char init ' AUTHORIZATION to REIMBURSE '.
...
{mainlc.i
 ...
 &postfind = " if avail LC then v-lcsts = LC.LCsts.
               ...
               do on error undo,return:
                   v-yes = no.
                   if v-lcsts = 'FIN' then
                   message 'Do you want to create a new Event (Authorization to Reimburse)?' VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO TITLE ' QUESTION !' update v-yes.
                    ...
                    if v-yes then do transaction:
                        ...
                        if avail lcevent then do:
                            if lcevent.sts <> 'FIN' then do:
                                message 'The status of last event (Authorisation to Reimburse number ' + string(lcevent.number) +  ') is not FIN, it is impossible create a new Event!' view-as alert-box error.
                                assign v-yes    = no
                                       s-sts    = lcevent.sts
