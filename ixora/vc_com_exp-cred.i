 ...
 * DESCRIPTION
        Описание - Алгоритмы расчетов по экспортным и импортным контрактам задолжников
 * RUN
        Способ вызова программы, описание параметров, примеры вызова
 * CALLER
        Список процедур, вызывающих этот файл - vccomexpdat.p,vccomcreddat.p,vcrepdexdat.p,vcrepdpldat.p.
 * SCRIPT
...
def var vv-summ as deci init 0.
def var vv-sumdoc1 as deci init 0.
def var vv-sumdoc2 as deci init 0.
def var vv-summa as deci init 0.
def var v-term as date.
def var vv-sum as deci init 0.
def var vv-plus1 as deci init 0.
def var vv-plus2 as deci init 0.
def var vv-sum-dt as deci init 0.
def var v-payret as deci init 0.
def var v-sum_check as deci.
def var v-PayReturn as deci.

/*Экспортные контракты*/
if lookup(trim(vccontrs.cttype),string({&cttype})) > 0 and trim(vccontrs.expimp) = "E" then do: /*Алгоритм расчета п.м. 9.3.6.1*/

    vv-date = date(s-dte - check_term(vccontrs.ctterm)).
    if v-sumgtd + v-sumakt > v-sumplat then do:
        vv-sum-dt = (v-sumgtd + v-sumakt) - v-sumplat.
        vv-sumdoc1 = 0. vv-sumdoc2 = 0.
        if (v-sumgtd + v-sumakt) - v-sumplat > {&limitexp} then do:
            vv-summa = 0.
            if v-sumplat <> 0 then do:
                ...
                if avail vcdoc then do:
                    ...
                    vcdocs.dndate < s-dte by dndate:
                        ...
                        vv-sumdoc1 = vv-sumdoc1 + v-payret.
                    end.
                    vv-sum-dt = vv-sum-dt - vv-sumdoc1.
                    for each vcdocs where vcdocs.contract = vccontrs.contract and lookup(trim(vcdocs.dntype),v-docsgtd) > 0 and
                    vcdocs.dndate < s-dte by dndate descending:
                        v-sum_check = konv2usd_docs(vcdocs.sum,vcdocs.pcrc,vcdocs.contract,vcdocs.dntype,vcdocs.dndate,recid(vcdocs)).
                    ...
                    if avail vcdoc then do:
                        ...
                        if avail vcdocs then do:
                            ...
                            if vv-plus2 > {&limitexp} then do:
                                ...
                                for each vcdocs where vcdocs.contract = vccontrs.contract and lookup(vcdocs.dntype, v-docsgtd) > 0
                                and vcdocs.dndate > v-term and vcdocs.dndate < vv-date by dndate:
                                    v-sum_check = konv2usd_docs(vcdocs.sum,vcdocs.pcrc,vcdocs.contract,vcdocs.dntype,vcdocs.dndate,recid(vcdocs)).
                        ...
                        for each vcdocs where vcdocs.contract = vccontrs.contract and lookup(trim(vcdocs.dntype),v-docsgtd) > 0 and
                        vcdocs.dndate >= v-term and vcdocs.dndate < vv-date by dndate descending:
                            v-sum_check = konv2usd_docs(vcdocs.sum,vcdocs.pcrc,vcdocs.contract,vcdocs.dntype,vcdocs.dndate,recid(vcdocs)).
                ...
                if avail vcdoc then do:
                    ...
                    vcdocs.dndate < s-dte by dndate:
                        ...
                        vv-sumdoc2 = vv-sumdoc2 + v-payret.
                    end.
                    vv-summ = vv-sumdoc2.
                    for each vcdocs where vcdocs.contract = vccontrs.contract and lookup(trim(vcdocs.dntype),v-docsgtd) > 0 and
                    vcdocs.dndate < s-dte by dndate:
            ...
            if deci((s-dte - 1) - vv-term) > check_term(vccontrs.ctterm) and vv-summa - v-PayReturn > {&limitexp}
            then do:
                v-workcond = true.
            end.
        end.
...
/*Импортные контракты*/
else if lookup(trim(vccontrs.cttype),string({&cttype})) > 0 and trim(vccontrs.expimp) = "I" then do: /*Алгоритм расчета п.м. 9.3.6.4*/

    if v-sumgtd + v-sumakt < v-sumplat then do:
        vv-date = date(s-dte - check_term(vccontrs.ctterm)).
        vv-sum-dt = v-sumplat - (v-sumgtd + v-sumakt).

        if v-sumplat - (v-sumgtd + v-sumakt) > {&limitimp} then do:
            vv-summa = 0.
            if v-sumgtd <> 0 or v-sumakt <> 0 then do:
                ...
                vv-summa = vv-summa - vv-sum.
            end. /*if v-sumgtd <> 0 or v-sumakt <> 0*/
        ...
        end. /*if konv2usd(v-sumplat - (v-sumgtd + v-sumakt), vccontrs.ncrc, s-dte - 1) > {&limitimp}*/
    end. /*if v-sumgtd + v-sumakt < v-sumplat*/
