...
case v-sel2:
    when 1 then run sel1.
    when 2 then run sel2.
    when 3 then do: run sel1. run sel2. end.
    when 4 then do: if IsKass(v-id) then run sel3. else run sel4. end.
end.
    ...
    for each txb.jh where txb.jh.jdt >= dt1 and txb.jh.jdt <= dt2 and txb.jh.sts = 6 and
        ...
        if not IsKass(v-who) /* and chkbuh(v-who)*/ then do: /* менеджеры */
            ...
            if chdoc(txb.jh.jh,v-txb,v-party) then do:
                /*find first wrk1 where wrk1.txb = v-txb and wrk1.id = v-who no-lock no-error.
               if not available wrk1 then do:*/
                    create wrk1.
                    wrk1.txb = v-txb.
                    wrk1.fil = v-fil-cnt.
                    wrk1.podrname = "Операционный отдел".
                    wrk1.id = v-who.
                    wrk1.fio = v-whoname.
                    wrk1.f = string(txb.jh.jh).
        ...
        /*if IsKass(v-who) then do:*/ /* кассиры */
            ...
            if v-in1 and v-100100 then do:
                ...
                if not available wrk1 then do:*/
                    ...
                    wrk1.txb = v-txb.
                    wrk1.fil = v-fil-cnt.
                    wrk1.podrname = "Отдел кассовых операций".
                    wrk1.id = v-who.
                    wrk1.fio = v-whoname.
                    wrk1.f = string(txb.jh.jh).
                    wrk1.doc = txb.jh.party.
