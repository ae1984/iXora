 ...
 * BASES
        BANK COMM
 * CHANGES
        07.03.2012 damir - добавил keyord.i,переход на новые форматы...
        27.04.2012 damir - закоментил return "1".
*/
...
def new shared var date_from as date.
def new shared var date_to   as date.
...
/*message stformat.executor view-as alert-box.*/
bar = "|--------".
...
if g-batch = false and total_page > 15 then do:
    ...
    if prt = no then do transaction:
        ...
        if v-noord = no then unix silent value ( partkom ).
        return "0".
    end.
end.
if v-noord = no then unix silent value ( partkom ).
