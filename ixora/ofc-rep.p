 ...
 * MENU
        Пункт меню
 * AUTHOR
        01/04/2011 k.gitalov
 * BASES
        BANK COMM
 * CHANGES
        21/06/2011 madiyar - добавил проверку на наличие у пользователя пакета главбуха p00121
        26/07/2011 k.gitalov - Изменения по ТЗ 1013
        04/05/2012 evseev - изменил путь к логотипу
        15.06.2012 Lyubov - добавила валюту ZAR
        13.07.2012 Lyubov - перекомпиляция
        04.01.2013 evseev - замена RNN на IDN
        11/06/2013 Luiza  - ТЗ 1446 Добавление столбца "Документ"
        10/07/2013 Luiza - ТЗ 1948
        18/07/2013 Luiza - ТЗ 1967
        09.08.2013 damir - Внедрено Т.З. № 2021.
*/
...
def var buxlist as char. /*init "id00205,id00247,id00195,id00214,id00374,id00516,id00726,id00227,id00563,id00528,id00387,id00231,id00918,id00801,id00207,id00443,id00303,id00412,id00182,id00616,id00775,id00200,id00193,id00092".*/
find first bookcod where bookcod.bookcod = 'ofcrep'
                     and bookcod.code    = 'ofcrep'
                     no-lock no-error.
if not avail bookcod or trim(bookcod.name) = "" then do:
    message "В справочнике <bookcod> код <ofcrep> отсутствует список id имеющих доступ к отчету!~nОбратитесь к администратору АБС!" view-as alert-box error.
    return.
end.
buxlist = bookcod.name.
...
if LOOKUP(g-ofc,buxlist) = 0 and not(chkbuh(g-ofc)) then do:
  v-ofc = g-ofc.
  find first ofc where ofc.ofc = v-ofc no-lock no-error.
  if avail ofc then do:
   ...
   for each jh where (jh.who = ofc.ofc or jh.who = "superman" ) /*and jh.sts = 6 and not jh.party begins "Storn"*/ and jh.jdt >= dt1 and jh.jdt <= dt2 no-lock:
     create tmp-jh.
...
/**************************************************************************************/
    run sel1("Тип отчета", "По исполнителю|По подразделениям|По всему филиалу").
    r-type = return-value.
    ...
    case r-type:
     /**************************************************************************************/
      ...
      /**************************************************************************************/
      when "По подразделениям" then do:

       for each ppoint no-lock :
        v-departs = v-departs + ppoint.name + "|".
       end.
       v-departs = substring ( v-departs , 1 , length(v-departs) - 1 ) .

       run sel1("Выбор подразделения", v-departs).
       v-departs = return-value.

        if v-departs <> "По подразделениям" then do:
         find first ppoint where ppoint.name = v-departs no-lock.
         v-dep = ppoint.dep.
         v-repwho = v-departs.

         for each jh where  jh.depart = v-dep /*and jh.sts = 6 and not jh.party begins "Storn"*/ and jh.jdt >= dt1 and jh.jdt <= dt2 no-lock:
          if not IsKass(jh.who) then
          do:
           create tmp-jh.
           buffer-copy jh to tmp-jh.
          end.
        ...
        else do: message "Необходимо выбрать подразделение!" view-as alert-box. undo. end.
      end.
      /**************************************************************************************/
      when "По всему филиалу" then do:
       find first cmp no-lock no-error.
       v-repwho = cmp.name.

         for each jh where  /*jh.sts = 6 and not jh.party begins "Storn" and*/ jh.jdt >= dt1 and jh.jdt <= dt2 no-lock:
          if not IsKass(jh.who) then
          do:
           create tmp-jh.
           buffer-copy jh to tmp-jh.
 ...
 for each tmp-jh by tmp-jh.jdt:
  ...
  case TypePay:
    ...
    when 1 then do: /*Платежи в тенге*/
         ...
         if tmp-jh.party begins "jou" or tmp-jh.party begins "rmz" then wrk.docno  = tmp-jh.party.
    end.
   ...
   if TypePay = 1 or TypePay = 2 or TypePay = 3 or TypePay = 4 then do:
         ...
         if avail remtrz then do:
           ...
           */
           ...
           if wrk.c-who = wrk.r-who then wrk.c-who = "".
           if remtrz.source = "IBH" and remtrz.rwho = "superman" and wrk.c-who = "" and wrk.r-who <> remtrz.cwho then wrk.c-who = remtrz.cwho.
   ...
   if TypePay = 5 or TypePay = 6 or TypePay = 7 then do:
       ...
       if avail dealing_doc then do:
          ...
          find first jl where jl.jh = tmp-jh.jh and jl.acc <> "" no-lock no-error.
          if not avail jl then find first jl where jl.jh = tmp-jh.jh and jl.dc = "d" no-lock no-error.
          find trxcods where trxcods.trxh = jl.jh and trxcods.trxln = jl.ln and trxcods.codfr = "spnpl" no-lock no-error.
          if available trxcods then wrk.v-knp = trxcods.code.
          if wrk.v-knp = "" then wrk.v-knp = GetKNP(tmp-jh.sub, tmp-jh.ref).
...
def var ZAR-summ as deci init 0.
...
function PrintSumm returns integer (input val as char, input summ as deci).
   put stream rep unformatted
 "<tr style=""font:bold;font-size:10""bgcolor=""#C0C0C0"">" skip
    "<td colspan=""7"">ИТОГО " val ":</td>" skip
  ...
  for each wrk break by wrk.ind by wrk.sort-crc  /*wrk.v-crc*/ /*wrk.v-date*/:
   if wrk.ind = 8 or wrk.ind = 9 then next.
    if first-of(wrk.ind) then do:
     ...
     if ZAR-summ > 0 then PrintSumm("ZAR",ZAR-summ).

     KZT-summ = 0. USD-summ = 0. EUR-summ = 0. RUB-summ = 0. GBP-summ = 0. SEK-summ = 0. AUD-summ = 0. CHF-summ = 0 . ZAR-summ = 0.
     ...
     case wrk.v-crc:
       ...
       when "ZAR" then do: ZAR-summ = ZAR-summ + wrk.v-summ.
       end.
       otherwise do:
         message "Неизвестная валюта! " wrk.oper-type wrk.v-crc wrk.v-trx view-as alert-box.
       end.
  ...
  KZT-summ = 0. USD-summ = 0. EUR-summ = 0. RUB-summ = 0. GBP-summ = 0. SEK-summ = 0. AUD-summ = 0. CHF-summ = 0. ZAR-summ = 0.
   ...
   for each wrk where wrk.ind = 8 or wrk.ind = 9 break by wrk.ind by wrk.gl-sort by wrk.sort-crc /*wrk.v-crc*/ /*wrk.v-date*/:
    ...
    if first-of(wrk.ind) then do:
     ...
     if ZAR-summ > 0 then PrintSumm2("ZAR",ZAR-summ).

     KZT-summ = 0. USD-summ = 0. EUR-summ = 0. RUB-summ = 0. GBP-summ = 0. SEK-summ = 0. AUD-summ = 0. CHF-summ = 0 . ZAR-summ = 0 .

     put stream rep unformatted "<tr style=""font:bold;font-size:10""><td colspan=""12"" > <strong><em>" wrk.oper-type " </em></strong> </td></tr>" skip.

    end.

    if first-of(wrk.gl-sort) and wrk.ind = 8 then do:
     put stream rep unformatted "<tr style=""font:bold;font-size:10""><td colspan=""12"" > <strong><em>" wrk.gl-sort-name " </em></strong> </td></tr>" skip.
    end.
