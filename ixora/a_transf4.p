...
form
    ...
    v_docdt    label " Дата выдачи      " format "99/99/9999" help " Ведите дату выдачи документа удостов. личости в формате дд/мм/гггг " validate(trim(v_docdt) <> "", "Заполните дату выдачи документа") skip
    v_docdtf   label " Срок действия    " format "99/99/9999" help " Ведите срок действия документа удостов. личости в формате дд/мм/гг " /*validate(trim(v_docdtf) <> "", "Заполните срок действия документа")*/ skip
...
on help of v-joudoc in frame f_main do:
    if v-ek = 1 then run a_help-joudoc1 ("TR4" + string(v-select4)). else run a_help-joudoc1 ("RT4" + string(v-select4)).
...
if new_document then do:  /* создание нового документа  */
    ...
    v_title = " БЫСТРЫЕ ПЕРЕВОДЫ (возврат перевода)" + v-sys.
    ...
    if v_u = 2 then do:       /* update */
        ...
        if available joudoc then do:
            ...
            if available joudop then do:
                if joudop.type <> "TR4" + string(v-select4)  and joudop.type <> "RT4" + string(v-select4) then do:
                    message substitute ("Документ не относится к типу возврат быстрых перевода ") + v-sys view-as alert-box.
                    return.
                end.
                if v-ek = 1 and joudop.type = "RT4" + string(v-select4) then do:
                    message substitute ("Документ создан для ЭК ГК 100500") view-as alert-box.
                    return.
                end.
                if v-ek = 2 and joudop.type = "TR4" + string(v-select4) then do:
...
procedure import_doc:
 /* 274596948 */
    if Doc:TransferStatus <> 6 then do:
   message "Неверный тип документа.~nДолжен быть документ на возврат!" view-as alert-box.
   return.
  end.
...
procedure Create_transaction:
    ...
    for each sendod no-lock.
        ...
        "Добрый день!\n\n Необходимо отконтролировать возврат быстрого перевода \n Сумма: " + string(v_sum) +
        "  " + v-crc_val + "\n документ :" + v-joudoc + "\n создал :" + g-ofc + "\n " + string(g-today) + "  " +
        string(time,"HH:MM"), "1", "","" ).
    end.
    hide all.
...
procedure save_date:
    ...
    joudoc.benName = v_lname1 + "," + v_name1 + "," + v_mname1 + "," + v_countr1 + "," + v_rez1.
    ...
    if v-ek = 1 then joudop.type = "TR4" + string(v-select4). else joudop.type = "RT4" + string(v-select4).
