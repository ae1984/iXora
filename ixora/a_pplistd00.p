 ...
 * MODULE
 * DESCRIPTION
        Список удаленных длительных платежных поручений для ЦО
 * BASES
        BANK COMM
 * RUN
...
DEFINE BROWSE b-list QUERY q-list
       ...
       t-pplist.crc    label "Вал" format "z9"
       ...
       t-pplist.con    label "Контрол" format "Есть/Нет"
       with 29 down overlay no-label title "Cписок удаленных платежных поручений ".
...
on "enter" of b-list in frame f-list do:
    clear frame f_main.
    find first t-ppout where t-ppout.txb = t-pplist.txb and t-ppout.id = t-pplist.id no-lock no-error.
