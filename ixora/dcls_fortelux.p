...
fname1 = "acrfortelux" + substring(string(g-today),1,2) + substring(string(g-today),4,2) + ".txt".
...
for each lgr where lgr.led = "TDA" :
    if lookup(lgr.lgr, "A38,A39,A40") = 0 then next.
...
for each lgr where lgr.led = "TDA" and lookup(lgr.lgr, "A38,A39,A40") > 0 :
    vcnt = 0.
    vtotaip = 0.
    vtotaipc = 0 .
    account:
    for each aaa where aaa.lgr eq lgr.lgr break by aaa.crc:
        ...
        if lgr.intcal = "D" then do:
           ...
           if ev-date <> ? then aaa.cltdt = ev-date. else aaa.cltdt = g-today.
           ...
           if DoChgRate then do:
              ...
              run savelog("dcls_fortelux" , "279.").
              run CalcInterests(v-intday1, v-intrat1, v-base1).
              run savelog("dcls_fortelux" , "281.").
              ...
              if v-intday2 > 0 then do:
                 run savelog("dcls_fortelux" , "290.").
                 run CalcInterests(v-intday2, v-intrat2, v-base2).
                 run savelog("dcls_fortelux" , "292.").
              end.
           end. else if DoCap then do:
              v-base1 = IntBase().
              v-intrat1 = aaa.rate.
              run savelog("dcls_fortelux" , "297.").
              run CalcInterests(v-intday1, v-intrat1, v-base1).
              run savelog("dcls_fortelux" , "299.").
              run CapInterests.
              /*Новые сроки при пролонгации*/
              /*find last acvolt where acvolt.aaa = aaa.aaa exclusive-lock no-error.
              if avail acvolt then do:
                 if ev-date = date(acvolt.x3) - 1 then do:
                    acvolt.x1 = string(ev-date + 1).
              ...
              v-base1 = IntBase().
              v-intrat1 = aaa.rate.
              run savelog("dcls_fortelux" , "317.").
              if v-intday2 > 0 then  run CalcInterests(v-intday2, v-intrat1, v-base1).
              run savelog("dcls_fortelux" , "319.").
           end. else do:
              aaa.cltdt = g-today.
              v-base1 = IntBase().
              v-intrat1 = aaa.rate.
              run savelog("dcls_fortelux" , "324.").
              run CalcInterests(s-intday, v-intrat1, v-base1).
              run savelog("dcls_fortelux" , "326.").
           end.
        end. else do:
           aaa.cltdt = g-today.
           next account.
