 ...
 * DESCRIPTION
        Формирование письма для рассылки в бухгалтерии работодателей клиентов-должников
 * RUN
 ...
 * MENU
        4-14-6
 * AUTHOR
        23/06/2004 madiar - скопировал из pkletterjb1.p с небольшими изменениями
 * CHANGES
        06/07/2004 madiyar - добавил БИК в реквизиты банка
        27/07/2004 madiyar - исправил опечатку в тексте письма
...
if p-letternom = "" then do:
  run pknewletter (s-ourbank, s-paramnom, replace(s-codemask, "*", "jb2"), p_newved, s-lon, lon.rdt, output p-letternom).
...
if p-newfile then do:
  output stream rep to value(s-filename + "jb2.html").
  output stream rep close.
end.

output stream rep to value(s-filename + "jb2.html") append.
...
put stream rep unformatted
    "<P align=""justify"">"
      ...
      "N&nbsp;" v-dognom " о предоставлении потребительского кредита от " v-dogdtstr "&nbsp;года. По состоянию на " v-letterdtstr "&nbsp;года на протяжении "
      p-dolgday "&nbsp;дней он(-а) не выполняет своих обязательств по погашению кредита и вознаграждения, в связи с чем его(ее) задолженность по Договору составляет "
      v-dolgsum " (" v-dolgsumwrd ")&nbsp;" v-crccode ", из них:<BR>" skip
      ...
      "а так же сумма неустойки (штрафных санкций, пени) " v-dolgpena " (" v-dolgpenawrd ")&nbsp;тенге.<BR>" skip
      "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;В обеспечение исполнения обязательств по вышеуказанному договору " v-sname " предоставил(-а) свою ежемесячную заработную плату, "
      "получаемую в Вашей организации. В связи с этим просим Вас оказать содействие Вашему сотруднику в погашении его(ее) обязательств путем ежемесячного перечисления "
      "бухгалтерией Вашей организации части заработной платы данного сотрудника в счет погашения его(ее) задолженности перед Банком по нижеуказанным реквизитам:<BR><BR>" skip
