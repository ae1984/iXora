 ...
 * CHANGES
        ...
        18.01.2004 nadejda - добавлен код региона клиента в соответствии с новым форматом отчета, 
                             выход при критических ошибках
        05.07.2004 saltanat - добав. new shared для s-contrstat
        19.11.2004 saltanat - формат поля LICENCE увеличен до 20-ти знаков.	
*/
...
def var v-err as logical init no.
...
if v-newmsg and length(v-psdnnum) < 19 then do:
  v-ans = yes.
  v-err = yes.
  put stream rep-err unformatted " Короткий номер ПС" skip.
end.

if v-newmsg and length(v-psdnnum) > 30 then do:
  v-ans = yes.
  v-err = yes.
...
if v-newmsg and (not v-psdnnum matches "..\/........\/...\/0\.*" or (substr(v-psdnnum, 17, 2) <> "0.")) then do:
  v-ans = yes.

  v-psdnnum = replace (v-psdnnum, ",", ".").
  if substr(v-psdnnum, 2, 1) = "/" then v-psdnnum = substr (v-psdnnum, 1, 1) + substr (v-psdnnum, 3, 1) + substr (v-psdnnum, 4).

  if substr(v-psdnnum, 17, 2) <> "0." then do:
    v-psdnnum = replace (v-psdnnum, ".", "").
    v-psdnnum = substr (v-psdnnum, 1, 16) + "0." + substr (v-psdnnum, 17).
  end.

  put stream rep-err unformatted " Неверный формат номера ПС " vcps.dnnum " -> " v-psdnnum skip.
end.
...
if v-ans then do:
  if v-err then v-str = " Были обнаружены критические ошибки данных!~n Сообщение не будет сформировано!". 
           else v-str = " Были обнаружены ошибки данных! ". 
  message skip v-str skip(1) view-as alert-box buttons ok title " ВНИМАНИЕ ! ".
  run menu-prt (v-fileerr).
end.


/* если обнаружены критические ошибки - сообщение не формировать */
if v-err then return.



{vcmsgparam.i &msg = "105"}
...
if avail vcps then do:
  ...
  for each vcps where vcps.contract = vccontrs.contract and vcps.dndate >= v-dtb and vcps.dndate <= v-dte no-lock break by vcps.dntype by vcps.dndate by vcps.dnnum:
    if vcps.dntype = "01" then v-text = v-psdnnum.
                          else v-text = "".
...
if avail t-rslc then 
  ...
  for each t-rslc where t-rslc.lcnum <> "":
    v-text = "/LICENCE/" + substr(t-rslc.lcnum, 1, 20).
    put stream rpt unformatted v-text skip.
