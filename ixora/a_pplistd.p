 ...
 * MODULE
 * DESCRIPTION
        Список удаленных длительных платежных поручений
 * BASES
        BANK COMM
 * RUN
...
for each ppout where ppout.del no-lock.
    create t-pplist.
...
on "enter" of b-list in frame f-list do:
    clear frame f_main.
    v-id = t-pplist.id.
    find first ppout where ppout.id = v-id and ppout.del no-lock no-error.
