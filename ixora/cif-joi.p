 ...
 * CHANGES
        ...
        31.07.08 galina дата ОКПО сохраняется
        14/04/2009 galina - ищем НК для РНН по первым 4-м цифрам
        19/11/2009 madiyar - проверка БИН/ИИН на контрольный разряд
        06/08/2010 aigul - добавление информации о клиентах связанных с банком особыми отношениями
        19.04.2011 damir - выходила ошибка запись отсутствует, исправлено.
...
/*БИН*/ def var v-bin as character.
...
&start = "find cif where cif.cif = s-cif.
          find last cif-mail where cif-mail.cif = cif.cif exclusive-lock no-error.
 ...
 if cif.jss ne '' then do:
       find sub-cod where sub-cod.sub = 'cln' and sub-cod.acc = s-cif and sub-cod.d-cod = 'rnnsp' and sub-cod.ccode <> 'msc' no-lock no-error.
       if avail sub-cod then do:
           ...
           find codfr where codfr.codfr = 'rnnsp' and codfr.code = substr(cif.jss, 4, 1) use-index cdco_idx no-lock no-error.
           if avail codfr then v-tax2 = codfr.name[1].
       end.
 end.
 else v-tax2 = ''.
...
v-jss[i - 1] = substring(v-jss[i - 1], 1, index(v-jss[i - 1], '&') - 1).
    end.
    else v-jss[i] = ''.
 end.
 if index(v-jss[10], '&') > 0
...
&posupdt = " do on error undo, retry:
 ...
 if trim(cif.bin) = '' then update cif.bin validate((chk12_innbin(cif.bin)) or (cif.bin = ''),'Неправильно введён БИН/ИИН') with frame cif.
 else displ cif.bin with frame cif.
 update cif.jss with frame cif.
 if cif.jss ne '' then do:
      run rnnchk( input cif.jss,output v-rnn).
      if v-rnn then do:
          message 'Неверный ключ РНН ! '. pause.
      end.
      find sub-cod where sub-cod.sub = 'cln' and sub-cod.acc = s-cif and sub-cod.d-cod = 'rnnsp' and sub-cod.ccode <> 'msc' no-lock no-error.
      if avail sub-cod then do:
           find codfr where codfr.codfr = 'rnnsp' and codfr.code = sub-cod.ccode no-lock no-error.
...
v-jss[7] cif-mail.mail with frame cif.
cif.ssn = v-tax1.
