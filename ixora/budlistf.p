 ...
 * DESCRIPTION
        бюджетные позиции фильтр по подраздел
 * RUN
...
select budget.gl, budget.des, budget.txb, sum(budget.plan[v-month]),sum(budget.fact[v-month]),sum(budget.fact[v-month]),sum(budget.overdraft[v-month]) from budget where budget.year =  v-year and budget.txb begins v-txb group by budget.gl.
open g2.
repeat:
...
def frame fmain
          v-txbname          at x 1 y  1 label "Фильтр по деп/филиалу " view-as text format "x(62)"
          fb1   at x 1   y 8 help " "
...
on "cursor-right" of browse fb1 do:
   close query fq2.
   open query fq2 for each budget where budget.year = v-year and budget.gl = ft-gl.gl and budget.txb begins v-txb no-lock use-index budyear.
