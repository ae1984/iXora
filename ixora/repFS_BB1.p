 ...
 * MODULE
        Кредитный модуль
 * DESCRIPTION
        Расшифровка кредитного портфеля для ген бухгалт
 * RUN
 ...
 * AUTHOR
        26/11/20012 Luiza создан по примеру lnaudit1
 * CHANGES
*/

def input parameter d1 as date no-undo.
def shared var v-reptype as integer no-undo. /* 5 - все */
...
do i = 1 to num-entries(lst_grp):
    ...
    for each txb.lon where txb.lon.grp = v-grp no-lock:
        ...
        run lonbalcrc_txb('lon',txb.lon.lon,d1,"2,9,22,23,49",no,txb.lon.crc,output proc).
        ...
        */
        ...
        run lonbalcrc_txb('lon',txb.lon.lon,d1,"2,9,49",no,txb.lon.crc,output wrk.nach_prc).
