 ...
 * MODULE
        Валютный контроль
 * DESCRIPTION
        Отчет по безвозмездным платежам свыше 10000т.
 * RUN
        15-4-6
 * CALLER
 * SCRIPT
...
def temp-table rmztmp
    ...
    field rmztmp_coun      as char
    field rmztmp_goal      as char.
...
if remtrz.fcrc = 1 
  ...
  if not avail sub-cod or sub-cod.ccode = 'msc' or sub-cod.ccode = 'KZ' then next.
  find codfr where codfr.codfr = 'iso3166' and codfr.code = sub-cod.ccode no-lock no-error.
  if not avail codfr then next.
  v-country = codfr.name[1].
  ...
  assign rmztmp.rmztmp_aaa  = aaa.aaa      
         ...
         rmztmp.rmztmp_coun = v-country
         rmztmp.rmztmp_goal = remtrz.detpay[1] + ' ' + remtrz.detpay[2] + ' ' + remtrz.detpay[3] + ' ' + remtrz.detpay[4].      
...
put stream m-out unformatted 
   ...
   "<TR><TD><BR>" skip
     "<TABLE width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"" align=""center"">" skip
       "<TR valign=""top"">" skip
         "<TD align=""left""><FONT size=""2"">" skip
                  "Национальный Банк Республики Казахстан " skip
         "</FONT><BR><BR>"
         "</TD>" skip
       "</TR>"
     "</TABLE></TR>" skip.
...
put stream m-out unformatted 
  "<TABLE width=""90%"" border=""0"" cellspacing=""0"" cellpadding=""3"" align=""center"" >" skip
    ...
    "<P align =""justify""><FONT size=""3"">"       skip.
       ...
       put stream m-out unformatted
       "<TR><TD colspan=""6""><b>&nbsp;&nbsp;&nbsp;&nbsp; Сведения о безвозмездных платежах и переводах денег, произведенных физическими" skip
       "                      лицами-резидентами, без предоставления документов, подтверждающих" skip
       "                      обоснованность переводов, в пользу физических лиц-нерезидентов из Казахстана за" skip
       "                      рубеж в размере, превышающих эквивалент 10 000 долларов США." skip
       "</b></TD></TR> " skip
       "<TR><TD colspan=""6"" align = ""center"">" skip
       "За период с " string (v-dtb,"99.99.9999") " по " string (v-dte,"99.99.9999") skip
       "<br><br><TR><TD colspan=""6"" align = ""center"">" skip.
       ...
       put stream m-out unformatted "<tr>"
                  ...
                  "<td align=""center"">Сумма переводов в <br> долларах США </td>"
                  "<td align=""center"">Страна перевода <br> (страна получателя)</td>"
                  "<td align=""center"">Цель перевода</td>"
                  "</tr>" skip.
    ...
    /*   accumulate rmztmp.rmztmp_uamt (TOTAL by rmztmp.rmztmp_rnn).
             ...
             v-sum = rmztmp.rmztmp_uamt.
                  if v-sum >= 10000 then do: 
                  i = i + 1.
                  put stream m-out  unformatted "<tr>"
                                     "<td>" rmztmp.rmztmp_fio "</td>"  skip
                                     "<td>" string(i) "</td>"
                                     "<td>" replace(trim(string(v-sum, "->>>>>>>>>>>>>>>>>>>>9.99")),".",",") "</td>"  skip
                                     "<td>" rmztmp.rmztmp_coun "</td>"  skip
                                     "<td>" rmztmp.rmztmp_goal "</td>"  skip
                                     "</tr>" skip.
                   end. 
       /*      end. */
       end.
...
unix silent cptwin vc50rpt.html excel.
