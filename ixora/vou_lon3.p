 ...
 * DESCRIPTION
        копия vou_lon c добавлением строк по погашению комиссии по годовой ставке
 * RUN
 ...
 * AUTHOR
        25.07.2012 dmitriy - копия vou_lon
 * CHANGES
...
def input parameter v-jhkom like jh.jh.
...
if v-jhkom <> 0 then do:
    find jh where jh.jh eq v-jhkom no-lock.
    for each jl of jh no-lock:
       create ljl.
       buffer-copy jl to ljl.
    end.
...
find ljl where ljl.jh eq v-jh and ljl.ln = 1 no-error.
...
/*find  first ljl where ljl.jh = v-jhkom and ljl.ln = 1 no-error.
if trim(ljl.rem[1]) ne ""   then do:
        create remfile.
        remfile.rem = "          " + ljl.rem[1] .
end.*/
