 ...
 * BASES
        BANK COMM TXB
 * CHANGES
        13.08.2013 damir - Внедрено Т.З. № 1182,1258,1257,1650. Добавил расчет v-amor_dis в разбивке признака сегментации; добавил Discount(lonrp18); исключил счет 1434 из Other assets;
                           com_exp - getRMZExp (добавил CHF,AUD); поставил по lc группу счетов 6000; добавил по com_inexp 5754; добавил по assets_other 1810,1890 и добавил getGL2;
                           добавил по depoGAR 2707 и добавил depoGAR в формулу расчета depo; добавил по depov%,depoSM%,depoCORP% 2723; в функции getRashsum закоментил действующий
                           алгоритм, восстановил закоментированный; добавил по lonast 1697; добавил по obiazPROCHIE 2890.
*/
...
if month(t-period.dtb) = 1 and day(t-period.dtb) < 7 then do:
...
/*    run setKritVal("zaimSO", t-period.pid, getKritVal("zaim",t-period.pid) + getKritVal("zaim1",t-period.pid) + getKritVal("zaim2",t-period.pid) + getKritVal("zaim3",t-period.pid) + + getKritVal("zaim4",t-period.pid)).*/
   ...
   /* run setKritVal("zaimItogo", t-period.pid, getKritVal("zaimSO",t-period.pid)).*/
    ...
    run setKritVal("intRozn", t-period.pid,  getRashsum("intRozn", "0", t-period.dte, t-period.dtb - 1 )).
    run setKritVal("intMsb", t-period.pid,  getRashsum("intMsb", "0", t-period.dte, t-period.dtb - 1 )).
    run setKritVal("intCorporate", t-period.pid,  getRashsum("intCorporate", "0", t-period.dte, t-period.dtb - 1 )).
    ...
    run setKritVal("com_capcost",t-period.pid, (( getKritVal("CAPITAL",t-period.pid) * 0.20 ) / 365 ) * 7 ).
    ...
    /*Итого обязательства */
    run setKritVal("itogo_obiazatelstva", t-period.pid,   getKritVal("depo",t-period.pid) /*+ getKritVal("depoGAR",t-period.pid)*/ + getKritVal("obiazPROCHIE",t-period.pid) +
    getKritVal("docSetCredit",t-period.pid)).
...
for each t-period no-lock:
    ...
    run setKritVal("com_taxgov",t-period.pid,AccrualMethod(t-period.dtb,t-period.dte,GetExpense( dtm_before_prev ,no,"","5761,5763,5764,5765,5766,5768,5722"),GetExpense( dtm_prev ,no,"","5761,5763,5764,5765,5766,5768,5722"), GetExpense( t-period.dte ,no,"","5761,5763,5764,5765,5766,5768,5722"))).
    ...
    run setKritVal("com_other",t-period.pid,AccrualMethod(t-period.dtb,t-period.dte,
                      GetExpense( dtm_before_prev ,no,"","5741,5743,5744,5747,5750,5752,5852,5853,5922") - abs( GetExpense( dtm_before_prev ,no,"","4852, 4853") ),
                      GetExpense( dtm_prev ,no,"","5741,5743,5744,5747,5750,5752,5852,5853,5922") - abs( GetExpense( dtm_prev ,no,"","4852, 4853") ),
                      GetExpense( t-period.dte ,no,"","5741,5743,5744,5747,5750,5752,5852,5853,5922") - abs( GetExpense( t-period.dte ,no,"","4852, 4853") )
                      )).
...
for each t-period no-lock:
 ...
 /*  Net interest income + Net Commission/FX income – Total operating expenses – HO operating expense allocation
     net_incom + com_NetCommFXincome1003 + com_inexp + com_inexp_val - com_exp_all - com_operexp
 */

  /* Branch operating income (pre provisions) = Net interest income + Net Commission/FX income + Other + Total operating expenses */
