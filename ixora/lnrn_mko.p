 ...
 * AUTHOR
        29/10/2009 galina - скопировал из lnrn.p с изменениями для МКО
 ...
 * CHANGES
        ...
        27/04/2011 madiyar - подправил формат
...
def var v-select as integer init 1.
...
{r-branch.i &proc = "lnrn1_mko(d1)"}
/*{r-brfilial.i &proc = "lnrn1_mko(d1)"}*/
...
/*20*/                  "<td bgcolor=""#C0C0C0"" align=""center"">Проср. ОД(в тенге)</td>"
                        "<td bgcolor=""#C0C0C0"" align=""center"">Размер<br>актива</td>"
...
/*45*/                  "<td bgcolor=""#C0C0C0"" align=""center"">Сумма резерва,<BR>тенге</td>"
                        ...
                        "<td bgcolor=""#C0C0C0"" align=""center"">Процентная <BR>ставка</td>" skip.
                        ...
                        "<td bgcolor=""#C0C0C0"" align=""center"">Статус действий</td>"
...
for each wrk no-lock break by wrk.bank by wrk.cif:
  ...
  if first-of(wrk.bank) then put stream m-out unformatted "<tr style=""font:bold""><td colspan=24>" wrk.bank "</td></tr>".
...
/*20*/            "<td align=""right"">" replace(trim(string(wrk.prosr_od_kzt,'>>>>>>>>>>>9.99')),'.',',') "</td>" skip
                  "<td align=""right"">" replace(trim(string(wrk.amt_akt,'>>>>>>>>>>>9.99')),'.',',') "</td>" skip
...
/*45*/            "<td align=""right"">" replace(trim(string(wrk.rezsum,'>>>>>>>>>>>9.99')),'.',',') "</td>" skip
                  ...
                  "<td align=""right"">" "'"wrk.lcnt "</td>" skip
                  "<td align=""right"">" replace(trim(string(wrk.intrate,'>>9.99')),'.',',') "</td>" skip.
...
put stream m-out unformatted
          "<tr align=""right"" style=""font:bold""><td colspan=6></td>" skip
            ...
            do i = 1 to 4:
                for each wrk2 where wrk2.rep_id = i and wrk2.bank = entry(k,v-bank) no-lock break by wrk2.bank by wrk2.priz_id by wrk2.id:
                    if first-of(wrk2.bank) then do:
                        ...
                        if i = 4 then put stream m-out unformatted
                            "<td colspan=10>СТАТУС ДЕЙСТВИЙ</td>" skip.
                    ...
                    if first-of(wrk2.priz_id) then do:
                       ...
                       if i = 4 then find first codfr where codfr.codfr = 'pkact' and codfr.code = wrk2.priz_id no-lock no-error.
                       put stream m-out unformatted
                          "<tr style=""font:bold"">" skip
                    ...
                    if last-of(wrk2.bank) then do:
                        ...
                        if i = 4 then put stream m-out unformatted
                            "<td colspan=10>СТАТУС ДЕЙСТВИЙ<br>(ИТОГО)</td>" skip.
            ...
            do i = 1 to 4:
                for each wrk4 where wrk4.rep_id = i and wrk4.bank = entry(k,v-bank) no-lock break by wrk4.bank by wrk4.priz_id:
                    ...
                    if first-of(wrk4.priz_id) then do:
                       ...
                       if i = 4 then find first codfr where codfr.codfr = 'pkact' and codfr.code = wrk4.priz_id no-lock no-error.
                       put stream m-out unformatted
                          "<tr>" skip
