 ...
 * CHANGES
        ...
        08/09/2005 madiyar - изменения в pkrepmon.i
        ...
        12/03/2008 madiyar - по запросу Евгения Хоменко (Костанай, мониторинг) телефоны - из клиентской записи
        03/06/2008 madiyar - пуш-отчет на старые даты тянулся не тот, исправил
        ...
        10.06.2009 galina - добавила поле срок кредита
                            изменила алгоритм расчета ежемесячного платежа
...
if datums <> g-today then do:
  ...
  unix silent value ("cptwin /data/reports/push/$DBID/pkmon" + fname + " excel").
  return.
end.
...
for each wrk no-lock:
    find first pkanketa where  pkanketa.bank = v-bank and pkanketa.lon = wrk.lon no-lock no-error.
 ...
 /*   find first lnsch where lnsch.lnn = wrk.lon and lnsch.f0 > 0 no-lock no-error.
    ...
    def var dn2 as integer.
    run day-360(lon.rdt,lon.duedt - 1,360,output dn1,output dn2).

    t-report.balmon = round(lon.opnamt * 30 / dn1, 2).
    if lon.prem = 0 then t-report.balmon = t-report.balmon + round(lon.opnamt * pkanketa.rateq / 1200,2).
    else t-report.balmon = t-report.balmon + round(lon.opnamt * lon.prem / 1200,2).
...
put stream m-out unformatted
  "<TR><TD align=""center""><h3>Задолженность по ссудным счетам клиентов за " string(datums)
       ...
       put stream m-out unformatted "<TR><TD><table border=""1"" cellpadding=""10"" cellspacing=""0"" >" skip
                         ...
                         "<td rowspan=""2"">Срок<br>кредита</td>"
                         ...
                         "<td rowspan=""2"">Кол-во<br>предстоящих<br> платежей</td>"
