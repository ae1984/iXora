 ...
 * DESCRIPTION
        Наложение ареста на определенную сумму на счете клиента
 * RUN
 ...
 * CHANGES
        ...
        25/11/2011 evseev - ТЗ-1119. вывод иин/бин в уведомление
        05.12.2011 evseev - ТЗ-625. переход на ИИН/БИН
        15/03/2012 id00810 - добавила v-bankname для печати уведомления
        ...
        25.07.2012 evseev - ТЗ-1233 добавил oper.dep@fortebank.com
        06.08.2012 evseev - СЗ от 02.08.2012
        10.07.2013 evseev - tz-1765
        07.08.2013 evseev - tz-1969
*/
...
/* блок объявления переменных ->*/
    ...
    def button bremtime LABEL "Временное удаление" .
    ...
    def browse b1 query q1 displ
         ...
         aas.chkamt format "->>>,>>>,>>>,>>9.99" label "Сумма "
         aas.payee  format "x(34)" label "Примечание"
         if aas.mn = "30037" then "Врем.удален" else "Действует"  format "x(14)" label "Статус"
         with 12 down title "Арест на определенную сумму по счету(действующие)" overlay.
    ...
    def browse b5 query q5 displ
          ...
          with 12 down title "История операций"  overlay.


    def frame getlist1
       aas.aaa format "x(21)" label     "Номер счета     " skip
       ...
       aas.dpname format "x(20)" label "Орган огранич   " help "Наименование органа выставившего ограничение" skip
       aas.chkamt format "->>>,>>>,>>>,>>9.99" label  "Сумма           " validate(aas.chkamt <> 0, "Введите сумму ") skip
    ...
    def frame getlist3
      ...
      aas.dpname format "x(20)" label "Организация, выставившая ограничение"
      with side-labels centered row 8.
    def frame getlock
      ...
      t-journal.dpname format "x(30)" label "Организация, выставившая ограничение" skip
    ...
    def frame fr1 b1 skip bRedakt bprint bdetail brem bremtime bhistory bexit  with centered overlay row 3 width 110  top-only.
    def frame fr4 b4 skip bdethis bexit with centered overlay row 3 width 90 top-only.
    ...
    def frame t_frame2
       ...
       with side-labels centered row 7.
    def frame t_frame3
       s_FindLogin label "Введите логин менеджера" format "x(8)"
       with side-labels centered row 7.
    def frame fr4 b4 skip bexit bdethis with centered overlay row 3 width 100 top-only.
    def frame fr5 b5 skip bdet bexit with centered overlay row 3 top-only.
/* <-блок объявления переменных*/
...
if avail sysc then v-mailoo = entry(5, sysc.chval, "|") no-error.
...
on choose of bt_AddNew in frame a2 do:
       ...
       if not available aaa then do:
         ...
         if aaa.sta = 'C' then do:
              message skip "Счет " + aaa.aaa + " закрыт !" skip
               "Добавление спец. инструкций невозможно !" skip(1)
    ...
    if avail aaa then do:
       ...
       if avail cif then do:
          if v-bin then message "Наименование клиента:" cif.name  skip "БИН клиента:" cif.bin view-as alert-box question buttons ok title "Визуальный контроль".
          else          message "Наименование клиента:" cif.name  skip "РНН клиента:" cif.jss view-as alert-box question buttons ok title "Визуальный контроль".
       end.
       create aas.
       ...
       aas.aaa = aaa.aaa.
       aas.payee = "Арест на определенную сумму".
       displ aas.aaa with frame getlist1.
       aas.whn = g-today.
       update aas.regdt aas.docnum aas.docdat aas.dpname aas.chkamt aas.payee with frame getlist1.
       aas.sta = 3.
       aas.mn = "30000".
       aas.activ = True.
       aas.contr = False.
       ...
       run savelog("aaahbal", "ink3 ; " + aaa.aaa + " ; " + string(aaa.hbal) + " ; " + string(aaa.hbal + aas.chkamt) + " ; " + string(aas.chkamt)).
       ...
       if lookup (aaa.lgr , "138,139,140,143,144,145") > 0 then do:
           run mail("DPC@fortebank.com", "METROCOMBANK <abpk@metrocombank.kz>", "Арест",
               "Арест №" + aas.docnum + " от " + string(aas.docdat,"99/99/9999") + " на " + string(aas.chkamt) + ". \n Зарегистрировал " + aas.who + " по счету " + aas.aaa + ", ИИН " + cif.bin + ", дата " + string(g-today,"99/99/9999")
               , "1", "", "").
           message "Вам необходимо выслать сканированную копию данного распоряжения на группу ДПК" view-as alert-box question buttons ok title "Сообщение".
       end.
    end.

    v-ofile = "ofile.htm" .
    v-ifile = "/data/export/tz1030_2.htm".
    ...
    repeat:
       ...
       repeat:
          ...
          if v-str matches "*docnum*" then do:
             v-str = replace (v-str, "docnum", aas.docnum ).
             next.
          end.
...
on choose of bt_Find in frame a2 do:
    ...
    ON CHOOSE OF bhistory IN frame fr1 do:
        ...
        ON CHOOSE OF bdethis IN frame fr4 do: /*История операций*/
            ...
            ON CHOOSE OF bdet IN frame fr5 do: /*Детали*/
                 ...
                 if avail bufl then do:
                    if bufl.chgoper = "A" or  bufl.chgoper = "E" or  bufl.chgoper = "Z" then do:
                       displ t-journal.docnum t-journal.docdat t-journal.dpname t-journal.payee with frame getlock.
                       hide frame getlock.
                    end.
            ...
            for each aas_hist where aas_hist.aaa = t-aashist.aaa  and aas_hist.ln = t-aashist.ln no-lock use-index aasprep:
                ...
                create t-journal.
                       ...
                       if      aas_hist.chgoper = 'A' then t-journal.name = "Введена  [" + ofc.ofc + "," + string(aas_hist.chgdat) + "," + STRING(aas_hist.chgtime, "hh:mm:ss") + "]" .
                       else if aas_hist.chgoper = 'E' then t-journal.name = "Изменена [" + ofc.ofc + "," + string(aas_hist.chgdat) + "," + STRING(aas_hist.chgtime, "hh:mm:ss") + "]" .
                       else if aas_hist.chgoper = 'D' then t-journal.name = "Удалена  [" + ofc.ofc + "," + string(aas_hist.chgdat) + "," + STRING(aas_hist.chgtime, "hh:mm:ss") + "]" .
                       else if aas_hist.chgoper = 'Z' then t-journal.name = "Врем.удален[" + ofc.ofc + "," + string(aas_hist.chgdat) + "," + STRING(aas_hist.chgtime, "hh:mm:ss") + "]" .
                       else t-journal.name =  "                    ***" .
        ...
        p-aaa = ''.
        p-ln = 0.
        for each aas_hist where aas_hist.aaa = aas.aaa and aas_hist.sta = 3 NO-LOCK USE-INDEX aasprep:
              if aas_hist.ln <> p-ln then do transaction:
                 ...
                 if aas_hist.chgoper = "D" then t-aashist.ctc = " [Удалена]".
                 else if aas_hist.chgoper = "Z" then t-aashist.ctc = " [Врем.удален]".
    ...
    ON CHOOSE OF bprint IN frame fr1 do:
       ...
       put unformatted "------------------------------------------------------------------------------------------------------------------" skip.
       put unformatted "                    НАЛОЖЕНИЕ АРЕСТА НА ОПРЕДЕЛЕННУЮ СУММУ                                              " skip.
       put unformatted "------------------------------------------------------------------------------------------------------------------" skip.
       put unformatted "  CIF    НАИМЕНОВАНИЕ        ДАТА РЕГ         СЧЕТ         N ДОК    ДАТА ДОК.  ОРГАН_ОГР         ПРИМЕЧАНИЕ       " skip.
       put unformatted "------------------------------------------------------------------------------------------------------------------" skip.
       if v-dep = 1 then
         for each aas where aas.sta = 3 and aas.aaa = s_FindAcc and aas.ln <> 7777777 use-index aaaln exclusive-lock:
       ...
       if v-dep = 2 then
         for each aas where aas.sta = 3 and aas.cif = s_FindCIF and aas.ln <> 7777777 exclusive-lock:
           find last b-cif where b-cif.cif = aas.cif no-lock no-error.
           put unformatted "  " aas.cif " " b-cif.sname format 'x(20)' string(aas.regdt,"99/99/99") "  " aas.aaa format 'x(20)' " " aas.docnum format 'x(7)' "  " string(aas.docdat,"99/99/99") "   " aas.dpname format 'x(15)'  "   " aas.payee  skip.
         end.
       if v-dep = 3 then
         for each aas where aas.sta = 3 and aas.who = s_FindLogin and aas.ln <> 7777777 exclusive-lock /*use-index aaaln*/:
           find last b-cif where b-cif.cif = aas.cif no-lock no-error.
           put unformatted "  " aas.cif " " b-cif.sname format 'x(20)' string(aas.regdt,"99/99/99") "  " aas.aaa format 'x(20)' " " aas.docnum format 'x(7)' "  " string(aas.docdat,"99/99/99") "   " aas.dpname format 'x(15)'  "   " aas.payee  skip.
         end.
       if v-dep = 4 then
       for each aas where aas.sta = 3 and aas.regdt >= dt_FindDateBegin and aas.regdt <= dt_FindDateEnd and aas.ln <> 7777777 /*use-index aaaln*/ exclusive-lock:
         find last b-cif where b-cif.cif = aas.cif no-lock no-error.
         put unformatted "  " aas.cif " " b-cif.sname format 'x(20)' string(aas.regdt,"99/99/99") "  " aas.aaa format 'x(20)' " " aas.docnum format 'x(7)' "  " string(aas.docdat,"99/99/99") "   " aas.dpname format 'x(15)'  "   " aas.payee  skip.
       end.
       output close.
    ...
    ON CHOOSE OF bdetail IN frame fr1 do:
       ...
       if avail buf then do:
          display aas.who aas.regdt aas.docnum aas.docdat aas.dpname with frame getlist3.
          hide frame getlist3.
       end.
    end.
    /* Снять ограничение */
           ...
           if yes-no ("Внимание!", "Вы действительно хотите снять ограничение?") then do:
              ...
              if not aas.contr then do:
                 ...
                 MESSAGE "Необходим контроль Директором/ЗамОД в 1.3.1.6" VIEW-AS ALERT-BOX QUESTION buttonS OK.
                  ...
                  s-aaa = aaa.aaa.
                  aas.mn = substr(aas.mn,1,3) + "39".
                  RUN aas2his.
                  if lookup (aaa.lgr , "138,139,140,143,144,145") > 0 then do:
                       find first aaa where aaa.aaa = aas.aaa no-lock no-error.
                       find first cif where cif.cif = aaa.cif no-lock no-error.
                       run mail("DPC@fortebank.com", "METROCOMBANK <abpk@metrocombank.kz>", "Отзыв/удаление Арест",
                           "Отзыв/удаление Арест №" + aas.docnum + " от " + string(aas.docdat,"99/99/9999") + " на " + string(aas.chkamt) + ". \n Зарегистрировал " + aas.who + " по счету " + aas.aaa + ", ИИН " + cif.bin + ", дата " + string(aas.whn,"99/99/9999")
                           , "1", "", "").
                       message "Вам необходимо выслать сканированную копию данного распоряжения на группу ДПК" view-as alert-box question buttons ok title "Сообщение".
                  end.
                  delete buf.
    ...
    ON CHOOSE OF bremtime IN frame fr1 do:
       find current aas exclusive-lock no-error.
       if avail aas then do:
           if aas.mn <> "30037" then do:
               if yes-no ("Внимание!", "Желаете временно удалить документ?") then do:
                   find first aaa where aaa.aaa = aas.aaa no-lock no-error.
                   find first cif where cif.cif = aaa.cif no-lock no-error.
                   op_kod= 'Z'.
                   aas.who = g-ofc. aas.whn = g-today. aas.tim = time.
                   s-aaa = aaa.aaa.
                   if length(aas.mn) <> 5 then aas.mn = "30000".
                   aas.mn = substr(aas.mn,1,3) + "37".
                   RUN aas2his.
                   browse b1:refresh().
                   run mail(g-ofc + "@metrocombank.kz; oper.dep@fortebank.com; " + v-mailoo, "<INK3@metrocombank.kz>", "Произведено временное удаление спец. инструкции",
                       cif.cif + ", " + cif.prefix + " " + cif.name + ", " + aaa.aaa + "~n" +
                       "Произведено временное удаление спец. инструкции №" + string(aas.docnum) + ", дата " + string(aas.regdt) + ", сумма " + string(aas.chkamt) + ".  После оплаты не забудьте возобновить специальную инструкцию", "", "","").
               end.
           end. else do:
               if yes-no ("Внимание!", "Желаете восстановить временно удаленный документ?") then do:
                   find first aaa where aaa.aaa = aas.aaa no-lock no-error.
                   find first cif where cif.cif = aaa.cif no-lock no-error.
                   find first aas_hist where aas_hist.aaa = aas.aaa and aas_hist.ln = aas.ln and aas_hist.chkamt = aas.chkamt and
                        aas_hist.chgoper <> "Z" use-index aasprep no-lock no-error.
                   if avail aas_hist then do:
                      op_kod = aas_hist.chgoper.
                      aas.mn = aas_hist.mn.
                   end. else do:
                      op_kod = "A".
                      aas.mn = "30000".
                   ...
                   RUN aas2his.
                   browse b1:refresh().
                   run mail(g-ofc + "@metrocombank.kz; oper.dep@fortebank.com; " + v-mailoo, "<INK3@metrocombank.kz>", "Восстановлен временно удаленный документ",
                       cif.cif + ", " + cif.prefix + " " + cif.name + ", " + aaa.aaa + "~n" +
                       "Восстановлен временно удаленный документ №" + string(aas.docnum) + ", дата " + string(aas.regdt) + ", сумма " + string(aas.chkamt), "", "","").
               end.
           end.
       end.
    end.
    ...
    repeat:
        ...
        case v-dep:
          when 1 then do: /*По номеру счета*/
               hide frame a2.
               repeat:
                    update s_FindAcc label  "Поиск по номеру счета" with centered row 7 side-label frame t_frame1.
                     find aaa where aaa.aaa = s_FindAcc no-error.
                     if not available aaa then do:
                        message "Счет не найден". pause 3.
                     end. else do:
                         if aaa.sta = 'C' then do:
                            message skip "Счет " + aaa.aaa + " закрыт !" skip "Добавление специнструкций невозможно !" skip(1) view-as alert-box button Ok title "Внимание!".
                            return.
                         end.
                         leave.
                     end.
               ...
               hide frame t_frame1.
               i_indx = 0.
               for each aas where aas.sta = 3 and aas.aaa = s_FindAcc and aas.ln <> 7777777 use-index aaaln.
                   i_indx = i_indx + 1.
               end.
               if i_indx <> 0 then do:
                   open query q1 for each aas where aas.sta = 3 and aas.aaa = s_FindAcc and aas.ln <> 7777777 use-index aaaln.
                   b1:SET-REPOSITIONED-ROW (1, "CONDITIONAL").
                   ENABLE all with frame fr1 centered overlay top-only.
                   apply "value-changed" to b1 in frame fr1.
                   WAIT-FOR WINDOW-CLOSE of frame fr1.
               end. else do:
                   display "По счету " + s_FindAcc + " нет инструкций по частичной блокировке сумм !" format "x(65)" with center row 7 frame bridin.
                   ...
                   if frame-index = 1 then do:
                      ...
                      p-aaa = ''.
                      p-ln = 0.
                      for each aas_hist where aas_hist.aaa = aaa.aaa and aas_hist.sta = 3 NO-LOCK USE-INDEX aasprep:
                            if aas_hist.ln <> p-ln then do transaction:
                               p-aaa = aas_hist.aaa.
                               p-ln = aas_hist.ln.
                               create t-aashist.
          ...
          when 2 then do: /*По CIF коду    */
               hide frame a2.
               update s_FindCIF label  "Поиск по CIF коду клиента" with centered row 7 side-label frame t_frame2.
               open query q1 for each aas where aas.sta = 3 and  aas.cif = s_FindCIF and aas.ln <> 7777777 /*use-index aaaln*/.
               b1:SET-REPOSITIONED-ROW (1, "CONDITIONAL").
               ENABLE all with frame fr1 centered overlay top-only.
               apply "value-changed" to b1 in frame fr1.
               WAIT-FOR WINDOW-CLOSE of frame fr1.
          ...
          when 3 then do: /* По менеджеру */
               hide frame a2.
               update s_FindLogin format "x(7)" label  "Введите логин менеджера" with centered row 7 side-label frame t_frame3.
               v-dp3 = 0.
               run sel2 (" Параметры поиска", " ПОКАЗАТЬ ВСЕ | ЗАДАТЬ ПЕРИОД", output v-dp3).
                if v-dp3 = 0  then return.
                if v-dp3 = 1 then do:
                   open query q1 for each aas where aas.sta = 3 and aas.who = s_FindLogin and aas.ln <> 7777777 /*use-index aaaln*/.
                end.
                if v-dp3 = 2 then do:
                   ...
                   open query q1 for each aas where aas.sta = 3 and aas.who = s_FindLogin and aas.regdt >= dt_1 and aas.regdt <= dt_2 and aas.ln <> 7777777 /*use-index aaaln*/.
                end.
                b1:SET-REPOSITIONED-ROW (1, "CONDITIONAL").
                ENABLE all with frame fr1 centered overlay top-only.
                apply "value-changed" to b1 in frame fr1.
          ...
          when 4 then do: /*За период      */
               ...
               open query q1 for each aas where aas.sta = 3 and aas.regdt >= dt_FindDateBegin and aas.regdt <= dt_FindDateEnd and aas.ln <> 7777777 /*use-index aaaln*/.
               b1:SET-REPOSITIONED-ROW (1, "CONDITIONAL").
               ENABLE all with frame fr1 centered overlay top-only.
               apply "value-changed" to b1 in frame fr1.
               WAIT-FOR WINDOW-CLOSE of frame fr1.
