 ...
 * DESCRIPTION
        Описание - 1. ПЕРЕВОДЫ В ТЕНГЕ БЕЗ ОТКРЫТИЯ СЧЕТА | 1. ОТПРАВЛЕНИЕ ПЕРЕВОДА | 2. ПЕРЕВОД В ДРУГОЙ БАНК
 * RUN
...
repeat:
    ...
    repeat:
        ...
        if v-str matches "*receibenefic*" then do:
            if v_lname1 <> "" then v-str = replace (v-str,"receibenefic",trim(v_lname1)).
        ...
        if v-str matches "*+C*" then do:
            if v_rez1 <> 0 then v-str = replace (v-str,"+C",substr(string(v_rez1),1,1)).
            else v-str = replace (v-str,"+C","").
            next.
        end.
        if v-str matches "*+D*" then do:
            if v_rez1 <> 0 then v-str = replace (v-str,"+D",substr(string(v_rez1),2,1)).
            else v-str = replace (v-str,"+D","").
            next.
        end.
        if v-str matches "*bankbenficir*" then do:
            if v_bank <> "" then v-str = replace (v-str,"bankbenficir",trim(v_bank)).
            else v-str = replace (v-str,"bankbenficir","").
            next.
        end.
        if v-str matches "*bikbenficiry*" then do:
            if v_swibic <> "" then v-str = replace (v-str,"bikbenficiry",trim(v_swibic)).
