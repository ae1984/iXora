 ...
 * CHANGES
        ...
        04/06/08 marinav - перекодировка
...
/* Счета АРП, по которым надо автоматически делать проводку */
 ...
 /*message v-lbin v-lbhst v-lbeks.*/
       ...
       repeat :
          ...
          if search(num[9]) eq num[9]  then do:
             ...
             if not avail lbinf then do:
                 ...
                 if t-gro = 'GROSS' or t-gro = 'MEP00' then do:
                        create t-qin.
                        ...
                        assign lbinf.rdt = g-today
                               ...
                               lbinf.whn = today
                               lbinf.tim = time.
                        if t-gro = 'GROSS' then lbinf.gc = 'gross'.
                        else lbinf.gc = 'smep'.
    ...
    if yn then  do:
      ...
      /* Display  " Sending LBI , wait ... "  with row 22 no-box frame sss. */
       ...
       for each que where que.pid = "LBI" exclusive-lock,
           each remtrz where remtrz.remtrz = que.remtrz and (remtrz.cover = 2 or remtrz.cover = 6) and remtrz.fcrc = 1 exclusive-lock .
