 ...
 * BASES
        BANK COMM
 * AUTHOR
        25.04.2012 id00787 - взял за основу dogovor.p
 * CHANGES
        03/05/2012 evseev - nbankBik.i
        09.10.2013 dmitriy - ТЗ 1823
...
if lookup(lgr.lgr,"246,202,204,222,208,249,247,248,A22,A23,A24,A01,A02,A03,A04,A05,A06,A19,A20,A21,A25,A26,A27,A34,A35,A36,А31,А32,А33,478,479,480,481,482,483,484,485,486,487,488,489,518,519,520,151,153,171,157,176,152,154,172,158,177,397,173,174,175,160,161") = 0  then do:
...
if bmm.chval = "TXB00" then v-podp = "Бояркина И.Я.".
        ...
        /*if lookup(lgr.lgr,"A22,A23,A24,A01,A02,A03,A04,A05,A06,246,478,479,480,481,482,483,518,519,520,151,153,171,157,176,152,154,172,158,177,173,175,174,202,204,222,208") > 0  then do:*/
            ...
            */
            ...
            v-ofile = "ofile.htm" .
            if lookup(lgr.lgr,"246,202,204,222,208,249,247,248,A22,A23,A24,A01,A02,A03,A04,A05,A06,A19,A20,A21,A25,A26,A27,A34,A35,A36,А31,А32,А33") > 0  then do:
                v-ifile = "/data/export/" + v-prefix + "ibfl.htm".
            end.
            if lookup(lgr.lgr,"478,479,480,481,482,483,484,485,486,487,488,489,518,519,520,151,153,171,157,176,152,154,172,158,177,397,173,174,175,160,161") > 0  then do:
                v-ifile = "/data/export/" + v-prefix + "ibul.htm".
            end.
               ...
               repeat:
                 ...
                 if v-dogsgn = "" then do:
                     ...
                     if v-str matches "*kcity*" then do:
                        v-str = replace (v-str, "kcity", bcit.chval).
                        next.
                     end.
                 end.
                 if v-dogsgn <> "" then do:
                 ...
                 if v-str matches "*namebank*" then do:
                    v-str = replace (v-str, "namebank", v-nbankDgv ).
                    next.
                 end.
                 if v-str matches "*filnameru*" then do:
                    v-str = replace (v-str, "filnameru", cmp.name ).
                    next.
                 end.
                 find sysc where sysc.sysc = "bnkadr" no-lock no-error.
                 if v-str matches "*filnamekz*" then do:
                    v-str = replace (v-str, "filnamekz", entry(14, sysc.chval, "|") ).
                    next.
                 end.
