...
/* Формирование реестра для простого кассира */
     ...
     Doc:ElementBy(y).
       ...
       do:
        ...
        /*******************************************************************************************/
         ...
         put stream m-out unformatted  v-text skip.
         put stream m-out unformatted  L skip.
         v-text = "| № | Док.N |           Назначение платежа                | Сумма  | Комиссия |".
         put stream m-out unformatted  v-text skip.
         put stream m-out unformatted  L skip.
         tmpx = Doc:supp_id.
         tmpd = Doc:whn_cr.
        ...
        string( Line:payacc + " " + Line:payname ,"x(44)" ) + "|" + string(Line:summ,">>>>>.99") + "|  " + string(Line:comm_summ,">>>>>.99") + "|".
        put stream m-out unformatted  v-text skip.
        tmpd = Doc:whn_cr.
...
/*********************************************************************************************************************/
 function GenHeader returns log ( input Doc as Class COMPAYDOCClass):
   ...
   v-text = g-comp.
   put stream m-out unformatted  v-text skip.
   v-text = "КАССИР:" + Doc:ofcname + "/" + g-ofc.
   ...
   if rez then  return.
     ...
     do:
       ...
       END.
                                                                                                                                       ...
                                                                                                                                       /*and note = ?*/
       if Doc:Find-All("who_cr = '" + g-ofc + "' and whn_cr >= " + string(v-dt) + " and whn_cr <= " + string(v-dt2) + supp_txt + " ) and jh <> ? and state <> -2 and state <> 7 no-lock BY whn_cr BY supp_id") > 0 then
       do:
