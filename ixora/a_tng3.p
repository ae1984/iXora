 ...
 * CHANGES
        ...
        11/03/2012 Luiza  - исправила счет комиссии 461120
        ...
        28/04/2012 Luiza - добавила запись типа документа в joudop
        03/05/2012 Luiza  - добавила поле срок действия УЛ
        04/05/2012 Luiza  - перенесла поле v-docdtf
        05.05.2012 damir  - добавлены a_tng3printapp.i. Новые форматы заявлений.
        14/05/2012 Luiza  - изменила Get_Nal и v-joudoc shared
        15/05/2012 Luiza  - АРП счет подтягиваем автоматом
        15/05/2012 Luiza  - v-countr для KZ редактируется
        24/05/2012 Luiza  - изменения в процедуре sc
        ...
        26/07/2012 Luiza   -  проверка суммы лимита только для 100500
        26/07/2012 Luiza   - слово EK заменила ЭК
        30/07/2012 Luiza  - при создании  проводки комиссии в назначении прописываем наименование кода комиссии
        ...
        24.12.2012 damir - Внедрено Т.З. № 1619.Тестирование ИИН/БИН.Перекомпиляция в связи с изменениями в a_tng3printapp.i.
...
def  var v_ktk as int no-undo init 461120.
...
def  var v_crc as int  no-undo format "9" init 1.
...
def  var v-bdt1 as date no-undo.
    def var v_arpll as char.


def  var v_comcode as char no-undo format "x(5)" init "029".
...
form
    v-joudoc    label " Документ" format "x(10)"  skip
                     "           ПОЛУЧАТЕЛЬ"
                     "                                           ОТПРАВИТЕЛЬ" SKIP
    v-label format "x(20)" v_rnn  no-label format "x(12)" colon 19 validate((chk12_innbin(v_rnn)),'Неправильно введён БИН/ИИН')
    ...
    v_countr1   label "                            Страна резидентства"  validate(can-find(first codfr where codfr.codfr = "iso3166" and codfr.child = false
    ...
    v_docdt    label " Дата выдачи      " format "99/99/9999" help " Ведите дату выдачи документа удостов. личость в формате дд/мм/гггг " validate(trim(v_docdt) <> "", "Заполните дату выдачи документа")
    v_docdtf   label " Срок действия    " colon 55 format "99/99/9999" help " Ведите срок действия документа удостов. личость в формате дд/мм/гг " /*validate(trim(v_docdtf) <> "", "Заполните срок действия документа")*/ skip
    ...
    v_comcode  label " Код комиссии     "
    v_comname  no-label colon 35 format "x(35)" skip
    v_sumk     label " Сумма комиссии   " format ">>>,>>>,>>>,>>>,>>9.99" skip
    v_oper     label " Назнач. платежа  "   format "x(70)" skip
    v_knp      label " КНП              " format "999" skip
    /*v_arpll      label "АРП счет          " format "x(20)" skip*/
...
DEFINE FRAME f-rez b-rez  WITH overlay 1 COLUMN SIDE-LABELS row 9 COLUMN 40 width 45 NO-BOX.
...
on help of v-joudoc in frame f_main do:
    if v-ek = 1 then run a_help-joudoc1 ("TN5"). else  run a_help-joudoc1 ("NT5").
...
if new_document then do:  /* создание нового документа  */
    ...
    v_title = " ПЕРЕВОДЫ В ТЕНГЕ БЕЗ ОТКРЫТИЯ СЧЕТА (выплата перевода)".
    ...
    if v_u = 2 then do:       /* update */
        do transaction:
            ...
            if available joudoc then do:
                ...
                if joudoc.sts = "TN5" or joudoc.sts = "NT5" then do:
                    if v-ek = 1 and joudoc.sts = "NT5" then do:
                        message substitute ("Документ создан для ЭК ГК 100500") view-as alert-box.
                        return.
                    end.
                    if v-ek = 2 and joudoc.sts = "TN5" then do:
                    ...
                    if available joudop then do:
                        if joudop.type <> "TN5" and joudop.type <> "NT5" then do:
                            message substitute ("Документ не относится к типу ПЕРЕВОДЫ В ТЕНГЕ БЕЗ ОТКРЫТИЯ СЧЕТА (выплата перевода)") view-as alert-box.
                            return.
                        end.
                        if v-ek = 1 and joudop.type = "NT5" then do:
                            message substitute ("Документ создан для ЭК ГК 100500") view-as alert-box.
                            return.
                        end.
                        if v-ek = 2 and joudop.type = "TN5" then do:
                            message substitute ("Документ создан для счета ГК 100100 ") view-as alert-box.
                            return.
                        end.
                    end.
...
procedure save_doc:
    ...
    if new_document then do:
        ...
        v_lname1 = "". v_name1 = "". v_mname1 = "". v_rez1 = "". v_countr1 = "".  v_sum = 0.
    ...
    {adres.i}
    v_addr = v-adres.
    displ v_addr  v_crc v_comcode v_knp with frame f_main.
    update v_tel v_sum with frame f_main.
    if v-ek = 2 then do:
    ...
    displ v_addr v_knp  v_comname  with frame f_main.
    ...
    /*------------------------------------------------------------*/
        ...
        update v_sumk v_oper v_lname1 v_name1 v_mname1 v_rez1 with frame f_main.
        if substring(v_rez1,1,1) = "1" then v_countr1 = "KZ".
    ...
    /* find arp  ------------------------------------------------*/
    for each arp where arp.gl = v_dt and arp.crc = v_crc and length(arp.arp) >= 20 and arp.des MATCHES "*вх*" and not arp.des  MATCHES "*СП*" no-lock.
    ...
    /*--------------------------------------------------------*/
    /*update v_arpll with frame f_main.
    find first arp where arp.arp = v_arpll no-lock no-error.
    ...
    if v-ja then do:
        ...
        if new_document then do:
            ...
            if not avail sernumdoc then do:
                ...
                sernumdoc.progtrans = "a_tng3".
                find last b-sernumdoc where b-sernumdoc.whn = g-today and b-sernumdoc.progtrans = "a_tng3" no-lock no-error.
        ...
        joudop.patt = trim(v_lname1) + " " + trim(v_name1) + " " + trim(v_mname1) + "^" + "" + "^" + v_rez1 + "^" +
                    "" + "^" + "" + "^" + "" + "^" + v_knp.
        if v-ek = 1 then joudop.type = "TN5". else joudop.type = "NT5".
...
procedure view_doc:
    ...
    displ v-label format "x(18)" no-label v_rnn format "x(12)" no-label  v_lname v_name v_mname   v_doc_num v_docdt v_docdtf
            v_docwho v_addr v_tel v_doctype v_rez v_countr v_public v-bdt1 v-bplace v_crc v_sum v_comcode v_comname v_sumk v_knp
          v_oper /*v_oper1 */ v_lname1 v_name1 v_mname1 v_rez1 v_countr1  v_dt v_joutrx format "zzzzzzzzz" v_kt  v_arp v_arpname v_dtk  v_ktk    with frame f_main.
   ...
   /* find first cursts no-lock no-error.
    ...
    if v-ek = 2 then do:
         ...
         /* формир v-param для trxgen.p */
            v-tmpl = "JOU0056".
            v-param = v-joudoc + vdel + string(v_sum) + vdel + string(v_crc) + vdel + v_arp + vdel + v-chEK + vdel + v_oper + vdel + substring(v_rez,1,1) + vdel + substring(v_rez1,1,1) + vdel + substring(v_rez,2,1) + vdel + substring(v_rez1,2,1) + vdel + v_knp.
            ...
            if v_sumk <> 0 then do:
                ...
                run trxgen ("jou0053", vdel, v-param, "jou", v-joudoc, output rcode, output rdes, input-output s-jh).
                if rcode ne 0 then do:
                    message rdes.
                    pause.
                    undo, return.
    ...
    /**/
        ...
        for each sendod no-lock.
            ...
            "Добрый день!\n\n Необходимо отконтролировать выплату перевода без открытия счета в тенге \n Сумма: " + string(v_sum) +
            "  " + v-crc_val + "\n документ :" + v-joudoc + "\n создал :" + g-ofc + "\n " + string(g-today) + "  " +
            string(time,"HH:MM"), "1", "","" ).
        end.
        v_joutrx = s-jh.
    ...
    for each jl where jl.jh = s-jh and jl.crc = 1 and (jl.gl = 100500 or jl.gl = 100100) no-lock:
        ...
        jlsach.lnln = 1.
            if v_countr1 = "KZ" then do:
                ...
                if jl.dc = "c" and jl.cam = v_sum and v_sum <> 0 then jlsach.sim = 250.
                if jl.dc = "d" and jl.dam = v_sum and v_sum <> 0 then jlsach.sim = 250.
            end.
            else do:
                if jl.dc = "c" and jl.cam = v_sumk and v_sumk <> 0 then jlsach.sim = 100.
                if jl.dc = "d" and jl.dam = v_sumk and v_sumk <> 0 then jlsach.sim = 100.
                if jl.dc = "c" and jl.cam = v_sum and v_sum <> 0 then jlsach.sim = 260.
                if jl.dc = "d" and jl.dam = v_sum and v_sum <> 0 then jlsach.sim = 260.
            end.
    release jlsach.
    end.
...
/*--------------------------------------------------------------*/
    ...
    v-res111 = TCIFNAME + "&" + TKOD + "&" + TSUMM + "&" + TCRC + "&" + TRECNAME + "&" + TRECINN + "&" +
