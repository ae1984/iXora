...
displ t-obesp.num label "№" format ">9"
      ...
      t-obesp.name label "Залогодатель      " format "x(18)"
      ...
      t-obesp.fdt label "Дата c    " format "99/99/9999"
      t-obesp.tdt label "Дата до   " format "99/99/9999"
      with 10 down overlay no-label no-box.

define frame ft b-obesp help "<Enter>-Изменить, <F4>-Выход" skip bsave
 with width 110 row 3 overlay no-label title "ОБЕСПЕЧЕНИЕ".
...
on "return" of b-obesp in frame ft do:
    ...
    displ  bb-obesp.num format ">9"
           ...
           bb-obesp.name format "x(20)"
           bb-obesp.numdog format "x(15)" validate(bb-obesp.numdog <> '' ,"Номер договора залога должен быть заполнен!")
           bb-obesp.dtdog format "99/99/9999" validate(bb-obesp.dtdog <> ? ,"Дата договора залога должна быть указана!")
           ...
           bb-obesp.sum format ">>>,>>>,>>>,>>>.99" /*bb-obesp.num bb-obesp.code bb-obesp.name bb-obesp.crc bb-obesp.sum with frame fr2 row b-obesp:focused-row + 5.*/
           bb-obesp.fdt format "99/99/9999" validate(bb-obesp.fdt = bb-obesp.dtdog ,"'Дата c' должна совпадать с датой договора!")
           bb-obesp.tdt format "99/99/9999" validate(bb-obesp.tdt = ? or bb-obesp.tdt >= bb-obesp.fdt ,"'Дата до' не может быть меньше чем 'Дата с'!")
    ...
    /*update bb-obesp.code bb-obesp.numdog bb-obesp.dtdog bb-obesp.sectp bb-obesp.crc bb-obesp.sum with frame fr2. */
    ...
    displ bb-obesp.fdt bb-obesp.dtdog with frame fr2.
    update bb-obesp.sectp with frame fr2.
    update bb-obesp.crc bb-obesp.sum with frame fr2.
    update bb-obesp.fdt with frame fr2.
    update bb-obesp.tdt with frame fr2.
    hide frame fr2.
    pause 0.
    run obespdes.
    open query q-obesp for each t-obesp no-lock.
    reposition q-obesp to rowid v-rid no-error.
    find first bb-obesp no-lock no-error.
...
on "insert-mode" of b-obesp in frame ft do:
    v-num = 0.
    find last bbb-obesp where bbb-obesp.acc = t-obesp.acc no-lock no-error.
    ...
    open query q-obesp for each t-obesp no-lock.
    reposition q-obesp to rowid v-rid no-error.
    find first bb-obesp where no-lock no-error.
    if avail bb-obesp then b-obesp:refresh().
