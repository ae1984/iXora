...
do:
   ...
   for each txb.aaa where txb.aaa.stadt >= v-dbeg and txb.aaa.stadt <= v-dend no-lock:

       if txb.aaa.stadt <> txb.aaa.regdt then next.      /* исключаем конвертированные */
       ...
       if not avail txb.sub-cod then do:
          ...
          run lonbal3('cif', txb.aaa.aaa, txb.aaa.stadt, "1", yes, output sm).



          mnth = month(txb.aaa.stadt).
          if mnth <> 12 then
             mnth = mnth + 1. else mnth = 1.

                   .

           smvl = round(crc-crc-date(decimal(sm), txb.aaa.crc, 1, date("1." + string(mnth) + "." + string(year(txb.aaa.stadt))) ), 2).
  ...
  if p-dt = g_date then do:
     for each txb.trxbal where txb.trxbal.subled = p-sub and txb.trxbal.acc = p-acc no-lock:
	    ...
	    find b-sub-cod where b-sub-cod.sub eq "gld" and b-sub-cod.d-cod eq "gldic"
	                   and b-sub-cod.acc eq string(txb.trxlevgl.glr) no-lock no-error.
