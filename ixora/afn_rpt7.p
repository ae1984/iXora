...
r1 = "Кредиты по целевому назначению".
...
for each codfr where codfr='lntgt' no-lock:
    create lnpr.
     lnpr.id = codfr.code.
     lnpr.kname = codfr.name[1].
end.

find first cmp no-lock no-error.
if avail cmp then vcode = cmp.code.
{r-branch.i &proc = "afn_rpt7f(comm.txb.bank)"}


def var h1 as char no-undo.
def var h2 as char no-undo.
def var h3 as char no-undo.
h1 = "Целевое предназначение кредитных денег".
h2 = "Сумма".
h3 = "Удельный вес".
