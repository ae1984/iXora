 ...
 * CHANGES
        ...
        15/02/2011 madiyar  - в jou-документ прописываем код клиента
        10.06.2011 aigul    - проверка срока действия УЛ
        21/07/2011 evseev   - добавил логирование
        28/07/2011 evseev   - исправил ошибку в 271 строке
        25/01/2012 evseev   - ТЗ-1245
        01.02.2012 lyubov   - изменила символ кассплана (410 на 220)
...
def new shared var d_tssum_nalog as decimal.
...
def new shared var d_1%    as decimal decimals 2. /* Сумма удерживаемая с 1 уровня  */
def new shared var d_2%    as decimal decimals 2. /* Сумма удерживаемая со 2 уровня */
def new shared var d_3%    as decimal decimals 2. /* Сумма для выплаты на 1 уровень */
...
v-gtoday = g-today.
v-gofc = g-ofc.
run savelog ("filclstda", "0) дата: " + string(g-today) + "; ID: " + g-ofc).
...
run sel2 ("Выбор :", " 1. Новая заявка на закрытие депозита | 2. Выплата с транзитного счета", output v-sel).
if v-sel = '0' then return.

if v-sel = '1' then do:

      v-comkod = '302,028,119,056,197'.
      v-tit = 'Закрытие депозита на филиале '.
      v-type = 'TDA,CDA'.
...
/*=========================================*/
do transaction:
run savelog ("filclstda", "1) дата: " + string(g-today) + "; ID: " + g-ofc + "; aaa: " + v-iik).
create clsdp.
       clsdp.aaa =  v-iik.
       clsdp.txb =  txb.bank.
       clsdp.sts = '1'.   /*1- заявка на выплату    2- заявка исполнена  3- произошла какая-то ошибка*/
       clsdp.level1 = d_1%.
       clsdp.level2 = d_3%.
       clsdp.cst = string(d_tssum_nalog).
...
/*=========================================*/
     ...
     /*********************************************************************/
       if v-ja then do:
              ...
              message "Подождите, пожалуйста, - производится выплата % на 1 уровень !".
              def var i as int.
              do i = 1 to 5000000.
              end.


              find last clsdp where clsdp.aaa =  v-iik and clsdp.txb =  txb.bank and clsdp.sts = '3' no-lock no-error.
              if avail clsdp then do:
                       message "Ошибка при выплате средств на 1 уровень депозита ! " view-as alert-box.
                       run savelog ("filclstda", "1.1) дата: " + string(g-today) + "; ID: " + g-ofc + "; aaa: " + v-iik + " - Ошибка при выплате средств на 1 уровень депозита !").
                       return.
              end.

              find last clsdp where clsdp.aaa =  v-iik and clsdp.txb =  txb.bank and clsdp.sts = '2' no-lock no-error.
              if not avail clsdp then do:
                       message "Ошибка при выплате средств на 1 уровень депозита: ! " view-as alert-box.
                       run savelog ("filclstda", "1.2) дата: " + string(g-today) + "; ID: " + g-ofc + "; aaa: " + v-iik + " - Ошибка при выплате средств на 1 уровень депозита !").
                       return.
              end.


              run savelog ("filclstda", "2) дата: " + string(g-today) + "; ID: " + g-ofc + "; aaa: " + v-iik).
          ...
          if v-rmz ne "" then  do:
                                  ...
                                  run savelog ("filclstda", "3) дата: " + string(g-today) + "; ID: " + g-ofc + "; aaa: " + v-iik + " - Платеж " + v-rmz + " на снятие средств со счета " + filpayment.iik + " сделан. Транзитный счет будет пополнен через 5 минут.").
                                  ...
                                  else message "Ошибка при формировании платежа на закрытие счета " filpayment.iik " !" view-as alert-box title "".
          end.
...
if v-sel = '2' then do:
       ...
       def new shared var v-id as char.
        ...
        if not avail filpayment then do:
           ...
           run savelog ("filclstda", "3.1) дата: " + string(g-today) + "; ID: " + g-ofc + "; aaa: " + v-iik + " - Платеж не найден !").
           return.
        end.
        ...
        if not avail arp then do:
           ...
           run savelog ("filclstda", "3.2) дата: " + string(g-today) + "; ID: " + g-ofc + "; aaa: " + v-iik + " - Счет не найден !").
           return.
        end.

        find first jl where jl.acc = filpayment.arp and jl.dc = 'C' and jl.jdt >= filpayment.whn
                       and  jl.cam = filpayment.amount + filpayment.amountcom and jl.rem[1] matches "*" + filpayment.name + "*"  use-index accdcjdt no-lock no-error.
        if not avail jl then do:
           message " Сумма клиента на транзитный счет еще не поступила !" view-as alert-box.
           run savelog ("filclstda", "3.3) дата: " + string(g-today) + "; ID: " + g-ofc + "; aaa: " + v-iik + " - Сумма клиента на транзитный счет еще не поступила !").
           return.
        end.
        ...
        if arp.cam[1] - arp.dam[1] < filpayment.amount + filpayment.amountcom then do:
           ...
           run savelog ("filclstda", "3.4) дата: " + string(g-today) + "; ID: " + g-ofc + "; aaa: " + v-iik + " - Нехватка средств на транзитном счете !").
           return.
        end.
        ...
        run savelog ("filclstda", "4) дата: " + string(g-today) + "; ID: " + g-ofc + "; aaa: " + v-iik).
        /*****************************************************/
        if v-ja then do:
          run savelog ("filclstda", "5) дата: " + string(g-today) + "; ID: " + g-ofc + "; aaa: " + v-iik).
          do transaction:
          /*galina*/ {filpay_kfm.i}
          ...
          v-glrem = v-npl + " " + v-npl1.
            ...
            if not avail sysc or sysc.chval = "" then do:
                   display " В настройках нет записи transf  !!".   pause.   return.
                   run savelog ("filclstda", "5.1) дата: " + string(g-today) + "; ID: " + g-ofc + "; aaa: " + v-iik + " -  В настройках нет записи transf  !!").
            end.
            ...
            run savelog ("filclstda", "5.2) дата: " + string(g-today) + "; ID: " + g-ofc + "; aaa: " + v-iik + " - В процессе определения режима работы - 'КАССА'/'КАССА В ПУТИ' произошла ошибка!").
                  return.
            end.
...
/****marinav*****/
            ...
            cursts.who = g-ofc .
/****marinav*****/
            run savelog ("filclstda", "6) дата: " + string(g-today) + "; ID: " + g-ofc + "; aaa: " + v-iik).
            find last acheck where acheck.jh = string(s-jh) and acheck.dt = g-today no-lock no-error.
            if not avail acheck then do:
                run savelog ("filclstda", "6.1) дата: " + string(g-today) + "; ID: " + g-ofc + "; aaa: " + v-iik).
          ...
          /*
            if filpayment.amountcom > 0 then do:
                ...
                if avail tarif2 then do:
                    ...
                    find current jh no-lock.
                    run setcsymb (s-jh, 150).
                    find first joudoc where joudoc.docnum = v_doc no-error.
                    if avail joudoc then do:
                           joudoc.info = filpayment.name . joudoc.perkod = filpayment.rnnto.
                    end.
                    run vou_bank(2).
...
/************************************************************/
   run savelog ("filclstda", "7) дата: " + string(g-today) + "; ID: " + g-ofc + "; aaa: " + v-iik).
  end.
