 ...
 * MODULE
        Закрытие опердня
 * DESCRIPTION
        Начисление процентов по депозитам "МЕТРО-СТАНДАРТ"
 * RUN
 ...
 * AUTHOR
        04.12.2008 - id00004.
 * CHANGES
        31/05/2011 evseev - убрал проверку if aaa.payfre <> 1 then do:
        30/06/2011 evseev - запрет пролонгации. ТЗ-1070
        04/11/2011 evseev - если начисл. % < 0.01, то аккумулировать в aaa.m10.
        22/11/2011 evseev - ведение журнала accr-m10
...
fname1 = "acrstd" + substring(string(g-today),1,2) + substring(string(g-today),4,2) + ".txt".
...
for each lgr where lgr.led = "TDA" and lgr.feensf = 1 :
    ...
    for each aaa where aaa.lgr eq lgr.lgr break by aaa.crc:
        ...
        if aaa.opnamt = 0 and aaa.cr[2] = aaa.dr[2] then
        do:
            aaa.cltdt = g-today.
            next account.
        end.
        ...
        /* Если проценты начиcляются ежедневно */
        if lgr.intcal = "D" then
        do:
            DoPay     = false.
            DoCap     = false.
            DoChgRate = false.
            ...
            do:
                ...
                if ev-date = date(acvolt.x3) - 1 then DoMature = true.
            end.
            else
            do:
            ...
            do:
                ...
                run PayInterests.
                if (aaa.regdt < 08/01/2011) and (v-intday2 > 0) then
    ...
    do:
        ...
        do transaction:
            ...
            if not avail trxlevgl or not avail trxlevgl11 then
            do:
...
Procedure CalcInterests.
    ...
    do :
        ...
        do:
            ...
            if available accr then
            do:
                if (accr.fdt <> g-today) then
                do:
                    create accr.
                    {accr.i}
...
Procedure PayInterests.
    ...
    do:
        ...
        if rcode ne 0 then
        do:
            put stream m-err unformatted "Не удалось сформировать проводку выплаты накопленных % : " aaa.aaa ", " string(truncate(v-pay,0)) " -> " rdes skip.
        end.
        else
        do:
            if not s-bday then
            ...
            if rcode ne 0 then
            do:
                put stream m-err unformatted "Не удалось отштамповать проводку выплаты накопленных % : " aaa.aaa ", " string(truncate(v-pay,0)) " -> " rdes skip.
            end.
        end.
    end.
End procedure.
...
Procedure EvaluateExpiryDate.
    ...
    if month(j_lstmdt) <> month(j_lstmdt + 1) then
    do:
        months = months + 1.
        if months = 13 then
        do:
    ...
    if months = 2 and days = 30 then
    do:
        months = 3.
        days = 1.
    end.
    if months = 2 and days = 29  and  (( (year(j_lstmdt)  + years) - 2000) modulo 4) <> 0 then
    do:
        months = 3.
        days = 1.
    end.
    ...
    if month(j_lstmdt) <> month(j_lstmdt + 1) then
    do:
        if month(j_expdt) = 3 and day(j_expdt) = 1 then
        do:
        end.
        else
            j_expdt = j_expdt - 1.
