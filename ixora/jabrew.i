...
modified by ja for special keys sequences processing 21/05/2001 (based on
jabre.i)
*/
def var key-i as inte.
def var vkey-i as char.
def var vkey as char.
def var vkeyfunction as char.
...
{&reframe}
  ...
  {&prechoose}
   ...
   if vkey <> "" then do:
      key-i = key-i + 1.
      vkey-i = entry(key-i, vkey, '^') no-error.
   ...
   if vkey-i = "" then do: 
      readkey.
      vkeyfunction = keyfunction(lastkey).
   end. 
   else do:
      vkeyfunction = vkey-i.
   end.
pause 0.
if vkeyfunction = "END-ERROR" then leave upper.
if vkeyfunction = "DELETE-LINE" then do:
 ...
 {&predelete}
  ...
  if vans then do:
...
else if vkeyfunction = "CURSOR-UP" then do:
	curflag = 1.
   ...
   else if vkeyfunction = "INSERT-MODE" then do:
   ...
   repeat:
	 ...
	 if clin < dlin then do:
	   ...
	   if vkeyfunction = "end-error" then do:
	    delete {&head}.
	    clin = clin0.
	    next upper.
	   end.
 ...
 else if vkeyfunction eq "CURSOR-DOWN" then do:
 curflag = 1.
    ...
    else if vkeyfunction = "PAGE-DOWN" then do:
	  curflag = 1.
    if crec <> lrec then do:
	color disp normal {&highlight} with frame {&framename}.
	   find {&head} where recid({&head}) = brec.
    ...
    else if vkeyfunction = "PAGE-UP" then do:
	  curflag = 1.
    if crec <> frec then do:
	color disp normal {&highlight} with frame {&framename}.
	   find {&head} where recid({&head}) = trec.
	...
	color disp messages {&highlight} with frame {&framename}.
     ...
     if vkeyfunction = "RIGHT-END" then do:
	  curflag = 1.
