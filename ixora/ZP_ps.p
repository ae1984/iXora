 ...
 * MODULE
        Платежная система
 * DESCRIPTION
        Формирование зарплатного RMZ
 * RUN
        Из программы autreg.p
 * CALLER
...
do:
  s-error = "".
    ...
    v-text = " LBARC ошибка sqn = " + string(num[1]) + " код завершения = "
     + s-error .
...
repeat:
 ...
 do:
   ...
   if substr(v-string,6,2) lt  substr(string(year(today)),3,2)
    then cc = "20".
   if substr(v-string,6,2) gt  substr(string(year(today)),3,2)
    then cc = "19".
 ...
 if v-string begins ":50:" or rep = "50" then
 do:
   if rep = "0" then do:
     rep = "50" .
   ...
   if index(v-ord,"/IRS/") ne 0 and v-irso eq "0" then
      v-irso = substr(v-ord,index(v-ord,"/IRS/") + 5).
   if index(v-ord,"/SECO/") ne 0 and v-secoo eq "0" then
      v-secoo = substr(v-ord,index(v-ord,"/SECO/") + 6).
 end.
 else
 if v-string begins ":52b:" then v-sbank = substr(v-string,6).
 else
 if v-string begins ":54c:" then v-rcbank = substr(v-string,6,9) .
 else
 if v-string begins ":57b:" then v-rbank = substr(v-string,6) .
 else
 if v-string begins ":59:" or rep = "59" then do:
   if rep = "0" then do:
    ...
    if index(v-ben,"/IRS/") ne 0 and v-irsp eq "0" then
      v-irsp = substr(v-ben,index(v-ben,"/IRS/") + 5).
    if index(v-ben,"/SECO/") ne 0 and v-secop eq "0" then
      v-secop = substr(v-ben,index(v-ben,"/SECO/") + 6).
 end.
 else
 if v-string begins ":70:" or rep = "70" then do:
    ...
    if index(v-det,"/NUM/") ne 0 and v-plnum eq "" then
    v-plnum = substr(v-det,index(v-det,"/NUM/") + 5).
    if index(v-det,"/KNP/") ne 0 and v-knp eq "000" then
    v-knp = substr(v-det,index(v-det,"/KNP/") + 5).
 end.
 if index(v-det,"/ASSIGN/") ne 0 then
 v-det1 = substr(v-det,index(v-det,"/ASSIGN/") + 8).
end.

input close .
pause 0.

if trim(v-sbank) ne trim(clecod) then
do on error undo, return on stop undo ,return:
   v-text = " В поле 52В код другого банка - " + trim(v-sbank) + " вместо " +
   trim(clecod).
   message skip v-text skip(1) view-as alert-box button ok title " ОШИБКА ".
   v-reterr = 1.
   return.
...
if v-bin = no then v-rnn = substr(v-ord,index(v-ord,"/RNN/") + 5,12).
              else v-rnn = substr(v-ord,index(v-ord,"/IDN/") + 5,12).
...
if aaa.crc ne v-crc  then
 do on error undo, return on stop undo ,return :
  v-text =  " SQN  = " + string(num[1]) +
  "."  + v-ref + "  валюта счета - " + string(aaa.crc)   +
    " не равна валюте платежа - "+ string(v-crc) + " ! " .
    message skip v-text skip(1) view-as alert-box button ok title " ОШИБКА ".
...
/*Снять спец инструкцию со счета юр лица, если она конечно есть*/
   find last comm.salary_p where comm.salary_p.acc = trim(v-acc) and comm.salary_p.sum = v-amt and comm.salary_p.knp = v-knp and comm.salary_p.del = ? exclusive-lock no-error.
   if avail comm.salary_p then do:
          find first aas where aas.aaa = comm.salary_p.acc and aas.ln = comm.salary_p.ln  and aas.chkamt = comm.salary_p.sum exclusive-lock no-error.
          if avail aas then do:
                  CREATE aas_hist.
                     ...
                     delete aas.
                     assign comm.salary_p.del = g-today
                            v-rtim = comm.salary_p.rtim. /*Время ввода дебет. части платежа u00777 03.11.2006*/
                     release comm.salary_p.
          end.
   end.
/*****/
 ...
 if vbal < 0 then do:
  ...
  if not avail arp then do :
     v-text = " SQN = " + string(num[1]) +
       ". " + v-acc + "  счет не найден в системе ! " .
      message skip v-text skip(1) view-as alert-box button ok title " ОШИБКА ".
      v-reterr = 4 .
      return.
  end.
  else do :
   if arp.crc ne v-crc  then
    do on error undo, return on stop undo ,return :
       v-text =  " SQN  = " + string(num[1]) +
          "."  + v-ref + "  валюта счета : " + string(v-crc) + " " + string(arp.crc)   +
...
if not avail bankl then do on error undo, return on stop undo ,return :
  v-text = " SQN = " + string(num[1]) +
           "."  + v-ref + " Не найден код банка " + v-rbank + " ! " .
  message skip v-text skip(1) view-as alert-box button ok title " ОШИБКА ".
  v-reterr = 5 .  /*  */  .
  return.
end.
...
if avail remtrz then do on error undo, return on stop undo ,return :
  message skip " SQN = " + string(num[1]) + "."  + v-ref + " уже обработан !"
          skip "REMTRZ   " remtrz.remtrz
          skip(1) view-as alert-box button ok title " ОШИБКА ".
  return .
end .
...
do on error undo :
  ...
  if v-date >= g-today then remtrz.valdt2 = v-date.
  ...
  remtrz.ord =  substr(v-ord,index(v-ord,"/NAME/") + 6,
                index(substr(v-ord,index(v-ord,"/NAME/") + 6),"/") - 1)
                + "/RNN/" + v-rnn .
 ...
 /* if v-bin = no then ben = trim(substr(v-ben,index(v-ben,"/NAME/") + 6)) + substr(v-ben,index(v-ben,"/RNN/"),17).
  ...
  if v-ben begins "/D/" then r-acc = substr(v-ben,4,20).
  else r-acc = substr(v-ben,1,20).
  remtrz.ba = r-acc.
  remtrz.bi = v-chg .
  ...
  if not avail bankl then do:
    ...
    if not avail bankt then do:
      ...
      if avail sysc then v-timclear = sysc.inval.
      else v-timclear = bankt.vtime.

      /* проверка на текущий день - вдруг он больше опердня */
      if remtrz.valdt2 = g-today and ((time > v-timclear) or (today > g-today)) then remtrz.cover = 2.
      repeat:
  ...
  remtrz.rcvinfo[1] = remtrz.rcvinfo[1]  + "/ZP/".
  remtrz.ref = (substring(trim(v-cif),1,6) +
             fill(" " , 6 - length(substring(trim(v-cif),1,6))))
             +  "ZPP" + (substring(trim(v-ref),1,16) +
             fill(" " , 16 - length(substring(trim(v-ref),1,12))))
             + (substring(trim(ourbank),1,12) +
             fill(" " , 16 - length(substring(trim(ourbank),1,12))))
  ...
  sub-cod.sub = "rmz".
  sub-cod.rcod =  trim(v-irso) + trim(v-secoo) + "," +
...
v-text = "Автоматическая регистрация зарплатного платежа " + remtrz.remtrz +
         " <- SQN = " + string(num[1]) +
...
procedure fm1.
  ...
  v-sumkzt = ''.
  /*v-sumkzt = trim(string(v-amt,'>>>>>>>>>>>>9.99')).*/
  ...
  v-racc = ''.
  if v-ben begins "/D/" then v-racc = substr(v-ben,4,9).
  else v-racc = substr(v-ben,1,9).
