 ...
 * DESCRIPTION
        Реестр по платежам Алматытелеком
 * RUN
 ...
 * AUTHOR
        12/09/2006 u00124
 * CHANGES
        19.09.2006 u00124 добавил столбец номер платежа
        09.10.2006 u00124 увеличил format(40)
*/
...
find first commonls where commonls.txb = seltxb and commonls.grp = selgrp and commonls.visible = yes no-lock no-error .
...
if available tcommpl then do:
  ...
  run mail("oud@almatytelecom.kz"  , "TEXAKABANK <abpk@elexnet.kz>", "биллинг", "", "1", "", outf + ".txt"). 
  run mail("dpuchkov@elexnet.kz", "TEXAKABANK <abpk@elexnet.kz>", "биллинг", "", "1", "", outf + ".txt").
end.
else do:
