 ...
 * MENU
        8-2-14-3
 * AUTHOR
        14.01.2004 nadejda - переделано полностью
 * CHANGES
        09.10.2006 u00600 - алиас "ast" заменен на "txb" для совместимости c r-brfilial.i
        22.02.2012 Lyubov - к СКВ относятся: AUD, GPB, SEK, CHF
        23.02.2012 Lyubov - как выяснилось, надо было просто убрать рубли
*/
...
for each t-gl:
  do v-dt = v-dtb to v-dte:
    for each txb.jl where txb.jl.jdt = v-dt and txb.jl.gl = t-gl.gl and lookup(string(txb.jl.crc), "1,4") = 0 no-lock use-index jdt:
      /* нужны только кредитовые проводки */
      if jl.dc = "d" then next.
...
for each t-gl:
  for each txb.aaa where txb.aaa.gl = t-gl.gl and lookup(string(txb.aaa.crc), "1,4") = 0 no-lock:
    ...
    if substr(txb.cif.geo, 3, 1) = "2" then next.

    /* тенговые счета не нужны
    if txb.aaa.crc = 1 then next.*/
    ...
    if v-stroka = 4 then do:
      ...
      if i = 1 and t-gl.glstr = "2223" and txb.aaa.expdt <= v-dte then v-stroka = 2.
        ...
        if txb.aaa.expdt = ? then v-stroka = 6.
        else do:
          v-days = txb.aaa.expdt - txb.aaa.regdt.
          v-god = v-days / 366.

          if v-god <= 1 then v-stroka = 5.
                        else v-stroka = 6.
        end.
      end.
    end.
...
for each t-accs break by t-accs.stroka by t-accs.clnsts by t-accs.crc:
  ...
  for each t-jl where t-jl.acc = t-accs.aaa no-lock use-index main:
    ...
    if avail txb.aab and txb.aab.rate > 0 then do:
      ...
      accumulate v-proc (total).
    end.
  end.

  v-cr = v-cr + (accum total v-sum).
  v-proccr = v-proccr + (accum total v-proc).
  ...
  if last-of(t-accs.crc) then do:
    ...
    do p = 1 to 2 transaction on error undo, return:
