...
define frame zsh
           ...
           cashofc.whn label "КОГДА" at 5
           remark2 label "ВАЛЮТА" at 4
           cashofc.amt label "СУММА" at 5
...
for each cashofc where whn eq g-today and ofc eq varofc by crc by sts:
    case cashofc.sts:
    ...
    if avail crc then 
       remark2 = crc.code.
    find ofc where ofc.ofc eq cashofc.who no-lock no-error.
            if avail ofc then remarkwho = ofc.name.
    find ofc where ofc.ofc eq cashofc.ofc no-lock no-error.
            if avail ofc then remarkofc = ofc.name.
                    
    displ cashofc.whn cashofc.amt
           with frame zsh. 
