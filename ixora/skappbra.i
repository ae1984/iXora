...
find last {&head} where {&where} use-index {&index} no-error.
    if available {&head} then do:
        ...
        if {&addcon} then do transaction on endkey undo, next upper:
            {&precreate}
            create {&head}.
            {&postcreate}
            {&postadd}
...
/*    else do:
        if {&addcon} then do transaction:
         ...
         if keyfunction(lastkey) = "end-error" then do:
           ...
           find {&head} where recid({&head}) = trec no-lock.
           find next {&head} where {&where} no-lock.
           trec = recid({&head}).
