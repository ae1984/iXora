 ...
 * MODULE
        Клиентская база
 * DESCRIPTION
        Установка льготных тарифов для VIP-клиентов  - все тарифы БЕСПЛАТНО
        включая даже те, которые были установлены вручную
 * RUN
        
 * CALLER
        clnlgot-1.p, clnlgotf-1.p
 * SCRIPT
...
if not avail t-cif then do:
  message skip " Клиенты с льготным обслуживанием вида 5 не найдены !" 
          skip(1) view-as alert-box button ok title " ОШИБКА ! ".
  return.
end.
...
/* льготные тарифы, установленные вручную, тоже меняем!
...
*/
      ...
      if p-oper then do:  /* создать/обновить льготу */    
        ...
        /* льготный тариф - 0 на все установки тарификатора */
