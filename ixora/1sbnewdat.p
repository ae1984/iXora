...
for each t-gl:
  for each txb.aaa where txb.aaa.gl = t-gl.gl no-lock:
    ...
    if v-stroka = 4 then do:
      /* если это юрлицо, и это счет-гарантия, и срок от 0 до 2 дней => строка "до востребования" */
      if i = 1 and (t-gl.glstr = "2223" or t-gl.glstr = "2208") and (txb.aaa.expdt - txb.aaa.regdt < 3 or txb.aaa.expdt = ? ) then v-stroka = 2.
      else do:
