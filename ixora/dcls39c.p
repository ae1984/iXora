 ...
 * CHANGES
        ...
        25/10/2012 madiyar - исключаем из расчета также счета 1859, 2858, 2859
        27/11/2012 madiyar - изменения в процедуре переоценки
*/
...
def stream s-file.
output stream s-file to value("dcls39c_" + string(year(g-today),"9999") + "_" + string(month(g-today),"99") + "_" + string(day(g-today),"99") + ".csv").

def stream s-file1.
output stream s-file1 to value("dcls39c_" + string(year(g-today),"9999") + "_" + string(month(g-today),"99") + "_" + string(day(g-today),"99") + ".log").


put stream s-file unformatted "Балансовый счет;Наименование счета;Остаток по номиналу;Вид валюты;Учетный курс;Рыночный курс;Разница между курсами;Сумма переоценки;Переоценка Дт5703;Переоценка Кт4703;Наименование филиала;Транзакция".
...
for each gl where gl.totact = no and gl.totlev = 1 no-lock:
  ...
  if gl.gl >= 185800 and gl.gl <= 185999 then next.
  if gl.gl >= 285800 and gl.gl <= 285999 then next.
  /*
  ...
  */
  ...
  for each crc no-lock:
     ...
     if avail glbal then do:
         ...
         if ((gl.gl >= 135100) and (gl.gl <= 135299)) or ((gl.gl >= 215100) and (gl.gl <= 215299)) then v-templ = "dcl0019".
         else v-templ = "dcl0012".
         ...
         put stream s-file unformatted ";" + v-city + ";" + string(s-jh) .
