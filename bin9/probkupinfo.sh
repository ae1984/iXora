...
Usage()
{
  ...
  echo "This value is used to convert date in the form of elapsed seconds\nfrom the start of the Epoch (00:00:00, January 1, 1970) to a local time."
  echo "Default value is $TimeZone." >&2
}
...
# 284    284      4  Value 0
# 288    288      4  Value 8192 (use this value as a "magic number")
...
# Magic number - Value 8192 at offset 288
    MagicNumber=`GetLong 288`
    
    case "$MagicNumber" in
      8192) ;; # Expected value
     16384) ;; # Expected value
     2097152) echo "$Script: $Backup uses a different byte order." >&2
            break;;
         *) echo "\
$Script: $Backup has wrong magic number (found $MagicNumber, expected 8192).">&2
            break;;
    esac
